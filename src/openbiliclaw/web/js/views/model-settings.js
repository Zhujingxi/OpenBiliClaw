...
export async function openMobileSettings(opener) {
  ...
  function moveSelected(delta) {
    ...
    const record = selectedRecord(state, state.activeRoute);
    if (!record) return;
    state = moveRouteItem(state, state.activeRoute, record.id, selectedIndex() + delta);
  ...
  function switchSettingsSection(nextSection, trigger = null) {
    ...
    if (currentSettingsSection === "models" && nextSection !== "models" && !confirmLeave())
      return false;
    ...
    window.requestAnimationFrame(() => {
      if (disposed) return;
      const panel = card.querySelector(`[data-mobile-settings-panel="${CSS.escape(nextSection)}"]`);
  ...
  savedRetry.addEventListener("click", () => {
    void loadSavedSync();
  });
  modelLoadRetry.addEventListener("click", () => {
    void loadModelSettings();
  });
  ...
  byId("mobileModelConnectionTypeGroups").addEventListener("keydown", moveTypeOptionFocus);
  ...
  byId("mobileModelEmbeddingEnabled").addEventListener("change", (event) => {
    ...
    numericValidation.afterDraftMutation();
    if (event.target.checked && state.models.embedding.providers.length === 0 && !providersLocked) {
      addConnection();
      return;
    }
    render();
  ...
  byId("mobileModelChatTimeout").addEventListener("input", (event) => {
    if (modelMutationBlocked() || modelControlLocked("models.chat.timeout_seconds")) return;
    state = updateRouteSetting(
      state,
      "chat",
      "timeout_seconds",
