  ...
  if (uiState.show_actions) {
    ...
    for (const b of btns) {
      ...
      if (b.action === "watch-later") {
        ...
        if (isSavedLocally("watch_later", savedItem.item_key))
        ...
        if (isSavedLocally("favorite", savedItem.item_key))
          btn.setAttribute("aria-pressed", "true");
...
function _initDelightVisibilityObserver() {
  ...
  if (!slot) return;
  _delightVisibilityObserver = new IntersectionObserver(
    (entries) => {
...
async function hydrateDelightTurns() {
  try {
    ...
    for (const turn of items) {
      ...
      if (!arr) {
        arr = [];
        byBvid.set(turn.subject_id, arr);
      }
      arr.push({
...
export function onStreamEvent(payload) {
  ...
  if (type === "refresh.pool_updated") {
    ...
    rerenderRuntimeDependentChrome();
    if (state.recommendations.length === 0 && recommendationLoadState === "failed-exhausted") {
      recommendationRecoveryAttempt = 0;
      recommendationLoadState = "failed";
    }
    if (state.recommendations.length === 0 && recommendationLoadState === "failed") {
      scheduleRecommendationRecovery();
    }
