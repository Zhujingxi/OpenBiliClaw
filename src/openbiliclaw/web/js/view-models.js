...
const UNKNOWNISH_TEXT = /* @__PURE__ */ new Set(["", "unknown", "none", "n/a", "未知"]);
...
export function getRecommendationImageLoadingAttrs(index, { eagerCount = Infinity, highPriorityCount = 2 } = {}) {
...
export function shouldAutoAppendRecommendations({
  ...
  return Boolean(
    userArmed && !loading && !autoAppendExhausted && activeTab === "recommend"
  );
}
...
export function normalizeSavedIdentity(item = {}) {
  ...
  return {
    ...
    content_type: normalizeText(item?.content_type) || (sourcePlatform === "bilibili" && contentId ? "video" : "")
  };
}
...
function formatCompactRuntimeTopicList(topics) {
  ...
  return full.replace(/小红书任务 \/ 小红书探索/g, "小红书任务 / 探索").replace(/小红书搜索 \/ 小红书探索/g, "小红书搜索 / 探索").replace(/抖音搜索 \/ 抖音热点/g, "抖音搜索 / 热点").replace(/YouTube 搜索 \/ YouTube 热榜/g, "YouTube 搜索 / 热榜");
}
...
export function normalizeRecommendation(item) {
  ...
  const sourcePlatform = normalizeSourcePlatform(item);
  const contentId = normalizeText(item?.content_id) || (bvid && !bvid.includes(":") ? bvid : "");
  return {
    id: Number(item?.id ?? 0),
    bvid,
...
export function formatPublishedTime(item, now = Date.now()) {
  ...
  if (Number.isFinite(parsed)) {
    ...
    if (diff >= -3e5 && diff < 6e4) return "刚刚";
    if (diff >= 0 && diff < 864e5) {
      return `${Math.max(1, Math.floor(diff / 36e5))} 小时前`;
    }
    if (diff >= 0 && diff < 6048e5) {
...
export function formatCountCn(n) {
  ...
  if (value <= 0) return "";
  if (value >= 1e8)
    return `${(Math.floor(value / 1e8 * 10) / 10).toFixed(1).replace(/\.0$/, "")}亿`;
  if (value >= 1e4)
    return `${(Math.floor(value / 1e4 * 10) / 10).toFixed(1).replace(/\.0$/, "")}万`;
  return String(value);
}
...
const TEXT_CARD_CONTENT_TYPES = /* @__PURE__ */ new Set([
  "tweet",
  "thread",
  "answer",
  "article",
...
export function getDelightUiState(delight, { highlightBvid = "" } = {}) {
  ...
  const scoreLabel = score >= 0.85 ? "大概率会戳中你" : score >= 0.65 ? "这条可能会拐到你" : "有点出其不意";
  ...
  if (normalized.state === "chatted" || normalized.state === "chatting") {
    const responseMessage = normalized.response_message || (normalized.state === "chatted" ? "这句已经记下，后面会更会试探。" : "");
    return {
      ...base,
...
export function normalizePoolStatus(status) {
  const topics = Array.isArray(status?.recent_pool_topics) ? status.recent_pool_topics : Array.isArray(status?.pool?.topics) ? status.pool.topics : [];
  return {
    pool_size: coerceNumber(status?.pool_available_count) ?? coerceNumber(status?.pool_size) ?? coerceNumber(status?.pool?.total),
    recent_replenish: coerceNumber(status?.last_replenished_count) ?? coerceNumber(status?.recent_replenish) ?? coerceNumber(status?.last_refresh_added),
    current_topic: normalizeText(topics[0]) || normalizeText(status?.current_topic) || normalizeText(status?.pool?.topics?.[0]) || null
  };
}
export function normalizeRuntimeStatus(status) {
  return {
    ...
    recent_pool_topics: Array.isArray(status?.recent_pool_topics) ? status.recent_pool_topics.map(normalizeText).filter(Boolean) : [],
...
export function getPoolStatusSummary(status) {
  ...
  const poolIsSufficient = runtime.pool_target_count > 0 && runtime.pool_available_count >= runtime.pool_target_count;
  ...
  return {
    available: `还有 ${runtime.pool_available_count} 条可换`,
    replenished: runtime.last_replenished_count > 0 ? `刚补进 ${runtime.last_replenished_count} 条` : runtime.last_discovered_count > 0 ? "这轮找到了内容" : poolIsSufficient ? "这会儿先不补货" : "这轮还没补进",
    topics: runtime.recent_pool_topics.length > 0 ? formatRuntimeTopicList(runtime.recent_pool_topics) : runtime.last_discovered_count > 0 ? "但可立即换的库存还没变" : poolIsSufficient ? "先把这一池给你慢慢换开" : "还在继续摸你的口味"
  };
}
export function getReadyRecommendationHint(status) {
  ...
  if (runtime.initialized_known && !runtime.initialized) {
    return {
      message: runtime.last_account_sync_error ? `画像初始化未完成：${runtime.last_account_sync_error}` : "画像尚未初始化，先完成初始化再来看推荐。",
      tone: "info"
    };
  }
  if (runtime.pool_available_count > 0) {
...
export function getMobileRecommendationHeaderState({
  ...
  const pendingOnly = runtime.pool_available_count === 0 && runtime.pool_pending_count > 0;
  ...
  return {
    ...
    poolChips: poolSummary ? [
      ...
      {
        value: pendingOnly ? `${runtime.pool_pending_count} 条` : runtime.manual_refresh_state === "running" ? runtime.pool_available_count > 0 ? "继续补" : "正在补" : runtime.last_replenished_count > 0 ? `补进 ${runtime.last_replenished_count} 条` : runtime.last_discovered_count > 0 ? "已发现" : poolSummary.replenished,
        label: pendingOnly ? "素材整理" : "最近补进",
        tone: "brand"
      },
      {
        value: runtime.recent_pool_topics.length > 0 ? formatCompactRuntimeTopicList(runtime.recent_pool_topics) : poolSummary.topics,
        label: "现在在忙",
        tone: "info"
      }
    ] : []
...
export function normalizeActivityFeed(payload) {
  const items = Array.isArray(payload?.items) ? payload.items.filter((item) => Boolean(item) && typeof item === "object").map((item, index) => ({
...
export function normalizeCognitionUpdateCard(item) {
  ...
  const contextLine = normalizeText(item?.context_line) || normalizeText(item?.contextLine) || fallbackContextLine;
  const explicitExpandHint = normalizeText(item?.expand_hint) || normalizeText(item?.expandHint);
  const expandHint = (() => {
...
export function buildNextCognitionHistoryState(currentState, nextSummaryPage) {
  const existingItems = normalizeCognitionHistoryItems(
    Array.isArray(currentState?.items) ? currentState.items : currentState?.recent_cognition_updates
  );
...
function normalizeInterestDomains(raw) {
  ...
  return raw.filter((d) => d?.domain).map((d) => ({
    domain: normalizeText(d.domain),
    weight: Number(d.weight ?? 0.5),
    specifics: Array.isArray(d.specifics) ? d.specifics.filter((s) => s?.name).map((s) => ({ name: normalizeText(s.name), weight: Number(s.weight ?? 0.5) })) : []
  }));
}
...
export function normalizeProfileSummary(summary) {
  return {
    ...
    exploration_openness: typeof summary?.exploration_openness === "number" ? Math.max(0, Math.min(1, summary.exploration_openness)) : 0.5,
    speculative_interests: Array.isArray(summary?.speculative_interests) ? summary.speculative_interests.filter((item) => item?.domain).map((item) => {
      ...
      return {
        ...
        specifics: Array.isArray(item.specifics) ? item.specifics.filter((s) => s?.name).map((s) => ({ name: normalizeText(s.name), confirmation_count: Number(s.confirmation_count ?? 0) })) : []
      };
    }) : [],
    speculative_avoidances: Array.isArray(summary?.speculative_avoidances) ? summary.speculative_avoidances.filter((item) => item?.domain).map((item) => ({
      domain: normalizeText(item.domain),
      reason: normalizeText(item.reason),
      confidence: Number(item.confidence ?? 0),
    ...
    recent_cognition_updates: Array.isArray(summary?.recent_cognition_updates) ? summary.recent_cognition_updates.map(normalizeCognitionUpdateCard).filter((item) => item.summary) : [],
    ...
    active_insights: Array.isArray(summary?.active_insights) ? summary.active_insights.filter((item) => item?.hypothesis).map((item) => ({
      hypothesis: normalizeText(item.hypothesis),
      evidence: Array.isArray(item.evidence) ? item.evidence.map((e) => normalizeText(e)).filter(Boolean) : [],
      confidence: typeof item.confidence === "number" ? Math.max(0, Math.min(1, item.confidence)) : 0.5,
      validated: Boolean(item.validated),
      created_at: normalizeText(item.created_at)
    })) : [],
    recent_awareness: Array.isArray(summary?.recent_awareness) ? summary.recent_awareness.filter((item) => item?.observation).map((item) => ({
