...
function renderStatusBar() {
  ...
  settings.addEventListener("click", () => {
    void openMobileSettings(settings);
  });
  ...
  if (state.degraded) {
    if (!existing) {
      ...
      existing.style.cssText = "background:var(--warning-soft);color:#d97706;font-size:12px;padding:6px 16px;text-align:center";
  ...
  for (const tab of TABS) {
    ...
    el.addEventListener("keydown", (e) => {
      ...
      if (target) {
        ...
        $tabBar.querySelector(`[aria-selected="true"]`)?.focus();
      }
    });
  ...
  onEvent(payload) {
    ...
    if (payload?.type === "config_reloaded") {
      window.dispatchEvent(
        new CustomEvent("openbiliclaw:config-reloaded", {
          detail: payload
        })
      );
    }
