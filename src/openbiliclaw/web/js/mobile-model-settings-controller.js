...
export function createMobileModelResourceCoordinator(options) {
  ...
  async function loadBoth() {
    return loadIndependentModelResources({
      ...
      onSnapshotBlocked: (snapshot) => options.onSnapshotBlocked?.(snapshot, { remote: false }),
...
export function readOwnMobileModelFieldError(byConnection, recordId, field) {
  if (!byConnection || typeof byConnection !== "object" || !Object.hasOwn(byConnection, recordId)) return null;
...
export function validateMobileModelNumbers(state) {
  ...
  const concurrency = chat.concurrency;
  const timeoutSeconds = chat.timeout_seconds;
  const outputDimensionality = embedding.output_dimensionality;
  ...
  for (const [index, connection] of (chat.connections || []).entries()) {
    const numCtx = connection?.num_ctx;
  ...
  if (typeof embedding.similarity_threshold !== "number" || !Number.isFinite(embedding.similarity_threshold) || embedding.similarity_threshold < 0 || embedding.similarity_threshold > 1) {
    addNumericError(result, {
...
function validationConnectionId(path, state) {
  ...
  const chatIndex = parts[0] === "models" && parts[1] === "chat" && parts[2] === "connections" ? Number(parts[3]) : Number.NaN;
  if (Number.isInteger(chatIndex)) {
    return String(state?.models?.chat?.connections?.[chatIndex]?.id || "");
  }
  const embeddingIndex = parts[0] === "models" && parts[1] === "embedding" && parts[2] === "providers" ? Number(parts[3]) : Number.NaN;
...
//# sourceMappingURL=mobile-model-settings-controller.js.map
