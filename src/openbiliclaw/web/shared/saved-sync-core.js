...
export function normalizeSavedItem(item = {}) {
  ...
  return {
    ...
    content_type: text(item.content_type) || (sourcePlatform === "bilibili" && contentId ? "video" : "")
  };
}
...
export function createStrictSavedApi(requestJsonStrict) {
  ...
  const readTimeout = 1e4;
  const writeTimeout = 12e3;
...
export function getSavedSyncPresentation(item = {}) {
  ...
  const actionable = !busy && status !== "synced" && status !== "already_synced" && !localOnly;
  return {
    status,
    labelKey,
    detailKey,
...
export function restoreSavedFocus(root, token) {
  ...
  const focusAction = (card) => {
    ...
    const action = actions.find((candidate) => candidate.dataset?.savedAction === token.action) || actions[0];
    action?.focus?.();
  ...
  if (token.kind === "list" || token.itemKey === "__list__") {
    ...
    const heading2 = root.querySelector?.("[data-saved-heading]");
    if (heading2) {
      heading2.focus?.();
      return true;
    }
  ...
  for (let cardIndex = 0; cardIndex < cards.length; cardIndex += 1) {
    ...
    sameIndex = cardIndex;
    const exact = Array.from(card.querySelectorAll?.("[data-saved-action]") || []).find((action) => action.dataset?.savedAction === token.action);
    if (exact) {
      exact.focus?.();
      return true;
    }
  }
  const index = sameIndex >= 0 ? sameIndex + 1 : Math.max(0, Math.min(Number(token.index) || 0, cards.length));
...
export function createDialogFocusController(options = {}) {
  ...
  let active = false;
  const focusables = () => Array.from(dialog?.querySelectorAll?.(FOCUSABLE_SELECTOR) || []).filter((node) => node.hidden !== true && !node.closest?.("[hidden], [inert]"));
...
export function createDurableTaskTracker(options = {}) {
  ...
  const foregroundHorizonMs = Number(options.foregroundHorizonMs ?? 2e4);
  const visibleDelayMs = Number(options.visibleDelayMs ?? 750);
  const hiddenDelayMs = Number(options.hiddenDelayMs ?? 5e3);
...
export function createSavedTaskCoordinator(options = {}) {
  ...
  const ownersByTask = /* @__PURE__ */ new Map();
  const taskByItem = /* @__PURE__ */ new Map();
  const recovering = /* @__PURE__ */ new Map();
  let disposed = false;
  ...
  return {
    ...
    async recover(rows, callbacks = {}) {
      ...
      await Promise.all([...grouped].map(async ([taskId, values]) => {
        ...
        const recovery = Promise.resolve().then(() => fetchTask(taskId)).then((task) => track(task, values, callbacks)).catch((error) => {
          if (disposed) return;
          track({
            task_id: taskId,
...
//# sourceMappingURL=saved-sync-core.js.map
