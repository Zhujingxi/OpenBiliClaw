...
This adapter is the sole owner of operational SQL used by the CLI.  Product
features continue to use repositories and units of work.
"""
...
class SQLiteOperationalStore:
    ...
    def backup(self, *, source: Path, destination: Path) -> Path:
        ...
        try:
            self._backup_into_descriptor(source=source_path, descriptor=descriptor)
    ...
    def _backup_into_descriptor(self, *, source: Path, descriptor: int) -> None:
...
__all__ = ["DatabaseBackupError", "RuntimeDatabaseHealth", "SQLiteOperationalStore"]
