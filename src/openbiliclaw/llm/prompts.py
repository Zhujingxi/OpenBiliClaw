...
_SINGLE_CONTENT_EVALUATION_SYSTEM_PROMPT = (
    ...
    "不要因为来源不同特殊处理评分逻辑。\n"
    "10. published_at 是来源提供的权威发布时间。不得根据模型知识截止时间或标题中的年份推测"
    "发布时间；对热点、时事、版本更新等时效性内容，应结合 published_at 判断内容是否仍然新鲜，"
...
_BATCH_CONTENT_EVALUATION_SYSTEM_PROMPT = (
    ...
    "10c. content_batch 中的 published_at 是来源提供的权威发布时间。不得根据模型知识截止时间"
    "或标题中的年份推测发布时间；对热点、时事、版本更新等时效性内容，应结合 published_at"
    "判断内容是否仍然新鲜，但不得仅因内容较新就覆盖与用户画像的真实匹配度。字段缺失或无效时"
    "保持中性，不得武断降分。\n"
