"""Provider-owned X manual-cookie form and verifier."""
...
X_CONNECTION_FORM = ConnectionForm(
    provider_id="x",
    ...
    fields=(
        FormField(
            ...
            min_length=20,
            max_length=65_536,
            pattern=r"(?s)(?=.*(?:^|;\s*)auth_token=[^;]+)(?=.*(?:^|;\s*)ct0=[^;]+).+",
        ),
    ),
)
...
class XCredentialVerifier:
    ...
    async def __call__(
        ...
        try:
            ...
            if "auth_token=" not in submission.cookie:
