"""X read capabilities and purpose-specific projections."""
...
from .manifest import POST_KIND, X_ID
from .models import XItem
...
class XProvider:
    ...
    async def search(self, query: SearchQuery, access: AccessHandle) -> ContentPage[ContentPreview]:
        ...
        return ContentPage[ContentPreview](
            ...
            next_cursor=ProviderCursor(provider_id=X_ID, value=page.next_cursor)
            if page.next_cursor
            else None,
        )
    ...
    def native_from_bytes(self, raw: bytes) -> NativeContent:
        return self.native(XItem.model_validate_json(raw))

    @staticmethod
    def native(item: XItem) -> NativeContent:
        return NativeContent(
            ref=ContentRef(
                provider_id=X_ID,
                content_kind=POST_KIND,
                provider_content_id=item.id,
                canonical_url=f"https://x.com/i/status/{item.id}",
            ),
            schema_version=1,
            payload=item,
        )
    ...
    def preview(content: NativeContent) -> ContentPreview:
        item = XProvider._payload(content)
        provenance = XProvider._provenance(content)
    ...
    @staticmethod
    def _payload(content: NativeContent) -> XItem:
        if not isinstance(content.payload, XItem):
            raise ValueError("X payload required")
    ...
    def _cursor(query: SearchQuery) -> str | None:
        ...
        if cursor is not None and cursor.provider_id != X_ID:
    ...
    def _ref(ref: ContentRef) -> None:
        if ref.provider_id != X_ID or ref.content_kind != POST_KIND:
            raise ContentIntegrationError(
