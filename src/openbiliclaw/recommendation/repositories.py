...
class SqliteRecommendationRepository:
    ...
    async def reward_context(self, shown_id: str) -> tuple[ShownRecord, Candidate]:
        """Resolve the durable exposure and attributed candidate for reward credit."""

        async with self.db.transaction() as session:
            row = await session.fetch_one(
                "SELECT s.record_json,c.candidate_json FROM recommendation_shown s "
        ...
        return (
            ShownRecord.model_validate_json(str(row[0])),
            Candidate.model_validate_json(str(row[1])),
        )
