    ...
    *,
    ...
    async def _degraded_mode_guard(request: Request, call_next: Any) -> Any:
        ...
        allowed = (
            ...
            # model routes is exactly when pulling a fix-carrying release matters,
    ...
    # api/routes/system.py. /api/health and /api/init-status moved to
    # api/routes/health.py (Phase 1 pilot 2). Both routers are included
    # at exactly this point in create_app() so the route registration order
    # (and therefore FastAPI's path-matching precedence) is identical to
    # the legacy inline handlers.
    from openbiliclaw.api.dependencies import HealthRouteDeps, SystemRouteDeps
    ...
    app.include_router(
        build_health_router(
            HealthRouteDeps(
                get_lan_ip=_health_lan_ip,
                health_profile_ready=_health_profile_ready,
                health_embedding_ready=_health_embedding_ready,
                embedding_required_for_init=_embedding_required_for_init,
                diagnose_embedding=_diagnose_embedding,
                embedding_pull_progress_view=lambda: _embedding_pull_progress_view(),
                progress_int=lambda v: _progress_int(v),
                degraded_issues_payload=_degraded_issues_payload,
                get_auth_gate=_get_auth_gate,
                get_init_coordinator=lambda: getattr(ctx, "init_coordinator", None),
                get_init_prereqs=lambda: getattr(ctx, "init_prereqs", None),
                get_account_sync_service=lambda: getattr(
                    ctx, "account_sync_service", None
                ),
                degraded=lambda: bool(getattr(ctx, "degraded", False)),
                degraded_reason=lambda: str(getattr(ctx, "degraded_reason", "")),
            )
        )
    )
    ...
    def _config_to_response(
        ...
        *,
        ...
        llm_projection = _native_llm_projection()
    ...
    async def update_config(payload: ConfigUpdateIn) -> ConfigUpdateResponse | JSONResponse:
        ...
        unknown_reset_fields = list(reset_fields)
        ...
        async with coordinated_config_write(config_path):
            ...
            # rebuild so the rebuilt model routes construct clients with the new
            # proxy. CN-direct clients never read this value.
            from openbiliclaw.network import set_outbound_proxy
