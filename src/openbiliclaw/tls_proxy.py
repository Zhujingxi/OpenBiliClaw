...
Sits in front of ``openbiliclaw-backend`` on :8443, terminates TLS with the
...
def _ensure_certs() -> None:
    ...
    if not _AUTO_GEN:
        raise RuntimeError(
            ...
            f"        then download https://<host>:8443/ca.crt and trust the CA).\n"
        )
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _handle(self) -> None:
        ...
        # (http://<host>:8443) that we rewrote the Origin to.
