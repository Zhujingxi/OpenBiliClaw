...
class WeiboSourceConfig:
    ...
    The backend obtains a short-lived guest session in memory; it never asks
    for, persists, or reports a user's Weibo login cookie as a credential.
    """
...
def _normalize_weibo_source_modes(value: object) -> tuple[str, ...]:
    """Normalize Weibo modes and repair the legacy creator-only dead end.

    Creator expansion deliberately consumes authors discovered by search or
    hot in the same cycle. A hand-written older config containing only
    ``creator`` therefore cannot produce anything; preserve the requested
    branch while adding the least surprising seed branch.
    """
...
def _normalize_weibo_non_negative_int(value: object, field_name: str) -> int:
    """Keep TOML booleans/floats from silently becoming Weibo budgets."""
