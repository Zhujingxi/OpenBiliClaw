...
def discover(
    source: str = typer.Option(
        ...
        "--source",
        "-s",
        help=("触发发现的内容源：bilibili、xiaohongshu、douyin、zhihu、reddit、bangumi 或 weibo。"),
        case_sensitive=False,
    ),
    ...
    force: bool = typer.Option(
        False,
        "--force",
        help="xiaohongshu / bangumi / weibo：忽略最小调度间隔强制执行一次。",
    ),
) -> None:
    """手动触发内容发现（按来源选择渠道）."""
    from openbiliclaw.soul.engine import SoulProfileNotInitializedError
    ...
    if source_normalized != "bilibili":
        raise typer.BadParameter(
            ...
            "bilibili、xiaohongshu、douyin、zhihu、reddit、bangumi、weibo。"
        )
