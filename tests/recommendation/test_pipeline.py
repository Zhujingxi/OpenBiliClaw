...
async def test_jobs_have_explicit_nonoverlap_and_budgets() -> None:
    ...
    jobs = recommendation_jobs(discovery=run, evaluation=run, expiry=run, replenishment=run)
    assert {x.resource for x in jobs} == {"network", "model", "database"}
...
def test_feedback_requires_shown_candidate() -> None:
    from openbiliclaw.recommendation.feedback import apply_feedback
    from openbiliclaw.recommendation.models import FeedbackKind

    with pytest.raises(ValueError):
        apply_feedback(candidate(1), FeedbackKind.LIKED)
    shown = candidate(1).model_copy(update={"state": CandidateState.SHOWN})
    assert apply_feedback(shown, FeedbackKind.LIKED).state is CandidateState.INTERACTED
...
async def test_rejection_admission_constraints_and_transition_trigger(tmp_path: Path) -> None:
    ...
    assert await SchemaMigrator(path).migrate() >= 4  # v4 added the recommendation tables
    db = SqliteDatabase(path)
    await db.open()
    repo = SqliteRecommendationRepository(db)
    item = candidate(30)
...
@pytest.mark.asyncio
async def test_expiry_and_mark_shown_helpers_persist(tmp_path: Path) -> None:
    from openbiliclaw.recommendation.feedback import expire_candidate, mark_shown

    path = tmp_path / "expire.db"
    await SchemaMigrator(path).migrate()
    db = SqliteDatabase(path)
    await db.open()
    repo = SqliteRecommendationRepository(db)
    item = candidate(31, state=CandidateState.SELECTED)
    await repo.add_candidate(item)
    shown = ShownRecord(
    ...
    assert (await mark_shown(repo, shown)).state is CandidateState.SHOWN
    assert (
        await expire_candidate(repo, item.candidate_id, CandidateState.SHOWN)
    ).state is CandidateState.EXPIRED
    with pytest.raises(ValueError, match="only admitted"):
        await expire_candidate(repo, item.candidate_id, CandidateState.DISCOVERED)
    await db.close()
