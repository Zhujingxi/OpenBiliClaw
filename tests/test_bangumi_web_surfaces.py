...
def test_setup_exposes_anonymous_bangumi_bootstrap() -> None:
    ...
    assert (
        "SourceStatus.INIT_SOURCE_KEYS || SourceStatus.SOURCE_KEYS).map(" in html
        or "SourceStatus.SOURCE_KEYS.map(" in html
    ), "init roster must derive from the shared roster"
