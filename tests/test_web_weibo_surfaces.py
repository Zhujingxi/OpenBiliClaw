"""Static contracts for the discovery-only Weibo frontend surfaces."""
...
def test_shared_roster_exposes_weibo_but_guided_init_filters_it_by_capability() -> None:
...
def test_desktop_settings_round_trip_anonymous_weibo_config_and_pool_share() -> None:
    ...
    assert "YouTube、Bangumi 与微博使用公开接口" in html
    assert "不参与初始化画像" in html
...
def test_weibo_current_docs_store_metadata_and_release_boundary_are_in_sync() -> None:
    ...
    assert "reddit / bangumi / weibo` 枚举" in storage
    assert "9 个 canonical family" in diagram
...
def test_popup_is_weibo_aware_without_new_platform_permissions() -> None:
    ...
    assert "weibo" not in manifest_text.lower()
    assert "sinaimg" not in manifest_text.lower()
