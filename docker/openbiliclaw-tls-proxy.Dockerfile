...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD python -c "import os,ssl,urllib.request,sys; port=os.environ.get('LISTEN_PORT','8443'); ctx=ssl.create_default_context(); ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE; sys.exit(0 if urllib.request.urlopen(f'https://127.0.0.1:{port}/ca.crl', context=ctx, timeout=4).status==200 else 1)"
