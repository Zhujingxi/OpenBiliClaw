...
| Formal discover / keyword dual-track / admission | required | PASS | Producer/CLI tests plus live search/ranked/latest smoke; inspiration axis, persisted cooldown shared by formal/inspiration and the shared public-query sanitizer passed. Rejected-only, `incomplete_results` and search-cap regressions prove inspiration keeps accepted rows as partial/degraded and never reports those outcomes as affirmative empty or complete | Real LLM admission/recommendation remains NOT_RUN |
| Eval / publication time / recommendation | required | NOT_RUN | Static/unit parity covers `repository` text cards, `created_at` publication and local-only save. Current cards visibly render owner/name, description and stars; richer source metadata is retained but not consumed by frontend JS | No real LLM-to-recommendation E2E artifact; topics/language/license/forks/issues/watchers are not yet rendered |
...
| `discover → pending_eval → real LLM → recommendation` | required | NOT_RUN | Pending | Must use configured local provider/model |
...
- LLM provider / model / route: not run; no LLM call was allowed in the safe smoke
...
- Required rows not PASS: real LLM init/recommendation、installed-browser UI、完整 rejected/partial/crash E2E、documentation/release-readiness
