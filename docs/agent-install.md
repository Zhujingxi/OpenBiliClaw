...
4. Installs Python dependencies for local mode, or generates missing LiteLLM/PostgreSQL infrastructure secrets in a local mode-`0600` `.env` and then builds / starts Docker Compose when `MODE=docker`; existing non-empty secret values and unrelated `.env` entries are preserved. X/Twitter discovery's `twitter-cli` and Reddit discovery's `rdt-cli` packages are part of the default dependency set, so AI one-line installs do not need an extra flag for either one
