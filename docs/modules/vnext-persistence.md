...
| worker runtime health | ✅ | Compose probe 要求 PID 1 为正式 worker、schema 位于 head；独立 Huey SQLite 通过 integrity check，并通过 `/proc/self/fd` 或 `/dev/fd` 连接 held inode，在 pathname 与 descriptor 前后同 inode且 parent 可写的前提下用内存 journal 执行 `BEGIN IMMEDIATE`、真实 `CREATE`/`INSERT`、`ROLLBACK`，不留下 probe artifact |
| 安全在线备份 | ✅ | `db backup` 仅在 Linux/macOS 支持：以 no-follow FD 固定 source，并用私有 hard-link set 保留 main/WAL/SHM/journal snapshot 语义；完整 SQLite snapshot 先写入 held、已 unlink 的 `0600` payload FD，目标名称在 payload 完整同步前始终不存在。macOS 用 `fclonefileat(payload_fd, dirfd, target)`、Linux 用 `O_TMPFILE` + `linkat(AT_EMPTY_PATH)` 原子 no-replace 发布；若 capability policy 拒绝 `AT_EMPTY_PATH`，仅在 `/proc/self/fd` 复核仍绑定 held inode 后使用 `AT_SYMLINK_FOLLOW` fallback。Directory sync 后先重查 parent pathname 与 held dir FD，再核对 held final FD 与 pathname identity。最终名称和 private staging 均不在 identity check 后 pathname-delete，避免删除并发替换；每个候选 parent 最多保留 32 个 `.obc-backup-source-*`，到达上限后必须在确认没有 backup 运行时显式清理。Windows/其他缺少安全 primitive 的平台在 destination reservation 前失败关闭 |
