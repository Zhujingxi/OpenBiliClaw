...
| `infrastructure.jobs.worker` | `build_default_source_registry()` provider；每个 source/feed job 从持久化 settings 重建七个 built-in connector，不使用动态插件或空 registry fallback |
...
settings commit 与 registry rebuild/publish 在同一串行临界区；worker 在下一次 job 自动读取新值。
...
这里没有“调用必在 execution timeout 的同一时刻返回”或“返回前一定已经写成 cancelled”的承诺：总等待最多还包含有限 cleanup window。若 insert/cancel 因数据库阻塞超过 cleanup bound，调用可在 row 尚未收敛前返回；done callback 会取走 late enqueue outcome，并对成功的迟到 insert 在后台再次写 `cancelled`。若后台 cancel 也失败，durable deadline 仍保证 row 到期后不能 claim，并由下一次 snapshot/claim 写成 `abandoned`。enqueue/cancel 失败都只按 exception class 记录，不包含 message/value，也不会触发未取回 task exception。
