...
| `features.sources.service` | `SourceTaskService.enqueue()`, `claim()`, `complete()`, `snapshot()`, `cancel()`；只允许 primary / fallback 含 browser 的 operation 进入 durable queue |
...
`SourceTaskRequest` 只接受七个 canonical source ID 与闭合 `SourceOperation`；service 在入队前核对 manifest 且只允许 primary 或 fallback 声明 browser 的 operation 持久化。claim 也只领取该来源的 browser-assisted operation。最早 pending / lease-expired task 通过条件更新获得随机 lease token；独立 session 并发 claim 测试证明同一任务只产生一个 owner。过期任务可以重新领取，旧 token 不得 complete。
...
`QueuedBrowserTransport` 在进入 enqueue 前预分配 task UUID，并用一个 `asyncio.timeout` 覆盖 enqueue + poll 全段。timeout 或调用方取消时，它会等待任何已在线程中的 insert 收敛，再把已知 UUID durable 标成 terminal `cancelled`、清除 lease，最后继续传播原始 timeout / cancellation。cancelled row 不再被 claim，旧 worker completion 会明确失败；重试只会看到旧 terminal row 和一个新 actionable row，不会留下重复待执行任务。扩展仍在自己的登录 tab 执行任务，task payload 不承载浏览器 Cookie。
