...
- `POST /v1/auth/login` verifies the configured PBKDF2 password and returns a one-time session bearer token; the route alone is bearer-exempt but retains origin, CSRF, body, timeout, and rate-limit controls
