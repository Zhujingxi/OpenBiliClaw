...
- `/v1/sources` and source form/connect/disconnect; connect accepts the provider form's secret `submission` mapping unchanged. `GET /v1/sources/{provider}/access-recipe` returns provider-declared browser artifact data (typed 404 when absent), while `POST /v1/sources/{provider}/access-material` accepts the exact declared artifacts and converges on the same form/verifier/vault path. Both plugin endpoints require static or minted bearer auth; the POST also requires matching device/CSRF headers
