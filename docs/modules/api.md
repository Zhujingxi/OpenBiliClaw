...
| `GET /api/chat/pending-confirmations` | ✅ | 返回 `{"count":N,"items":[...]}`，只列未结算的高优先级对象且最多 3 条：未验证假设 `confidence>=0.60`、open 疑惑 `interpretation_confidence>=0.50`，按置信度降序。`?count_only=1` 只返回 `{"count":N}`，供后续 badge/入口轻量轮询。用户主动列表不套用系统冷却。 |
