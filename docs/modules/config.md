...
四端共享同一份状态与渲染模块：`src/openbiliclaw/web/shared/model-config-state.js` 承载 DOM-free 状态转移（hydrate、route 增删改移、credential keep/set/env/clear、probe fingerprint、revision 冲突、circuit 视图、override 锁、changed-since-probe 检测、单连接向导 draft lens），`src/openbiliclaw/web/shared/model-config-render.js` 承载 descriptor-driven 字段 / credential / 连接类型分组 DOM 构造；桌面、移动与 `/setup/` 向导通过 ES module 直接引用，插件 popup 通过 `extension/scripts/sync-model-config-state.mjs` 生成的逐字节副本对齐（`tests/js/model-config-parity.test.mjs` 守卫）。`/setup/` 向导在 snapshot 报 `migration.state != "none"` 时改为渲染迁移 interstitial 并指向完整设置页的迁移面板，不再内联尝试 legacy 迁移，并在保存前披露将被保留的既有 fallback 连接数量。
