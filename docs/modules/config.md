...
`openbiliclaw.model_config` 提供配置重构使用的标准库领域层、原生 `[models]` 持久化边界，以及 legacy `[llm]` 的只读 compatibility adapter。`Config.models` 可以严格读取 `schema_version=1` 的原生配置；仅存在 `[llm]` 时，loader 会从原始 table 在内存中构造确定性迁移候选和脱敏 `MigrationReport`，但启动与普通保存都不会把它写回磁盘。阶段 4–7 已提供单记录 adapter、原生 Chat / Embedding 有序 route 核心与事务型 `ModelConfigService`；`RuntimeContext` 已能构造、交换和恢复隔离的模型候选，但配置 API、UI 与生产 consumer 尚未在 Task 8 composition cutover 前切换，仍按下文的 `[llm]` 配置工作。
...
- `ModelConfigService.save()` 在 resolved config path 的共享异步锁内重新读盘：先比较 revision，再合并已持久化 credential、应用完整 migration resolution、校验并构造完整 runtime candidate。legacy 首次显式保存先创建 mode-preserving、绝不覆盖同名文件的 `config.toml.pre-model-refactor*.bak`；随后用同目录临时文件执行 flush、文件 `fsync`、`os.replace` 与目录 `fsync`，最后交换 runtime candidate。写入、交换或交换期间取消失败时会恢复旧字节、mode 和旧 runtime 对象；取消在恢复后继续向上传播。
- `render_model_config_document()` 是 service 的唯一 model-authority source editor：完整 TOML 先解析，`[models...]` / `[llm...]` 表头用 `tomllib` 语义识别，只替换模型 authority；未知 table、注释、空白、混合位置与 CRLF 原字节保留。inline/dotted authority、损坏 TOML 或无法证明唯一 `[models]` 时失败关闭。私有 `_service_storage.py` 承担 base/local layer、原子写、恢复与 backup 细节，公开 DTO、协议和 facade 保持在 `service.py`。
- `config.local.toml` 的 `[models]` 继续作为只读高优先级 layer：快照公开精确 override path 与 source，保存 shadowed field 会得到字段化错误；其它字段可以写入 base，且 local 值不会被烘焙进 `config.toml`。legacy local model override 会阻止首次迁移，直到先显式转换该本机 layer。
...
- `save_config(..., models_authoritative=False)` 是普通调用默认值：保存前必须成功读取并解析已有目标文件，否则以不包含原始内容或底层错误详情的 `ConfigError` 中止且保持原字节不变；已有 `[models]`、`[llm]` 或两者会从目标文件的已解析原始表重新发射，保留未知 scalar / list / nested table、array-of-tables 顺序与凭据。高优先级 layer 只保护其 `override_paths` 中的字段，不会阻断其他已知 legacy 字段的显式编辑，也不会把覆盖值烘焙进主文件。`models_authoritative=True` 才只按 `Config.models` 写 `[models]` 并移除 `[llm]`；当前尚无普通运行入口调用这条迁移写路径。
...
当前仍不包含生产 consumer 的 `RuntimeContext` composition cutover、模型配置 API、UI 或 CLI 模型编辑器；Task 7 的 service、backup 与 staged runtime hooks 不能被推断为这些入口已经切换到 `[models]`。
