...
`openbiliclaw.model_config` 提供配置重构使用的标准库领域层、原生 `[models]` 持久化边界，以及 legacy `[llm]` 的只读 compatibility adapter。`Config.models` 可以严格读取 `schema_version=1` 的原生配置；仅存在 `[llm]` 时，loader 会从原始 table 在内存中构造确定性迁移候选和脱敏 `MigrationReport`，但启动与普通保存都不会把它写回磁盘。生产 `RuntimeContext`、CLI 调用链与 OpenClaw bootstrap 均从 `Config.models` 构造同一个全局有序 Chat route 和共享设置 Embedding route。模型编辑的权威后端入口现为 `/api/model-config`；桌面 Web 与浏览器插件均已接入权威列表/详情编辑器，legacy `/api/config` 只提供非权威兼容投影，CLI 编辑面仍待后续迁移。
...
当前 runtime composition、权威模型 HTTP API，以及桌面 Web / 浏览器插件模型编辑器已切换到 `[models]`。移动 Web 与 CLI setup/save 编辑器仍待后续任务迁移。普通保存仍按上文契约保留 parsed raw authority，不会静默把 legacy 配置改写为原生配置。
...
| `fallback_enabled` | bool | `false` | legacy 兼容开关；原生 `[models.embedding].providers` 直接用数组是否存在后续 Provider 表示 fallback，不写这个布尔值 |
| `fallback_provider` | string | `""` | legacy 第二个 embedding 备选 Provider。原生模型编辑器不使用该槽位，而是按 `providers` 的 1–10 项原序依次 fallback |
| `multimodal_enabled` | bool | `false` | 是否启用**封面图单独** embedding（image-only 向量，与文本同一模型空间），供 recommendation `precompute_delight_scores` 的封面视觉加成消费。默认关闭。开启后仍需当前 `model` 支持图像（如 `gemini-embedding-2`，或 `dashscope` + `qwen3-vl-embedding`）；本地 `ollama` + `bge-m3` 等纯文本模型会自动跳过，不报错。与 `[discovery].multimodal_evaluation_enabled`（vision LLM 评估）相互独立。浏览器插件与桌面 Web 都在原生 Embedding 共享设置区直接编辑该值；provider 类型从 descriptor 能力列表选择，无硬编码下拉 |
...
模型后端使用四个独立接口；桌面 Web 与浏览器插件已直接接入，CLI 编辑面仍待后续迁移。
...
桌面 Web 与浏览器插件「设置 → Models」都按 Chat / Embedding / Runtime 三个 tab 组织。Chat 把 primary 与 fallback 视作同一种稳定-ID 记录，按数组顺序显示、拖拽或按钮/键盘移动，第 1 项只在展示层派生为 Primary，最多 10 项；Embedding 使用同样的 1–10 项 Provider 顺序，但 model、维度、阈值与多模态开关只存在于唯一共享设置区。桌面 Web 选中记录后在右侧 inspector 编辑并在窄屏退化为 list → detail；插件固定采用顺序式 list → detail，Back 返回列表且不丢草稿或 selected ID。
...
模型保存只发送 revisioned `PUT /api/model-config`，通用设置仍独立发送 `PUT /api/config`，桌面与插件通用 payload 都不再包含 legacy `llm`。保存请求在途时整个模型 fieldset 锁定且拒绝第二次保存或草稿 mutation；失败保留原草稿，成功只用响应 snapshot 重新 hydration。`config_reloaded` 到达时，干净草稿自动 hydration 并保留仍存在的 active tab/selected stable ID；有未保存修改时只显示远端 revision 冲突提示。字段错误按 connection ID 映射，不依赖可变数组位置。精确 probe 用 request generation、revision、route kind、stable ID 和精确 draft fingerprint 关联完成结果；Embedding fingerprint 额外包含共享 settings，因此只重排或切换 inspector 可把结果挂回原 ID，但任何相关编辑或 revision 变化都会丢弃旧结果，且不会把 A 的状态渲染到 B。插件分别为 snapshot 与 descriptor 维护 latest-request gate：snapshot-only reload 可淘汰旧 snapshot 而保留仍有效的 descriptor，重叠的完整加载只安装最新 descriptor，旧 descriptor 成功或失败均被忽略。加载期间产生的本地编辑保留为草稿并记录迟到 snapshot 为远端更新；normal save 与一键操作由同一 save generation 串行化，save 开始时立即使旧 probe 失效，完成后再统一恢复控件。一键本地 Ollama 仅在当前 snapshot 与 descriptor 都成功应用、且加载期间没有编辑或其他 save 时发出 PUT。
