...
`openbiliclaw.model_config` 提供配置重构使用的标准库领域层、原生 `[models]` 持久化边界，以及 legacy `[llm]` 的只读 compatibility adapter。`Config.models` 可以严格读取 `schema_version=1` 的原生配置；仅存在 `[llm]` 时，loader 会从原始 table 在内存中构造确定性迁移候选和脱敏 `MigrationReport`，但启动与普通保存都不会把它写回磁盘。生产 `RuntimeContext`、CLI 调用链与 OpenClaw bootstrap 均从 `Config.models` 构造同一个全局有序 Chat route 和共享设置 Embedding route；legacy `/api/config` 投影/修改仍由 Task 9 迁移，CLI 的旧 setup/save 向导仍由 Task 14 迁移。
...
当前 runtime composition 已切换到 `[models]`；本阶段不声称模型配置编辑入口已经完成迁移。legacy `/api/config` 投影/修改属于 Task 9，图形 UI 属于后续 UI tasks，CLI setup/save 编辑器属于 Task 14。普通保存仍按上文契约保留 parsed raw authority，不会静默把 legacy 配置改写为原生配置。
...
- LLM：当前 UI 仍展示 legacy 默认/备选 Provider、并发、key/model/base_url、DeepSeek `reasoning_effort`、OpenRouter headers 与旧 module override 字段；这是 Task 9/UI 迁移前的兼容编辑面。生产 runtime 只消费 `Config.models`，旧 module override 不影响模型选择
