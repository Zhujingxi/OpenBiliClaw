...
- `Config.model_meta` 是不落盘的 `ModelConfigMeta(source, migration, override_paths)`，记录 `native / legacy / default` 来源、当前迁移状态，以及 `config.local.toml` / 环境变量贡献的精确覆盖叶路径；`models_meta` 与 `model_config_meta` 是只读别名。
