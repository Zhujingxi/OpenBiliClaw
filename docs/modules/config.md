...
四端共享同一套状态机契约，渲染层按表面收敛：`src/openbiliclaw/web/shared/model-config-state.js` 承载 Web 侧 DOM-free 状态转移，插件侧 strict TypeScript `extension/popup/popup-model-config-state.ts` 保留同一公共 API；两者由 `tests/js/model-config-parity.test.mjs` 的行为向量持续校验 hydrate、route 增删改移、credential keep/set/env/clear、probe fingerprint、revision 冲突、circuit 视图、override 锁、changed-since-probe 检测与单连接向导 draft lens。`src/openbiliclaw/web/shared/model-config-render.js` 承载 descriptor-driven 字段 / credential / 连接类型分组 DOM 构造与 `escapeHtml` 等转义原语；桌面与移动通过 ES module 同时引用 Web 模块，渲染器接受 `classPrefix`（桌面默认 `model`，移动传入 `mobile-model`）以输出各表面 stylesheet 实际覆盖的类名，`/setup/` 向导引用共享 state 模块与共享 render 模块的 `escapeHtml` 原语（其 descriptor/credential/连接类型渲染因向导布局保留本地实现，统一收敛为计划 §10 的后续项）。`toModelConfigPayload` 对 `enabled=false` 的 Embedding route 序列化空 providers（后端以 `embedding_disabled_with_providers` 拒绝 disabled-with-providers），路由项仍保留在客户端状态供同会话重新启用。`/setup/` 向导在 snapshot 报 `migration.state != "none"` 时改为渲染迁移 interstitial 并指向完整设置页的迁移面板，不再内联尝试 legacy 迁移，并在保存前披露将被保留的既有 fallback 连接数量。
