...
`openbiliclaw.model_config` 提供配置重构使用的标准库领域层和原生 `[models]` 持久化边界。`Config.models` 现在可以严格读取 `schema_version=1` 的原生配置；现有运行时 Provider、配置 API、UI 和 legacy `[llm]` 转换尚未切换，仍按下文的 `[llm]` 配置工作。仅存在 `[llm]` 时，本阶段不会在启动或普通保存中自动迁移。
...
- `render_model_config()` 以固定字段和数组顺序生成原生 `[models]`，不会写空的 inline secret 占位；`compute_model_revision()` 对归一化值、连接顺序和凭据指纹计算稳定 revision，revision 本身不包含凭据。
- `Config.model_meta` 是不落盘的 `ModelConfigMeta(source, migration, override_paths)`，记录 `native / legacy / default` 来源、当前迁移状态和 `config.local.toml` 覆盖路径；`models_meta` 与 `model_config_meta` 是只读别名。
- `save_config(..., models_authoritative=False)` 是普通调用默认值：已有 `[models]`、`[llm]` 或两者会从目标文件的已解析原始表重新发射，保留未知 scalar / list / nested table、array-of-tables 顺序与凭据，不把合并后的 `config.local.toml` 值烘焙进主文件。`models_authoritative=True` 才只按 `Config.models` 写 `[models]` 并移除 `[llm]`；当前尚无普通运行入口调用这条迁移写路径。
