...
> 以下 `[llm...]` 表只用于识别存量配置和解释迁移报告。新安装、示例、
> `/setup/`、bootstrap、Docker 与桌面打包均写原生 `[models]`；不要再以
> 本节作为新配置模板。使用 `openbiliclaw models list` 查看迁移状态，
> 使用 `openbiliclaw models ...` 完成显式转换或后续编辑。
...
- 安装包 `/setup/` 的模型步骤不调用本接口；它先 exact probe 当前 Chat draft，再携带 revision 和 credential action 调用 `PUT /api/model-config`。`/api/config` 在该页面只处理 B 站 Cookie 等非模型字段。
