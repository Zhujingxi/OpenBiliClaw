...
- 绝对 `directory`（如 `/srv/private/openbiliclaw/logs`）或 `~` 前缀的家目录相对路径会被脱敏为其最后一段（basename），`file_path` 只携带脱敏后的目录段与文件名（例如 `logs/backend.log`），不会把 `/srv`、`/Users/<user>`、`~` 等主机布局泄露到网络响应。
