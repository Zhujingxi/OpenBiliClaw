...
Web 三个表面共享 `src/openbiliclaw/web/shared/model-config-state.ts` 的 DOM-free 状态转移（hydrate、route 增删改移、credential keep/set/env/clear、probe fingerprint、revision 冲突、circuit 视图、override 锁、changed-since-probe 检测、单连接向导 draft lens），`src/openbiliclaw/web/shared/model-config-render.ts` 承载 descriptor-driven 字段 / credential / 连接类型分组 DOM 构造与 `escapeHtml` 等转义原语；`npm run build:web` 以类型剥离方式原位生成同名 `.js` ES modules，生成的 `.js` 是 checked-in distributable runtime assets，`npm run check:web-build` 在 CI 只读校验 TS/JS 逐字节一致。这样桌面、移动与 `/setup/` 的运行时 URL 继续使用 `.js`，而 clean Python/Docker/desktop 安装不需要 Node/npm。渲染器接受 `classPrefix`（桌面默认 `model`，移动传入 `mobile-model`）以输出各表面 stylesheet 实际覆盖的类名；`/setup/` 向导引用共享 state 模块与共享 render 模块的 `escapeHtml` 原语（其 descriptor/credential/连接类型渲染因向导布局保留本地实现，统一收敛为计划 §10 的后续项）。插件 popup 在 Phase E 后维护严格 TypeScript 实现 `extension/popup/popup-model-config-state.ts`，由 `build:popup` 生成忽略的 `.js` 运行时；它与 Web 实现通过 `tests/js/model-config-parity.test.mjs` 的行为向量保持契约对等，不再是 checked-in 逐字节副本。`toModelConfigPayload` 对 `enabled=false` 的 Embedding route 序列化空 providers（后端以 `embedding_disabled_with_providers` 拒绝 disabled-with-providers），路由项仍保留在客户端状态供同会话重新启用。`/setup/` 向导在 snapshot 报 `migration.state != "none"` 时改为渲染迁移 interstitial 并指向完整设置页的迁移面板，不再内联尝试 legacy 迁移，并在保存前披露将被保留的既有 fallback 连接数量。
...
- Windows 盘符限定（drive-qualified）的 `filename` 也不允许出现在 wire 形式中：在 Windows 上 `PureWindowsPath("logs") / "C:backend.log"` 会折叠为盘符限定的 `C:backend.log`，悄悄丢弃配置的目录，因此与目录侧规则保持一致——drive-relative 形式（`C:backend.log` / `c:backend.log` / `D:private\backend.log`，大小写盘符均覆盖）先剥离盘符再降为 basename，drive-only 形式（`C:` / `c:` / `D:` / `d:`，大小写盘符均覆盖）归为空字符串（此时 `file_path` 仅含脱敏目录与尾部 `/`，不携带盘符）。脱敏在主机 OS 方言之外独立生效，POSIX 后端与 Windows 后端产出一致。
