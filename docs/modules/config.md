...
- 绝对 `directory`（如 `/srv/private/openbiliclaw/logs`）、`~` 前缀的家目录相对路径、Windows 盘符绝对路径（如 `C:\Users\secret\logs`）与 UNC 路径（如 `\\fileserver\private\logs`）统一脱敏为其最后一段（basename），`file_path` 只携带脱敏后的目录段与文件名（例如 `logs/backend.log`），不会把 `/srv`、`/Users/<user>`、`~`、`C:` 或 `\\server` 等主机布局泄露到网络响应。
- 绝对 `filename`（如 `/srv/private/backend.log`）同样降为 basename。
...
**安全更新语义**：`PUT /api/config` 会把与当前 canonical 值脱敏后 wire 形式完全一致的 `directory` / `filename` 识别为“未修改回显”并跳过，因此前端只做 GET→ PUT 全量保存不会把绝对路径覆盖成 basename；只有提交与当前脱敏值不同的新路径才会真正改写配置。
