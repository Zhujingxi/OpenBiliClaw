...
| `[host]` | `api_host`, `api_port`, optional opaque `bearer_secret_ref`, optional PBKDF2 `password_hash` (written by `openbiliclaw set-password`) |
...
Secrets are never valid inline values. Model, host bearer, and content credentials are referenced through the credential vault. `host.password_hash` is a verifier, not a secret: it is generated by `openbiliclaw set-password`, redacted from diagnostics, and must not be hand-edited. Environment variables use the `OPENBILICLAW_` names implemented in `core.config`; CLI values have highest precedence.
