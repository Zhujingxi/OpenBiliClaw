...
- 绝对 `directory`（如 `/srv/private/openbiliclaw/logs`）、`~` 前缀的家目录相对路径、Windows 盘符绝对路径（如 `C:\Users\secret\logs`）、Windows 无盘符根路径（如 `\Users\alice\private\logs`，在 Windows 上会解析到当前盘根目录）与 UNC 路径（如 `\\fileserver\private\logs`）统一脱敏为其最后一段（basename）；根/卷根路径（`/`、`C:\`、`~`、`\`）与裸 `~user` 形式（如 `~alice`）脱敏为空字符串。`file_path` 只携带脱敏后的目录段与文件名（例如 `logs/backend.log`），不会把 `/srv`、`/Users/<user>`、`~`、`~alice`、`C:`、`\Users` 或 `\\server` 等主机布局泄露到网络响应。
