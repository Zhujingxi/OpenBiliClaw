...
| `inspiration_aspect_window_size` | int | `32` | coverage-aware 候选窗口上限：先从 like 二级兴趣构建较大的排序窗口，再由 `inspiration_interest_sample_size` 控制本轮真正脑暴的兴趣数量。合法范围 `1..96`，非法值回退默认值 |
| `inspiration_interest_sample_size` | int | `6` | 每轮真正送入 `discovery.keyword_brainstorm` 的 like 二级兴趣数量。合法范围 `1..16`，超上限会 clamp 到 `16`，非法 / 小于 `1` 回退默认值 |
| `inspiration_max_probe_searches_per_stage` | int | `12` | 单次 inspiration stage 的共享 grounding 搜索预算。regular + explore 同轮触发时共用一次 brainstorm 和这组搜索预算，不再各跑一遍。合法范围 `1..64`，超上限会 clamp 到 `64` |
| `inspiration_platforms_per_probe` | int | `2` | `platform_sources` 后端每个 probe query 最多扇出到几个已启用平台源。合法范围 `1..4`，超上限会 clamp 到 `4` |
| `inspiration_riskcontrolled_probe_budget` | int | `4` | 单次 stage 内风险受控平台源的 probe 预算；当前 B 站、抖音 direct 和 X cookie replay 属于 risk-controlled，会受该预算和自身 cooldown / 限流约束。`0` 表示本 stage 不使用风险受控平台源；合法范围 `0..32`，超上限会 clamp 到 `32` |
| `inspiration_search_pages_per_probe` | int | `1` | 每个 probe 在支持分页的平台源里最多翻几页，并把 Exa / You 等一次性搜索的请求量扩到 `results_per_query × pages_per_probe`。默认 `1` 保持成本不变；合法范围 `1..5` |
| `inspiration_search_results_per_query` | int | `5` | 每个 aspect seed query 每页请求搜索预览结果数量。合法范围 `1..10` |
| `inspiration_max_seeds_per_aspect` | int | `3` | 每个 aspect 从搜索预览中抽取的 inspiration seed 上限。合法范围 `1..8` |
| `inspiration_max_expansions_per_seed` | int | `4` | Profile Curator / Detail Expander 可为单个 seed 保留的横向扩展上限。合法范围 `1..12` |
| `inspiration_max_keywords_per_platform` | int | `12` | 每轮 inspiration stage 最多为单个平台额外插入的关键词数。合法范围 `1..48` |
