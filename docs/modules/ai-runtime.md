...
- `ModelRoute` checks tools, structured output, vision, context, streaming, and reasoning capabilities for the primary and every fallback at construction time. Any incompatible model on a registered route fails startup. Composition always registers and validates required Assistant and Understanding routes, while optional RecommendationBrief and visual-inspection routes/services are registered only when the configured model satisfies their complete requirements. `assistant.dialogue` requires streaming because both its SSE and compatibility non-streaming adapters consume the canonical streamed workflow.
