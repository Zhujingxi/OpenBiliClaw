...
- `ModelRoute` checks tools, structured output, vision, context, streaming, and reasoning capabilities for the primary and every fallback at construction time. Any incompatible fallback fails startup. `assistant.dialogue` now requires streaming because both its SSE and compatibility non-streaming adapters consume the canonical streamed workflow.
