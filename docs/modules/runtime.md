...
| 功能 | 状态 | 说明 |
|------|------|------|
| 桌面应用统一品牌图标 | ✅ | Windows 安装包 / EXE 使用多尺寸 `packaging/icon.ico`，macOS `.app` 使用完整 iconset 生成的 `packaging/icon.icns`；运行后的 Windows 系统托盘与 macOS 菜单栏不再绘制旧的临时圆环标记，而是从随包 Web 资源 `openbiliclaw/web/icon-192.png` 加载同一官方图标。PWA / favicon、桌面包与托盘因此共享同一品牌源图。 |
