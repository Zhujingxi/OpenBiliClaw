...
| 原生保存 service 热重载 | ✅ | `saved_sync_service` 是可替换组件：每次构造新 `BilibiliAPIClient` 时同步创建 router + `BilibiliNativeSaveAdapter` + `SavedSyncService`，任务 starter 统一走 `BackgroundTaskRegistry.track()`；所有组件构造成功后才与 client 一起原子发布。注入 / degraded context 只提供无平台 adapter 的 local-only service。 |
