...
| 移动 Models 资源与草稿收敛 | ✅ | `web/js/mobile-model-settings-controller.js` 分别跟踪 snapshot / descriptor readiness；先到的 config reload 不会跳过 descriptor，失败资源可单独重试，两者都 ready 前锁定编辑器。model / preset / credential / Embedding 共享设置变更统一刷新列表、exact-probe health 与错误投影；纯 Back 导航保留服务端错误。Runtime 非法整数在 PUT 前就地拦截，关闭 overlay 会解析 shell 重绘后的 live opener。 |
