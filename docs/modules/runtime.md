...
| 账号同步 | ✅ | `AccountSyncService` 同步 B 站账号历史、收藏和关注等信号；历史按 `view_at + 同秒 bvid 集合` 增量导入，收藏 / 关注只把新增 ID 转成画像事件，避免重放旧信号。新增事件先经 48h 跨源去重（扩展已上报的同一行为不再双计，`exclude_source="account_sync"` 防自压制），画像就绪后走 `ProfileUpdatePipeline` 增量管线而非整层重算；画像分析（管线及回退路径）默认受 360 秒墙钟上限保护，超时会取消并记录可见原因，不会把账号同步循环永久占住，文案明确模型服务 6 分钟未返回、Base URL / 模型名 / 网络 / 代理 / 响应过慢等常见原因，并经 `/api/init-status.detail` 同步给三端；同步失败以平台/原因分类经 runtime-status 下发到桌面 Web。X 来源启用且 Cookie 存在时同周期增量拉取 likes/bookmarks（tweet ID 集合去重，首轮从 events 表播种），并与 discovery producer 共用 `XSourceHealthStore`：冷却/登录失效/403 时不出网，首个 429 后不再紧接第二个请求。 |
...
**错误可见化**：每个拉取阶段的异常都会 `logger.warning` 并分类进状态字段 `last_sync_error_kind`（`auth_expired`——B 站 Cookie 已失效，优先级最高 / `x_auth_expired`——X Cookie 缺失或失效 / `x_blocked`——X 403 / `x_rate_limited`——X 429 / `error`——一般失败 / 空——干净同步时清除），经 `get_runtime_status()` 与 `/api/runtime-status` 的 `last_account_sync_error_kind` 下发。X 三种分类的成品文案由后端明确说明只影响 X 喜好子路径，避免来源卡显示 X 限流、首页却误报整个账号同步故障；`x_rate_limited` / `x_auth_expired` severity 为 `warning`，`x_blocked` 为 `error`。画像分析阶段的 LLM 故障额外经 `classify_llm_unavailability` 分类为 `no_provider` / `model_not_found` / `rate_limited`：前两类是配置故障，文案指向设置页（「修复后会自动恢复」）而非虚假的「稍后会自动重试」，`rate_limited` 是真实退避。桌面 Web 消费后端 `last_account_sync_severity` 选择 warning/error 样式；扩展 popup / 移动 Web / CLI 目前不展示此横幅。
