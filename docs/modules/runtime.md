...
| 启动优先的原子库存维护 | ✅ | `run_forever()` 在任何 delight 文案、候选协调器或后台 loop 启动前先调用 `_enforce_pool_cap()`；该入口每轮只调用一次 `Database.maintain_pool_inventory()`，先零 LLM 恢复合格历史 `suppressed` 结果，再统一 topic/source/stale/explore/raw 维护。低于 target 时 canonical available 优先，source/topic 超额可延期并在统一结果中观测。 |
...
`run_forever()` 的生命周期固定为“原子维护/历史恢复 → 可选 `prepare_delight_candidates()` → candidate coordinator 与 expression/background loops”。API 配置热重载和 OpenClaw 都通过新 controller 的同一个入口启动，因此 replacement controller 也会先复用已支付结果，再允许任何 provider 工作；composition root 不执行第二次恢复。
