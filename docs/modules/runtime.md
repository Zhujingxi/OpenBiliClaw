...
| 跨 digest 关键词库存整理 | ✅ | `KeywordPlanner.run_once()` 在计算平台缺口前，把当前画像 dislike 与平台 `avoid_topics` 交给 storage 原子整理 `regular/pending`。默认 24 小时内的安全旧 digest 行可继续计入水位和被 producer claim，原 digest/生成溯源不改；过龄、避雷、重复和超过动态高水位的行过期。整理或 all-digest count 能力缺失、抛错或返回畸形账本时，该平台立即退回旧的硬过期 + exact-digest count，不能因部分升级错误压住生成。`keyword_digest_grace_hours=0` 是独立回滚。 |
