...
| App-owned 配置应用队列 | ✅ | 设置保存先事务性落盘；若 dialogue execution、settlement、event owner 或 runtime handoff 正忙，`PUT /api/config` 返回 202，把修订交给 latest-wins 后台队列。正在应用的修订安全 drain，未开始的连续保存合并为最新配置；所有 runtime rebuild 共享同一 app-local reload lock。最终成功更新 last-good 并广播 `config_reloaded`；最新修订失败恢复 last-good、还原 process proxy 并广播 `config_reload_failed`，旧失败遇到更新修订时不覆盖新文件。shutdown 取消内存 worker，但已落盘配置由下次启动接管。 |
