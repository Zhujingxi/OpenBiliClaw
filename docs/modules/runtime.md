...
| 微博后台 discovery producer | ✅ | `WeiboDiscoveryProducer.produce_if_due()` 在微博平台族欠份额且启用时，按固定 `search → hot → creator` 顺序读取公开微博：search claim 统一关键词，hot 只把热搜词转成搜索 query，creator 只使用同轮已经看到的作者 UID。分支日预算 / 最近健康写 `weibo_discovery_runs`，HTTP 429 与 outcome 短退避写 `weibo_discovery_state`，生产性 cadence 单独写共享 `source_producer_runs`；关键词只有最终 handoff 才 used，明确空失败，瞬态 / schema / normalizer / 未执行分组 rollback，同轮已抓候选仍保留。结果显式返回 `ran / made_progress / productive_supply`，零保留不会冒充补货。所有分支只 enqueue `content_type="post"` 的 raw candidates；没有用户 Cookie、扩展任务、init 事件或站内写操作。`RuntimeContext` 只在来源启用时创建并拥有 `WeiboClient`，热重载 / shutdown 关闭旧 client；状态页只读前两张微博表，零上游 I/O。 |
...
桌面首页同时读取本地-only `/api/sources/status`，把它明确命名为「来源接入」后与上面的「账号同步」组合展示：九个平台中已启用来源的缺凭据、不完整、过期、失败、受阻、限流或未知状态会点名平台并原样显示后端 `detail`；正常的 `unverified` / `syncing` 和无需登录来源不进入故障横幅。微博项只读取 producer 本地 ledger/cooldown，不会因状态轮询访问微博。这样扩展心跳、公开源与命令行凭据状态都可定位，但不会被冒充为 `AccountSyncService` 的同步阶段。
...
微博 steady-state discovery 是项目自有的匿名 HTTP 路径。`produce_if_due()` 要求 `[sources.weibo].enabled=true`、画像可读、来源低于 quota、未处于持久 cooldown 且到达 `min_interval_minutes`；手动 `--force` 绕过生产性 cadence 与 outcome 短退避，但不绕过来源开关、预算、HTTP 429 cooldown 或 pool gate。固定执行顺序承担 creator seed 依赖：

- `search`：从 `KeywordFetchCoordinator.claim("weibo")` 领取统一 regular 关键词；请求、归一化与 pipeline handoff 都成功才 `used`，上游有证据的 empty 置 `failed`，timeout / network / upstream / schema / normalizer / 未执行 claim 回滚。claim 池暂空或没有 coordinator 时都可回退 Soul 兴趣词，但只接受真实字符串，不把结构值字符串化成 query。
- `hot`：读取 `hotSearch.data.realtime` 后仅提取少量 query seed，再复用 `search_posts()` 获取真实 `mblog`；热搜数字和热词自身从不成为候选或互动指标。
- `creator`：只读取同一轮 search / hot 归一化帖子中的数字 UID；没有种子返回 `no_creator_seeds`，不伪造关注列表或长期跟踪账号。

三分支各有 UTC 日预算，计数只覆盖最终去重、截断并经 candidate pipeline 过滤后实际保留的候选。候选按 `weibo-search` / `weibo-hot` / `weibo-creator` 分组进入共享 `DiscoveryCandidatePipeline`，由统一 admission floor 和后台 evaluator 收口；producer 不 inline 写 `content_cache`。每个实际执行 mode 的 `units/discovered/reason/error_code` 写入 `weibo_discovery_runs`，其中 `units` 是最终保留数，既供 UTC 日预算，也供 `GET /api/sources/status` 读取各 mode 最近结果；`WeiboClientError(code="rate_limited")` 会把有界 `Retry-After` 写入 `weibo_discovery_state`。明确空、成功但零 handoff、基础设施故障还会分别写 300 / 120 / 60 秒 outcome 退避，防止空轮热循环；这三类不进入来源 rate-limited 健康态。只有本轮最终实际保留候选时才另写 `source_producer_runs` 并清除 outcome 退避，后者仅是跨重启 cadence 地板，不参与健康判定。schema 漂移、访客拒绝或 transport 错误保留同轮 partial 内容并给稳定 reason，不切换到用户 Cookie、浏览器任务或详情 N+1。client 同时按 endpoint 校验 MIME；search empty 只接受明确 zero-total / known empty container，非空未知 card fail closed。归一化发布时间统一转 UTC，invalid / future 留空。
