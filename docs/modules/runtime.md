...
| 功能 | 状态 | 说明 |
|------|------|------|
| 扩展原生保存共享 runtime | ✅（5/6 executor 已接） | 扩展已定义与后端一致的 `native_save` task/result allow-list、canonical HTTPS URL 规则、`NATIVE_SAVE_EXECUTE` / `NATIVE_SAVE_RESULT` 消息契约和 active-tab runner。runner 与 legacy dispatcher 共用 `globalThis` mutex，在单一绝对 deadline 内等锁/加载/执行，严格校验 final tab 与 sender URL、tab/ID/item/platform；content 用 256 项 bounded outcome-promise cache 保证近期重复消息只复用并重放同一结果。Reddit/X、YouTube、小红书与抖音 executor 已接入 source dispatcher；startup/install/alarm/wake/direct execution 全部等待同一个共享 MV3 recovery barrier，再领取任务，仅记录 runner 自有 tab ID 的可选 `chrome.storage.session` 恢复会关闭该 orphan。Reddit 网络不确定或 2xx 未确认写入不回退点击；X 只使用目标邻近结构化 rate 状态或相对动作前基线新增/更新的 toast；YouTube favorite 精确匹配 `OpenBiliClaw` 并在 create 后 close/reopen，Watch Later 只认 `WL`。小红书/抖音只接受 exact canonical identity + current page + target；登录/不可用节点必须在祖先与 computed style 上有效可见，隐藏 SPA 模板不阻断；收藏控制必须位于唯一 exact content-ID 详情容器内，且最近 identity ancestor 必须就是该容器，隐藏祖先、嵌套推荐卡与全局同名控制均排除，再以 selected 确认且 count 变化不算成功。request fallback 前重新查询 selected，等待期间已选中则零点击 `synced`。risk fingerprint 只读取该详情容器并按具体 DOM 事件 identity 与动作前基线做 directional after-minus-before 比较，全局无关 toast、stale event 的移除或重排不会误报。当前 live page 未暴露完整稳定 request contract，production 不发收藏 request；fixture 覆盖 request success/明确 rejection/429/不确定写入禁 fallback。五者均只通过 fixture 测试、尚未真实账号验证；知乎 executor 仍待接。 |
| 扩展原生保存 broker 与 adapter 注册 | ✅（5/6 executor 已接） | `RuntimeContext.extension_native_save_broker` 是热重载不替换的 test-injectable 稳定实例；local/degraded construction 与 config rebuild 都注册六平台 adapter，service/router 会替换而 broker 不变。wake best-effort 发布 `<slug>_task_available`。Reddit/X、YouTube、小红书与抖音 executor 已接入但尚未真实账号验证；知乎 executor 仍待接。 |
...
| 原生保存 local-first 入口 | ✅ | 自动和手动同步都复用 `SavedSyncService.create_sync_task()` / `run_sync_task()`；`POST /api/saved/{list_kind}` 先提交本地 membership。`unsupported_adapter_missing` 可重试，`unsupported_content_type` 仍为终态；已接线的 Reddit/X/YouTube/小红书/抖音 executor 仍可能因扩展离线进入 `extension_required`，知乎等待后续 executor。 |
...
扩展共享原生保存基础（5/6 executor 已接：Reddit/X、YouTube、小红书与抖音仅 fixture 验证、尚未真实账号验证；知乎待接）：
