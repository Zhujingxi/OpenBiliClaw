...
| 连续候选评估协调器 | ✅ | `CandidateEvalCoordinator` 是 runtime 内唯一 claim owner：默认期望 3 个、每个最多 30 条的 LLM worker，任一槽位完成即补位；主协调任务串行执行 token-aware 落库与 admission，最多 90 条在途。有效 worker 为 `min(candidate_eval_concurrency, max(1, llm.concurrency-1))`，达到 `pool_target_count` 立即停止新 claim。60 秒 tick 仅是漏通知安全唤醒，候选入队与库存消费走事件唤醒。 |
