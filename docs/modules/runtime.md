...
| 原生保存 service 热重载 | ✅ | `saved_sync_service` 是可替换组件：每次构造新 `BilibiliAPIClient` 时同步创建 router + `BilibiliNativeSaveAdapter` + `SavedSyncService`，只有顶层 runner 通过 task starter 进入 `BackgroundTaskRegistry.track()`，service-owned heartbeat/save/watchdog 内部保留至 I/O 结束。重载先取消旧 registry inflight；所有新组件构造成功后才与 client 一起原子发布，任一构造失败则保留完整旧组件。注入 / degraded context 只提供无平台 adapter 的 local-only service。 |
| 原生保存 local-first 入口 | ✅ | 自动和手动同步都复用 `SavedSyncService.create_sync_task()` / `run_sync_task()`；`POST /api/saved/{list_kind}` 先提交本地 membership，再按当前 `saved_sync.auto_sync_enabled` 决定是否启动后台任务。手动 `/sync` 无视该默认关闭开关，平台失败只更新 durable item result，不回滚本地记录。当前 runtime 只注册 Bilibili write adapter，其它平台返回 `unsupported`。 |
