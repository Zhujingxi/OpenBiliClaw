...
| 桌面应用统一品牌图标 | ✅ | Windows 安装包 / EXE 使用多尺寸 `packaging/icon.ico`，macOS `.app` 使用完整 iconset 生成的 `packaging/icon.icns`；运行后的 Windows 系统托盘与 macOS 菜单栏不再绘制旧的临时圆环标记，而是从随包 Web 资源 `openbiliclaw/web/icon-192.png` 加载同一官方图标。所有派生图使用透明外缘，深色系统背景下不再出现白色方框；Windows PyInstaller 启动页也会加载同一品牌图标。PWA / favicon、桌面包、启动页与托盘因此共享同一品牌源图。 |
...
| XHS 自动发现停止门与平台熔断 | ✅ | XHS `/next-task` 每次领取 legacy search / creator / bootstrap 前现读热重载后的来源开关和全局 scheduler；关闭时旧任务保持 pending、扩展只得到 204，不再打开自动抓取页面。search / creator 领取间隔与平台冷却写入 `xhs_task_runtime_state`，跨后端 / MV3 重启生效；安全验证、操作频繁或 429 触发 1 小时冷却，producer 停止产词，`/api/sources/status` 在来源仍开启时显示 `rate_limited/feed_paused`。明确由用户发起的 native-save 独立于 discovery 开关，但不会越过安全冷却。 |
