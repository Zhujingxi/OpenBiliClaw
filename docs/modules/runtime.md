...
| 连续候选评估协调器 | ✅ | `CandidateEvalCoordinator` 是 runtime 内唯一 claim owner：配置与构造器均硬限制为最多 3 个、每批最多 30 条，即最多 90 条 raw 在途；任一 worker 完成即补位。主协调任务串行持久化 token-owned 评分，再按 copy-aware headroom admission。API / OpenClaw 会将其实际构造的抖音、YouTube、知乎 producer 显式标为由协调器拥有评估权：producer 只 enqueue，`DiscoveryCandidatePipeline.on_candidates_enqueued` 恰好一次唤醒协调器，绝不再同步 `drain_pending()` 另领 claim；隔离/CLI producer 默认保留旧 inline drain。projected 只等于 `available + admitted_pending_copy + evaluated_pending_admission`，raw pending/evaluating 不计入；超出 headroom 的达标结果保留为 durable `evaluated`。有效 worker 为 `min(candidate_eval_concurrency, max(1, llm.concurrency-1))`，60 秒仅作 safety wake。 |
...
- `candidate_evaluation_owned_by_coordinator`：API / OpenClaw 在 coordinator attach 后只对各自实际构造、且会同步 drain 的 Douyin / YouTube / Zhihu producer 置为 `True`。这些 producer 的 raw enqueue 已由 pipeline 回调同步 `notify("candidate_enqueued:pipeline")`，因此它们不得再调用 `drain_pending()`；这让所有 runtime claim 都受 3×30 上限约束。未接入 coordinator 的独立 producer 保持 `False`，继续兼容旧 inline drain。
