...
Global `--config` and `--data-dir` options are accepted either before or after the selected command (and around nested source/profile commands). Every command and nested command has concise `--help` discovery text. `check` performs a complete lifecycle/readiness check without binding a socket and prints `configuration and runtime check passed` on success; a missing explicit config path exits with a concise argparse error and no traceback. `serve` starts the composed FastAPI host using `[host]` settings. `set-password` never echoes or stores the raw password; `ext-token` stores only a SHA-256 token hash and prints the raw token once. `export` always writes one versioned ZIP archive regardless of the output filename extension; `--include-config` adds the selected `--config` (default `config.toml`) with vault references intact and password verifiers redacted. `import` validates the format and migrates the staged snapshot forward before installing it into the data directory; it refuses a non-empty destination unless `--force`. The removed monolithic commands are not compatibility-supported.
