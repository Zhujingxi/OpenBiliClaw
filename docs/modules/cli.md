...
CLI 与 API 是不同进程时不共享实时 coordinator。`init` 遇到 `GuidedInitError` 仍按原样输出状态面板并以 1 退出，同时 best-effort 调用 `Database.record_external_init_failure()` 把 reason、detail 和失败阶段写入共享 `init_runs`；若 API 正有活跃 GUI run 则不覆盖。这样 Docker 宿主机命令结束后，`/setup/`、`/web`、移动 Web 和插件能恢复同一真实失败原因；该记录只代表终态，不声称提供 CLI 跨进程实时进度。

`cli_models.py` 对 Click 类型和命令基类只依赖公开 `click` 包，不再导入 Typer 的私有 `_click` 转发；因此项目允许的 Typer 0.24+ 不会在 CLI（包括 `serve-api` 入口）导入阶段中断。
