...
| `openbiliclaw db backup DESTINATION` | Publish a complete held snapshot with an atomic no-replace primitive (locked fixed slot + held-FD `fclonefileat` on macOS; unlinked `O_TMPFILE` + `linkat(AT_EMPTY_PATH)` on Linux), then revalidate exact bytes, SQLite integrity, and the final pathname/FD after directory sync. macOS reuses a zeroed successful slot and boundedly retains nonzero failures; Windows or a platform without the primitive fails before destination reservation. |
