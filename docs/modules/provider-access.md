...
- `AccessService` provides connect/status/replace/disconnect/rehydrate. Successful-verification caching is bounded by both a five-minute default TTL and provider expiry, and credential replacement always re-verifies. Composition startup idempotently restores and re-verifies the default single-account connections in the vault, so connections survive restart. One exact, form-validated credential resubmission may claim a restored connection and narrow it to the requested permissions; the broker compares inside the opaque vault boundary, re-verifies, and rejects mismatched or subsequent duplicates as `already_connected`. Credential replacement also consumes this one-time restart allowance;
