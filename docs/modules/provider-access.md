...
Browser-extension session, managed-browser, OAuth, and CLI/browser credential import are not implemented AccessMethods. Adding one requires a replayable typed method and the same secret-resolution boundary; presentation code cannot introduce browser-specific credential payloads.
