...
> 状态：基础设施已接入独立 vNext worker 的 profile/feed 用例，chat application adapter 也已实现；尚未接入公开 API、legacy runtime、CLI 或前端。当前 v0.3 legacy 模型路由仍是现有用户请求的权威。
...
`tasks.py` 目前定义六个可复用 typed task：`profile_delta`、`keyword_generation`、`candidate_assessment`、`candidate_batch_assessment`、`chat_response` 和 `recommendation_explanation`。每个 agent 都注册会触发 PydanticAI output retry 的 task-specific validator：
...
- 单候选与 batch candidate 输出不含 application-owned assessment row ID，并强制复制输入 content ID / profile revision；batch 还要求每个输入恰好出现一次且顺序一致；
- chat response 必须返回非空、长度受限的可直接流式输出文本；
...
对应 Pydantic Evals YAML 位于 `evals/datasets/`。既有四份 versioned dataset 分别加载 code-backed offline evaluator，检查画像 evidence/change/concept、关键词 uniqueness/relevance/source-neutral、单候选 identity/score range/topic 与推荐 grounding/length/concept；batch/chat 由 application contract 测试覆盖。metadata 明确记录每个 case 的 range、required concept 和数量边界。离线测试同时覆盖不等于 expected output 的有效替代答案与逐项反例。推荐解释 dataset 另带使用 `openai:obc-analysis` 的 `LLMJudge` 主观质量 rubric，供显式 eval run 经 LiteLLM 使用；CI 只反序列化并检查其配置，不执行 judge 或 live provider。上述 deterministic grounding 只是低成本 overlap guard，不声称识别所有 hallucination；主观质量与事实忠实度仍由显式 `LLMJudge` eval 层覆盖。profile 与 batch candidate 已接入 vNext worker，chat adapter 等待 Task 21 HTTP 接线；它们都未替换 legacy 业务调用。
...
一行 Docker installer / `agent_bootstrap.py --mode docker` 会在跨进程锁内补齐 `.env`，拒绝 symlink，保留无关条目，用同目录 mode-`0600` 临时文件写入、flush/`fsync` 后原子替换，并在支持时同步目录。手动 Compose 用户必须先生成 `.env`。不要提交该文件。当前 backend 与 vNext worker 暂时使用 master key 访问同一私有 Compose 网络；后续 cutover 任务应改发最小权限 virtual key。
