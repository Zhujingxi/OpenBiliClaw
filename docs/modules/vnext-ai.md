...
- `TaskRunner` 在调用前验证输入，经 PydanticAI 执行并验证输出；`run()` 返回完整 typed output。`stream()` 对 `semantic_retry_limit=0` 的交互任务实时产出 schema-valid typed snapshot，并在结束时运行完整 output validator；需要语义 retry 的任务则按 attempt 缓冲 snapshot，只有最终 validator 接受的 attempt 才对调用方可见，失败 attempt 不泄漏。两条路径共享 timeout、usage、cancellation 与一次 `ai_runs` lifecycle。它不实现 provider fallback、网络重试、限流或本地响应缓存。`CachePolicy.BYPASS` 只转发 LiteLLM 官方请求体指令 `cache: {"no-cache": true}`。
