...
| v0.3.x 画像 / 评估输入上限放宽 | ✅ | 画像摘要扁平兴趣 tag 上限 10 → 30，且兴趣域 / 兴趣 tag 一律按 weight 降序排序后再截断（域 tag 先于 specifics 填充，保证每个高权重域至少有 tag 级曝光）；`disliked_topics` 8 → 16；batch 评估 payload 的 `description` 截断 200 → 400 字符；负例锚定上限 8 → 16（见 soul 模块） |
...
- 摘要会保留一级兴趣 `interest_domains`（前 8 个域，每域最多 5 个 specifics）和扁平兴趣 `interests`（最多 30 条），并带上 `first_seen` / `last_seen` / `source`，让搜索词生成和内容评估能区分长期稳定兴趣、近期新增兴趣和推断来源。两个列表都按 weight 降序排序后再截断，强兴趣不会被列表顺序挤掉；扁平 tag 先填所有域 tag、再按域权重填 specifics，保证每个高权重域至少有 tag 级曝光。
- 摘要会带入 `disliked_topics[:16]`；这些是长期避雷项，和 batch evaluator 的短期 `negative_examples` 互补。
...
| 策略 | 消费的画像字段 | 用途 |
|------|-------------|------|
| **SearchStrategy** | 通过 `build_profile_summary()` 消费 `personality_portrait`, `core_traits[:5]`, `cognitive_style[:5]`, `values[:5]`, `motivational_drivers[:5]`, `current_phase`, `life_stage`, `interest_domains[:8]`, `interests[:30]`, `disliked_topics[:16]`, `deep_needs[:5]`, `style`, `context`, `exploration_openness`, `source_platform_mix`, `mbti`, `recent_awareness[:5]`, `active_insights[:5]`, `_active_speculations[:5]`（兴趣域 / tag 按 weight 排序后截断；`favorite_up_users` 已从画像输出移除） | 生成搜索词、计算兴趣锚定分，并避开长期雷点 |
