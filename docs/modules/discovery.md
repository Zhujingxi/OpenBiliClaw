...
4. **混源连续评估**
   ...
   **候选 wire 减肥实验（replay-only，已否决 row landing）**：batch evaluator 现在具备隔离的 `sparse-json` 与 `row-wire-v1` 测试 seam，但生产默认仍是原有 pretty JSON。两种 treatment 共用唯一 canonical builder：每次请求分配 `0..N-1` local ID，只保留 title / author 与非空正文、简介、时长、互动、标签、目录指标、tail recall 和真实图片引用；URL、全局内容 ID、重复作者/来源字段、平台指标别名和零值不会进入 treatment wire。同平台/类型被提到 batch defaults，mixed batch 才逐项携带；只有 effective context 精确等于 `explore` 才标 `mode=explore`。`row-wire-v1` 对同一 canonical envelope 使用固定 22 列及可逆转义，decoder 会拒绝错误列宽、未知 escape、非 canonical 数字/列表和不连续 ID。输出仍为严格 JSON；多成员必须按 local ID 绑定，缺失/重复/未知成员走 bounded repair，禁止按位置误绑；多模态文字和图片锚点也使用同一 local ID。真实 100×3 中 sparse 相对 production 的 prompt/total 中位分别节省 `27.99% / 24.05%` 并通过全部门；row 相对 sparse 的 prompt 中位只省 `2.20%`，未达锁定 `5%`，style/franchise agreement 也低于 A/A floor。故不调阈值、不切 production、不改 eval-cache namespace；codec 与 local-ID parser hardening 仅保留为 replay 证据和测试覆盖。
