...
| evaluator reason-off replay | 🧪 | 新增 replay-only `reason-off` 臂：A 保持当前生产 reason diet，B 完全省略成功评估结果的 `reason` 字段，生产默认不变。回放继续使用冻结画像/候选、同 route、重复 A/A 与 A/B；artifact 按 logical run 汇总 prompt/completion/total tokens，并以隐私安全 digest 比较 `topic_group/style_key/franchise_key` 的 presence、fill-rate 与 agreement。B 仍输出 reason、分类结构不可核验或 agreement 跌破 A/A floor 都会 fail closed；`evaluation_response_missing` 故障哨兵继续保留。下游 style/franchise cap-drop 尚未在该 replay 中复刻，会显式标为未测。 |
