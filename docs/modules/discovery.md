...
| evaluator JSON minify replay | 🧪 | 新增 replay-only `json-minify` 臂：A 保持生产精确 pretty JSON，B 仅以确定性的 `separators=(",", ":")` 压缩 batch evaluator 的 profile layers、negative examples 与 content batch；字段和值、system prompt、输出契约、batch size、route 与生产默认均不变。实例级开关避免并发污染；artifact 不保存 prompt，只保存长度、隐私安全摘要、tag/语义相等证明、图片输入摘要、topic/style/franchise agreement/fill、显式供应商 cache 口径、cached/uncached usage、profile-layer cache 和 root/member-repair 归因；原始内容/图片 ID 使用进程随机盐摘要。首轮 provider run 因发现 `json_object` 空内容触发的成功适配器重试会被最终 usage 覆盖而主动中止，不作为实验数据；harness 现逐 wire attempt 归因并累加 OpenAI-protocol usage，缺任一次 usage 就阻断。除既有质量门外，landing 还要求 prompt token 至少少 `10%`、total token 至少少 `8%`，所有调用 usage/cache metric 完整且 cache/repair 不劣于 A/A 噪声。真实 100×3 通过前不启用生产 compact JSON。 |
