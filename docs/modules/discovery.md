...
> 从用户画像出发，在 B 站、小红书、抖音、YouTube、X、知乎、Reddit、Bangumi、微博和通用 Web 等来源主动寻找潜在会喜欢的内容。
...
- **sources.platforms** — 九个平台族的唯一可枚举注册表；discovery 已看过滤、pool 配额统计、已看事件身份和 URL host 推断统一复用别名 / strategy 前缀规则，`bangumi/bgm` 与 `bgm.tv/subject/*` 归入同一来源族，`weibo/wb/微博` 与 `weibo.com` / `weibo.cn` 归入微博族
- **SourceAdapter 协议** — 多源适配层（`sources/`），在上述 4 个 B 站策略之外挂载非 B 站内容源（小红书、抖音、YouTube、X、知乎、Reddit、Bangumi、微博、V2EX 等）
...
- **WeiboClient / WeiboDiscoveryProducer** — 第九个正式内容来源，由项目自有 `httpx` client 直连微博公开接口。移动 H5 search / creator 在内存里申请匿名访客 `SUB`，热搜接口只生成 query seed，再搜索真实微博；三分支统一归一为 `content_type="post"` 与 `weibo-search` / `weibo-hot` / `weibo-creator`，只写 `discovery_candidates`。它不接收用户 Cookie、不走扩展任务、不参与 guided init，也没有账号写回。search 同时接入 merged planner、inspiration axis 与 `KeywordFetchCoordinator` 的 claim/use/fail/rollback 生命周期；限流进入持久 cooldown，schema 漂移 fail closed。完整契约见 [微博来源文档](weibo.md)。
...
| 多平台发布时间元数据 | ✅ | Bilibili、小红书、抖音、YouTube、X、知乎、Reddit、Bangumi 和微博仅从语义明确的来源字段提取发布时间；`published_at` 统一规范为 UTC RFC 3339，只有相对时间时写 `published_label`。两字段贯穿 `DiscoveredContent` → `discovery_candidates` → `content_cache`，重新发现时空值分别保留已有非空值；缺失/异常值不影响候选入队。旧缓存不联网回填，也不以发现时间、任务时间、互动时间或推荐生成时间代替发布时间。 |
...
| 微博匿名公开 discovery | ✅ | 第九个正式平台族 `source_platform="weibo"`（`wb` / `微博` alias）。`WeiboClient` 用进程内匿名访客读取公开 search / creator，并把 hot 词只作为 query seed；`WeiboDiscoveryProducer` 复用统一关键词、份额门、分支预算、cadence、限流 cooldown 和候选 pipeline，只接纳真实 `post`。`reads_count` 仅在真实存在时映射 view，点赞 / 评论 / 转发分别映射 like / comment / share；无用户 Cookie、无 init / 行为事件、无扩展任务、无站内写回 |
