...
| v0.3.114 配置页服务探测 | ✅ | `POST /api/config/probe-service` 对用户当前表单草稿做无写入真实探测：LLM 走临时 `LLMRegistry.complete_provider()`，embedding 走临时 `EmbeddingService.probe()`，结果供 PCWeb / 插件设置页行内展示。它属于 degraded 恢复控制面：不依赖失败的 active registry，但真实 LLM 请求仍经过 RuntimeContext 的稳定 total gate。 |
...
chat 探针使用 `max_tokens=4096`，避免 reasoning-first 服务尚未输出最终内容就被误判。失败以 `ok=false` 的正常响应返回，前端可直接显示实例 / provider 类型 / model / latency / error。active registry 因旧配置构建失败时，该端点仍从提交草稿临时建 registry，并继续通过 RuntimeContext 的 total gate；它不会把 degraded 业务流量一并放开。详见 [配置参考](config.md)。
