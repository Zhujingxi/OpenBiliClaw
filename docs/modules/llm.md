...
| v0.3.150+ DeepSeek thinking 显式关闭 | ✅ | `OpenAIProtocolProvider` 的 DeepSeek preset 在 `reasoning_effort=""` 时写入 `thinking={"type":"disabled"}`。DeepSeek v4 默认开启 thinking，单纯省略字段并不会关闭 reasoning；descriptor 将该空值显示为 `disabled`，其余只允许官方的 `high / max`，不会把非协议值 `off` 发给上游。配置页探测和短结构化任务因此能避免 thinking 先耗尽输出预算后返回空 `content` |
...
| 阶段 9 权威模型草稿探测 | ✅ | `POST /api/model-config/probe` 对一条 revision-bound Chat draft，或一条 Embedding provider + 完整共享 settings 做无写入真实探测；只调用目标稳定 ID，不走 fallback/cache。DeepSeek Chat 即使草稿配置 `high / max`，探测也显式使用 `reasoning_effort=""`，由 adapter 发送 `thinking.type=disabled` 的 8-token 最小请求；这只改变探测请求，不修改草稿或正式 runtime。gate admission 后检查 init，取得短 model path lock 后再次检查 init，再从请求 revision 捕获 `keep` 凭据；因此等待慢保存期间启动的 init 会在 credential/network 前安全返回。网络调用不持配置锁；完成后再次校验 revision/record/settings，过期结果返回最新 snapshot 且不进入 history、不关闭 live circuit。旧 `/api/config/probe-service` 只保留 `kind="network_proxy"` |
...
- `kind="chat"`：提交一条完整 `connection`；临时构造单项 `OrderedLLMRoute` 并发送最小 chat completion。DeepSeek 探测固定关闭 thinking、保留草稿中的 `high / max` 仅用于身份与正式运行，避免连通测试被长推理拖到 deadline。
...
各端模型编辑器（桌面 Web / 移动 Web / 插件 popup / `/setup/` 向导）都基于同一条 `GET/PUT /api/model-config` + descriptor + exact probe 契约；Web 的权威 DOM-free 状态机源码是 `web/shared/model-config-state.ts`，构建后仍以 `/web/shared/model-config-state.js` 提供。该 Web `.js` 本身也是 checked-in distributable asset，保证无 Node 的 clean Python/Docker/desktop 安装可直接提供 URL；`npm run check:web-build` 负责拒绝 TS/JS 漂移。插件以独立严格源码 `popup/popup-model-config-state.ts` 为权威，测试与打包前由 `build:popup` 生成忽略的 `.js` 运行时；`tests/js/model-config-parity.test.mjs` 用共同的行为向量钉住两端契约对等（含 `__proto__` 等 prototype-like 稳定 ID 的 own-property 字段错误桶，以及等 revision 远端快照不再误判为远端更新），不再要求两个实现逐字节相同。编辑器文案中文优先（上移 / 下移 / 未命名连接等），`Chat / Embedding / Runtime / Primary / Fallback` 等技术术语保留英文。
