...
| v0.3.171 Eval reason 减肥 | ✅ | `_SINGLE_CONTENT_EVALUATION_SYSTEM_PROMPT` / `_BATCH_CONTENT_EVALUATION_SYSTEM_PROMPT` 的 reason 契约：`score < 0.5` 写空串 `""`，其余写一句不超过 30 个字的中文诊断。0.5 是静态 prompt 常量，`admission_min_score` 的支持范围同步限制为 `[0.5,1]`，explore 为 0.58，因此无 reason 条目不能入池；system message 保持 call-invariant，解析/缓存/持久化接受空 reason。Evaluator reason 不是用户文案：推荐表达和 delight copy 走各自生成链，当前不会直接展示 `relevance_reason`。旧 2026-07-18 replay PASS 已因 source-context 污染、独立快照和 16K 非生产预算作废；修正版要求同一冻结样本内重复 A/A+A/B、生产 4096 上限、缺失响应即失败和 JSON 原始证据 |
