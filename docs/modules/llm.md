...
| v0.3.171 Eval reason 减肥 | ✅ | `_SINGLE_CONTENT_EVALUATION_SYSTEM_PROMPT` / `_BATCH_CONTENT_EVALUATION_SYSTEM_PROMPT` 的 reason 契约:`score` 严格低于固定 `0.5` 的条目 reason 写空串 `""`(这些条目必低于所有 admission 门槛、会被丢弃,写理由纯浪费输出 token);`score >= 0.5` 的条目写一句口语化、可直接展示给用户的中文,不超过 30 个字(可经 delight 兜底原样露脸)。0.5 是 baked 静态常量而非运行时 `admission_min_score`,既守缓存前缀 byte-identical 铁律,又严格低于 0.60 默认 / 0.58 explore 全部准入路径;system message 保持 call-invariant,解析零改动(空 reason 早已 `.strip()` 归一) |
