...
Settings 的 Model 区从 `/v1/models/catalog` 搜索/浏览 provider 和 model，显示 catalog protocol/env metadata，支持 endpoint override 与 write-only password key；catalog filter 始终保留当前 provider，provider 清空时同步清空 dependent model。`/v1/models/current` 的 model provider/name、model 与 embedding credential presence、embedding provider/name/endpoint、`reloaded` 与 `restart_required` 作为只读 current configuration 显示，保存提示遵循服务端返回状态。高级 custom provider 表单要求 protocol、endpoint 和完整 capabilities，和后端 escape hatch 保持一致。加载、空、错误、保存中与保存失败均有可访问状态。

可访问性包括不遮挡 brand 的 skip link、desktop/mobile nav labels、desktop/mobile 明显的 `aria-current` active treatment、每 route 的 document title、unknown-route fallback notice、form labels、loading/error live announcements、route heading focus、Alt+Left keyboard navigation、明显 focus ring、high-contrast palette 以及系统/用户 reduced-motion policy。390px mobile navigation uses a three-column wrapping grid、导航 label 不在单词中间断行，form controls shrink within the viewport。Recommendations 的 secondary refresh action 先调用 supervised `/v1/recommendations/refresh` 再读取 authoritative feed；空 feed/provider/search/profile/content-detail state 给出原因与 Connect/Search CTA。Connect 从 `/v1/sources` 枚举 provider，连接前后都在可访问 live status list 显示全部 provider connection status，mutation success/error 单独可访问呈现。Assistant 在表单上方显示完整 server/local turn history，turn failure 就地显示，缺少 capability 时链接 Settings；server-hydrated structured output 与 live output 使用同一可读 projection（message text、recommendation intro 与可用 title/canonical URL），不会显示 persisted raw JSON 或 opaque recommendation ID；输出保持 escaped plain text 与换行（只移除 `**` delimiter，不使用 raw HTML renderer），404 stale conversation ID 会清除。Runtime 显示 health badge、检查时间、supervised jobs、可展开 recent events 和手动 refresh。Store/component/view tests 覆盖上述状态与 navigation/keyboard path。
