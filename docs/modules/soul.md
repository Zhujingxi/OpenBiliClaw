...
| 对话确认锚（Wave A/B，v0.3.182+） | ✅ | `DialogueAnchorManager` 持久化至多一个 `{kind,ref,generation,established_at,unrelated_streak,origin_turn_id,ambiguous_count}`，四种释放为结算、连续两轮 unrelated、2h TTL、replaced。card discuss 建锚前校验 durable payload 仍持有同一 `attempt_token`；读取校正清 token 后，停顿旧请求不能恢复建锚。待聊列表主动 open 以 `pending_open` 建锚且不受 12h/72h 时间 gate；系统疑惑提问也建锚，系统假设卡等待用户操作。Dialogue 回灌统一读取所有 session 的 completed `{chat,hypothesis,confusion}` scope（含 agent-only 疑惑 question，probe 仍排除），而 API turn 列表继续按 session 过滤；durable 请求把产生端 session 逐请求传给学习 payload。归属矩阵、ambiguous/Jaccard 防双计与 confusion FIFO 语义不变。 |
