...
较早 non-builder 完成 release/relation 后也只能刷新自己的 per-ref head，不能让
`_latest_head_key` 越过更晚的跨 ref reservation 回拨。`submit_and_wait()` 只供 worker
外 producer 排队等 completion；actual worker、活跃 job 产生的任意层 child，以及父
job 结束后仍存活的 detached child 重入 `submit()` / `submit_and_wait()` 都立即抛
`DialogueSettlementReentryError`，不会 inline 调 dispatcher。worker 内嵌套结算由
