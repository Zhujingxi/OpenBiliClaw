...
| 三端对话长列表与证据（2026-08-01） | ✅ | extension popup、mobile web、desktop web 共用 renderer 与 `reply_to_turn_id` context preview；客户端只提交 ID。卡片/问题、待聊 inbox 和 composer 使用独立有界滚动，重绘时保留读者位置、展开 evidence 与移动端草稿；机器 ID-only evidence 过滤后不显示空依据。 |
