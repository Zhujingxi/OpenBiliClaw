...
| 态势门控（`soul/posture_gate.py`，v0.3.176+） | ✅ | 深层写入一致性门控（Phase 3）。`build_posture_gate_prompt` 静态 system（三判定 accept/downgrade/reject + 「冲突不是错误是新假设」）+ `sort_keys`（入 invariance 清单）。`PostureGate` 三模式：`off`=完全旁路、**门控 LLM 零调用**、逐字节等价；`shadow`(默认)=**commit boundary 捕获不可变快照**（before/after/source_refs/gate_id），异步旁路任务只消费快照（判定前对活状态再写入不污染判定，带断言）、判定落台账 `shadow_*`、LLM 异常落 `shadow_error`、**零延迟不阻塞原写入**；`enforce`=同步判定，异常/解析失败/非白名单 verdict 保守 downgrade。三接入点：①对话 goal/value/state 深层候选（interest/dislike 快线不过；downgrade 置信=confidence×0.6 转 insight）；②管线 VALUES/CORE 层 updater 写入前（downgrade 固定置信 0.5 转 insight，写入回滚；ROLE 不过门控，校准：偏使用模式错误代价低）；③soul 整份重建（downgrade/reject→放弃本次 rebuild + 台账）。新 caller `soul.posture_gate` 注册 usage recorder。enforce 受 save-time 三条件校验（见 config.md，`posture_gate_force_enforce` 逃生门） |
