...
桌面与移动共用 TypeScript 源 `src/openbiliclaw/web/shared/saved-sync-core.ts` 作为唯一实现；`npm run build:web` 原位发射运行时 `saved-sync-core.js`，该 `.js` 作为 checked-in package data 随 clean Python wheel、Docker 与桌面安装包交付，安装路径不需要 Node；`npm run check:web-build` 在 CI 拒绝缺失或漂移。对外 URL 和 import specifier 保持 `.js`。核心同时以 ES module 导出并挂载 `globalThis.OBCSavedSyncCore`，承载提交 fence、retained 列表状态、mutation registry、严格 saved API、durable task tracker / coordinator 与焦点捕获恢复。桌面 `index.html` 用 `<script type="module">` 在 `app.js` 之前加载构建产物，后端 `_desktop_asset_version()` 继续把 `.js` 路径纳入桌面缓存版本指纹；移动 `js/saved-sync-runtime.ts` 是薄适配器，直接复用核心导出的 `DialogFocusControllerOptions`、`DurableTaskTrackerOptions`、`SavedSyncTask` 与 `FocusToken` 类型，并向核心注入 `Date.now`、定时器、`document.hidden` 与 `activeElement` 等浏览器依赖，避免迁移期的宽泛 ambient shim。核心本身保持 surface-neutral：不含 CJK 文案，DOM 与计时器全部由调用方注入；`getSavedSyncPresentation()` 只返回 `labelKey / detailKey / actionKey`，桌面在 `desktopSavedSyncPresentation` 里映射为桌面中文文案，移动端在自有视图模型里映射移动端文案。插件保留包内 `popup-saved-sync.ts`，测试和打包前由 `build:popup` 生成忽略版本控制的同名 `.js` 运行时，不依赖 `/web/shared/`，行为对等由扩展保存测试钉住。
