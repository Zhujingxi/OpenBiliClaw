...
| 扩展原生保存 durable broker | ✅（API 基础） | `ExtensionNativeSaveBroker` 把六个非 B 站平台的 canonical item / route 裁成无凭证 job，写入独立 `extension_native_save_jobs` ledger；active job 按 platform + item + requested action 原子复用。`owns(task_id, platform_slug)` 在所有持久状态下保持 exact source 归属，使跨 slug 或晚回调不会落入 legacy queue。HTTP source task multiplex 已接入；生产 adapter/broker 注册与扩展 executor 仍属后续任务。 |
