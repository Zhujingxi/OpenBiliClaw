...
| 四端保存 / 同步 UI | ✅ | 推荐卡只对本地保存做 optimistic update；状态与并发 fence 按 `list_kind:item_key` 隔离，迟到 status 水合不能覆盖新 mutation。保存页手动同步不受自动开关影响，显示真实 target、逐项状态、失败重试、批量确认和仅终态的分平台结果。Task ID 超过前台等待窗口后继续按页面可见性后台轮询，网络中断可恢复；刷新失败保留最后成功快照并显示重试。Task/result 文本先清洗再用 textContent/转义渲染；本地删除只调用 `/remove`。 |
