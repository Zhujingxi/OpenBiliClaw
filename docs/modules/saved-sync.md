...
| Local-first 保存 | ✅ | `SavedSyncService.save_local()` 先提交 membership；普通可同步来源在自动同步关闭时只落 `pending` native state，不调用 adapter。显式 local-only 来源（当前为微博）直接落 `unsupported/local_only_source`，不进入任务账本。 |
...
| 微博 local-only 边界 | ✅ | identity 层将 `weibo / wb` 与精确匹配的 `weibo.com / weibo.cn / sinaimg.cn / sinaimg.com` URL 归为 canonical `weibo:<post_id>`，lookalike host 不会误认。推荐卡可正常保存到本地收藏 / 稍后看 membership，并立即得到 terminal `unsupported/local_only_source`；微博没有 `NativeSaveAdapter`、扩展 executor、durable browser job 或站内 favorite/watch-later 写回，也不会创建 `native_save_tasks`。三个界面隐藏单项、批量同步与重试动作；删除仍只删本地 membership。 |
...
| 安全失败归一化 | ✅ | 未注册但未来可能接 adapter 的路由写为 `unsupported/unsupported_adapter_missing`，仅该组合可在 adapter 到位后重新快照；显式能力边界 `local_only_source` 与 executor 返回的 `unsupported_content_type` 保持 local-only 终态。malformed target / result 写固定 `failed/invalid_adapter_result`；adapter 异常写 `failed/adapter_exception`。 |
...
| 三个图形化保存界面 + CLI 配置可见（现统一为三端内容库子项） | ✅ | 插件 side panel、移动 Web、桌面 Web 都把「稍后再看 / 收藏 / 历史记录」收进单一内容库入口；两个保存子项复用原有列表与状态实现，不复制 API 或平台路由矩阵，旧直达链接会迁移到对应子项。推荐卡只对本地保存做 optimistic update；状态与并发 fence 按 `list_kind:item_key` 隔离，迟到 status 水合不能覆盖新 mutation。保存列表均由后端 `sync_status / sync_task_id / resolved_target / error_code` 驱动：`pending + 空 sync_task_id` 表示自动同步关闭后的本地待同步，保留手动按钮；`pending + 非空 sync_task_id` 与 `syncing` 显示可访问的同步中状态并禁用重复提交。`unsupported_content_type` 与显式 `local_only_source` 都显示「仅本地保存」且无同步按钮；历史 `unsupported_adapter_missing` 作为滚动升级兼容状态保留重试；`extension_required` 给出连接登录态插件指引，登录、限流和失败状态给出可操作重试文案，成功态原样显示后端 `resolved_target`。微博 saved 卡显示 canonical 平台身份，`wb` 与精确 host 归一为微博，lookalike host 不误认。插件 / 移动 saved 与 config 请求、桌面 saved 请求均有有界 timeout；插件的单个 deadline 覆盖初次设备会话交换、401 强制换票与受保护请求。列表成功加载后从持久化 `sync_task_id` 去重恢复任务，task→item ownership 排除重复提交；页面重新可见时恢复查询，销毁时清理 tracker。刷新失败保留最后成功快照并显示重试。保存卡封面在真实 403、网络错误或已缓存失败时移除破图 `<img>`，三端都在原尺寸位置显示 SVG 占位并保留卡片按钮的可访问名称。批量同步与重试加载先捕获列表级焦点并在重渲染后优先还原同一动作；Task/result 文本先清洗再用 textContent/转义渲染；本地删除只调用 `/remove`。桌面 Web 首屏仍可并行水合 `watch_later` / `favorite` 数量，响应 `total` 分别更新内容库的两个子项徽标（不相加）；数量为零或读取失败时保持隐藏，进入子项后的完整刷新以 per-list generation fence 阻止迟到首屏响应覆盖新徽标。CLI 仅由 `config-show` 展示自动同步开关，不提供保存 / 同步动作。 |
