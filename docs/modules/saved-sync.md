...
| 扩展原生保存 durable broker | ✅（API 基础） | `ExtensionNativeSaveBroker` 把六个非 B 站平台的 canonical item / route 裁成无凭证 job，写入独立 `extension_native_save_jobs` ledger；active job 按 platform + item + requested action 原子复用。`owns(task_id)` 提供 global native ownership，先阻断所有 legacy fallthrough；`owns(task_id, platform_slug)` 再判断 exact source，使跨 slug 或晚回调固定冲突。HTTP source task multiplex 已接入；生产 adapter/broker 注册与扩展 executor 仍属后续任务。 |
...
- Native job 优先于同源 discovery/bootstrap queue；路由先查 global ownership，任何 native UUID 都不进入任一 legacy namespace，再查 exact slug 决定提交或 409。全局未归属 ID 只有在当前 legacy queue 实际拥有该 ID 时才进入旧 handler。X 目前只有 native queue 形态。生产 adapter/broker 注册与扩展 executor 仍属后续 wiring，真实账号写入目前仍只有 Bilibili adapter。
