...
| 持久化同步任务 | ✅ | 自动 / 手动触发统一经 `create_sync_task()` 生成一个 UUID，并在单个 `BEGIN IMMEDIATE` 事务中 claim 所有 eligible 项；已有 owner 的 `pending/syncing` 行不会被重复任务窃取。`task_starter` 登记失败会立即释放 owner；runner 以 batch heartbeat 保护尚未逐项执行的 pending，正常 / 取消退出立即释放余项，崩溃后超过 5 分钟由轮询或下一次创建回收。 |
...
- `run_sync_task(task_id)`：仅原子 claim 持久化的 `pending` 行；`synced` / `already_synced` 等已完成状态不会被同一 task 重跑。进程内同 task runner 使用引用计数单飞并在最后一个调用者退出时清理，跨 service / 进程由逐项 `execution_id` claim 防重；每项 adapter heartbeat 保护 live I/O，task heartbeat 同时保护同平台串行队列中尚未 claim 的后排 pending，即使 batch 长于 stale 阈值也不会被窃取。runner 在正常 / cancellation `finally` 原子释放余下 pending；若进程直接崩溃，`get_sync_task()` 轮询与下一次 `create_sync_task()` 会按 task heartbeat 的 5 分钟 lease 回收。调用超过 240 秒或上层被取消时会请求取消 adapter；若 adapter 延迟 / 吞掉 cancellation，调用方先返回但 tracked watchdog 继续持有该项 owner 与 heartbeat，直到真实 coroutine 终止，期间不能创建重试造成重叠平台写入。若延迟调用最终返回合法 terminal 结果，watchdog 持久化该真实结果；只有取消 / 异常终止才落预备的 timeout / interrupted。
