...
| 封面视觉加成（可选，需多模态 embedding） | ✅ | `[llm.embedding].multimodal_enabled` + 支持图像的 embedding 模型开启时，`precompute_delight_scores()` 对**已达阈值**的候选额外算一个「封面↔画像兴趣锚点」跨模态余弦，映射为**有界、只加不减**的 `delight_score` 加成（`_VISUAL_COVER_BONUS_MAX=0.05`）。兴趣锚点每轮只 embed 一次；封面向量优先命中 discovery 入池按 URL 预热的缓存（冷未命中才抓一次）。默认关闭，纯文本用户 `delight_score` 与旧版逐字节一致（加成恒 0，不改变谁成为惊喜候选）。跨模态余弦 floor/ceil 为**保守未标定**初值，换用真实多模态模型后需按日志观测分布重标（见代码 `_VISUAL_COVER_*` 注释、CLAUDE.md 铁律 3） |
