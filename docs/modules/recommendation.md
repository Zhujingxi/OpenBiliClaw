...
| 跨平台公平性归一化 | ✅ | 四路 bonus 在 `serve()` 叠加成 `combined_bonus` 后、喂给 MMR 选择器前，经 `_normalize_bonus_per_platform` 按 `source_platform` 分组，**以 0 为固定点做正负两侧分段归一化**：正值按平台内 positive max 缩放到 `+cap`，负值按 negative magnitude 缩放到 `-cap`，0 / 缺失保持 0。这样 Bilibili-only 信号（P2 弹幕 / P3 关键帧）不会结构性抬高 Bilibili 候选，也不会把弱正向变成负惩罚；缺信号的平台只丢**平台内**区分度，不丢**跨平台**高度。`combined_bonus` 为空（全信号关 / 无质心）时 no-op，all-zero / single-item / 单边平台组都保持符号和零点；`snapshot_ranking.py` 镜像同一归一化，保证离线快照与 `serve()` 一致 |
