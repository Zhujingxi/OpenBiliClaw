...
- `claim_native_sync_task()` 是建立 active task owner 的唯一入口：它在单个 `BEGIN IMMEDIATE` 中选择 eligible membership，同时写入非空 `task_id` 与 `task_claimed_at`。非空但全空白的 `item_keys` 会 fail closed，真正空序列才表示整表选择。`release_native_sync_task()` 只释放登记失败 task 仍为 pending 的行。执行前 `claim_native_sync_task_runner(task_id, runner_id)` 在 `BEGIN IMMEDIATE` 中取得唯一 runner lease，并同时写 `task_runner_id/task_started_at/task_heartbeat_at`；fresh 的其它 runner 会使 claim 返回 `False`，stale lease 才允许接管。`heartbeat_native_sync_task(task_id, runner_id)` 与 `release_pending_native_sync_task(task_id, runner_id)` 都要求 runner token 匹配，因此第二个 service 既不能执行 item，也不能释放 live owner 的后排 pending。runner 正常 / 取消退出释放尚未 claim 的 pending owner；进程崩溃则由轮询的 `release_stale_pending_native_sync_task(task_id)` 或手动创建的 `release_stale_pending_native_sync_tasks(list_kind, item_keys)` 在 task heartbeat 超过 5 分钟后清理 token / owner。所有 task / runner 边界拒绝空白 ID。
...
| `task_runner_id` | 当前唯一 batch runner token；task heartbeat、item claim 和 pending release 都按它 fence。 |
