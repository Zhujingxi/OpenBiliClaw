...
| 扩展原生保存 job ledger | ✅（基础设施） | `extension_native_save_jobs` 保存脱敏后的六平台扩展任务；partial unique index 保证 `(platform, item_key, requested_action)` 只有一个 pending/in-progress row。claim、exact slug ownership、correlated completion、pending cancellation/extension-required 与 stale claim failure 均有 status fence。 |
...
- `extension_native_save_jobs` 是与 native task ledger 分离的浏览器执行 ledger。`create_or_reuse_extension_native_save_job()` 在 `BEGIN IMMEDIATE` 中原子复用 active row；`claim_extension_native_save_job()` 按平台领取最老 pending job；`owns_extension_native_save_job(job_id, platform_slug)` 只承认 exact source；`complete_extension_native_save_job()` 只接受匹配 platform slug + job UUID + item key 的 in-progress row；`mark_unclaimed_extension_native_save_job_extension_required()` 与 `cancel_unclaimed_extension_native_save_job()` 只更新 pending；`expire_stale_extension_native_save_jobs()` 把不确定的 claimed write 固定完成为 `failed/extension_task_timeout`，绝不重放。所有读取返回新的 `dict` copy。
