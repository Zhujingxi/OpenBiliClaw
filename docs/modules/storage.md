...
入队、队头出队与清空都使用独立短连接的 `BEGIN IMMEDIATE`，不会发生 read-modify-write 丢更新。`pop_confusion_replay_head()` 的 `expected_id` 是 FIFO fencing token：T2 不能越过 T1。orphan claim 释放同时校验 claim identity 和 `NOT EXISTS(chat_turns.turn_id)`，不会把已经补建成功或被 retarget 的提问回滚。完成回复的恢复查询只读 `scope='confusion'`，并用 `julianday` 比较 ask receipt；畸形 legacy payload 按未处理对待，不会让 `json_extract` 使扫描失败。
