...
- `claim_native_sync_task()` 是建立 active task owner 的唯一入口：它在单个 `BEGIN IMMEDIATE` 中选择 eligible membership，同时写入非空 `task_id` 与 `task_claimed_at`。非空但全空白的 `item_keys` 会 fail closed，真正空序列才表示整表选择。`release_native_sync_task()` 只释放登记失败 task 仍为 pending 的行；`mark_native_sync_task_started()` 同时写 `task_started_at/task_heartbeat_at`，runner 通过 `heartbeat_native_sync_task()` 持续刷新整批剩余 `pending/syncing` 行，防止长串行 batch 的后排 pending 被误回收。runner 正常 / 取消退出由 `release_pending_native_sync_task()` 原子释放尚未 claim 的 pending owner；进程崩溃则由轮询的 `release_stale_pending_native_sync_task(task_id)` 或手动创建的 `release_stale_pending_native_sync_tasks(list_kind, item_keys)` 在 task heartbeat 超过 5 分钟后释放。所有按 task 查询 / 修改的 DAO 都拒绝空白 `task_id`，不会把 `task_id=''` 的未领取 pending 行误当成一个批次。
