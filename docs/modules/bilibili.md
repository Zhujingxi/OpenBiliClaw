...
| 弹幕 fetch outcome | ✅ | `get_danmaku_texts_result()` 区分 `success`（可为空）、`no_data` 与 `transient_failure`；HTTP、XML、限速和网络异常保留原因并交给上层重试。`get_danmaku_texts()` 继续提供只返回文本列表的兼容包装 |
...
`transient_failure`。持久化调用方必须使用该结果保留成功空结果与可重试失败的区别；旧
