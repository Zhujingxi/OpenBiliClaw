...
| 持久化 | `init_runs` 表（`storage/database.py`）：`run_id / status / stage / stages_json / partial_success / error_reason / error_detail / sequence / started_at / updated_at / finished_at`。`error_detail`（v0.3.156+，存量库自动 ALTER 迁移）存失败细节：未知异常的 `类名: 首行消息`（截断 300 字）或 `GuidedInitError` 的人类可读 message，`fail(run_id, reason, detail=...)` 落库；v0.3.168+ `complete(partial_success=True, reason=..., detail=...)` 也保留 discovery 降级原因并随 `init_completed` 事件下发。CLI 在独立进程里遇到 `GuidedInitError` 时调用 `record_external_init_failure()` 写入同一终态（不伪造跨进程实时进度）；若 GUI run 正在 `starting/running` 则拒绝覆盖。重新预约同 run_id 时清空。 |
...
桌面/插件/移动端 Models 设置页，或 `openbiliclaw models ...`。桌面宽度下
左侧是一列 Chat connection type，右侧为 descriptor-driven inspector；
窄屏改为顺序式「类型列表 → 详情」界面，选中类型后才显示 inspector 与保存
按钮，并提供可聚焦回当前选项的「返回连接类型」操作，不同时堆叠两个面板。

向导先读取 `GET /api/model-connection-types?capability=chat` 和脱敏的
`GET /api/model-config`。向导内所有 API 调用都经同一个有界 `apiFetch`
助手发出：同源 `credentials`、每请求携带 `X-OBC-Auth: 1`、AbortController
超时、结构化 `{status, details}` 错误，401 统一进入登录引导；整页不再调用
`/api/config`。选择 connection type 后，只渲染 descriptor
...
`clear` 动作。切换 preset 或 connection type 时，已由用户编辑且在新
descriptor 中仍适用的 model / Base URL 等字段与现有稳定连接名称保持不变；
preset 默认值只填充未触碰或空字段，不再覆盖用户输入，新类型不支持的字段会
清空。空 Chat snapshot 会创建全局不与 Embedding ID 冲突的
...
写入 `/api/config`，B 站登录态也不再经该接口提交——向导依赖浏览器插件的
Cookie 同步，前置检查统一改读 `GET /api/init-status`。模型保存由
`ModelConfigService` 协调 runtime swap 与 init 写门控。

模型步骤完成后，向导进入 B 站与来源选择，展示同款前置清单：B 站登录门禁
读取 `GET /api/init-status` 的 `prerequisites.bilibili_logged_in`，且只在
勾选来源包含 B 站时拦截，未勾选 B 站不阻塞其它所选平台；随后使用
`POST /api/init` 启动和 `runtime-stream` / 轮询恢复进度。向导 load 时会
读 `/api/init-status`：`running` 直接恢复进度，`initialized` 按首池状态
进入完成页或「整理首轮内容池」；最近存在终态失败时直接回到初始化恢复步骤，
Docker 且模型已就绪时也跳过首个模型表单，展示可复制的宿主机命令而不调用
`POST /api/init`。安装包入口（`packaging/entry.py`）
...
恢复状态的四端契约：Soul/profile 的 `initialized` 是唯一 readiness 权威；raw/pending pool、推荐计数、最近发现数和 refresh 状态都不能把 `initialized=false` 提升为已就绪。桌面 `/web`、移动 `/m` 和 popup 因此继续显示恢复入口并隐藏换一批/加载更多等无效动作；CLI 仍输出原始失败面板，并把终态写给共享状态。`start_mode=cli_only` 时三个 GUI 只展示/复制 Docker 命令，`local_only` 则禁用远程启动。桌面画像页区分“后端离线”和“已连接但画像未初始化”，分析/画像/embedding 失败的设置入口落到 Models；内部 runtime event type 通过固定映射转成人类文案，不直接泄露事件码。
...
- `tests/test_database.py` — `init_runs` CAS / 白名单列 / reconcile，以及 CLI 外部失败终态写入与活跃 GUI run 防覆盖。
...
- `tests/test_web_guided_init_e2e.py` — Playwright 驱动真实 `/setup/` 与 `/web` 页面，stub 外部 HTTP 响应来覆盖浏览器交互：成功进度、前置失败、启动冲突、终态重试、runtime-stream 静默 watchdog，PC setup / Web 等首批 `pool_available_count>0` 后才完成，以及 `initialized=false` 时已有推荐/raw/pending 候选信号仍保留恢复入口；v0.3.168+ 直接断言超时原因、Base URL / 模型设置恢复动作、重试 / 设置入口、`role=alert`、Docker 复制命令不 POST，以及 discovery 超时的部分完成 / 后台补池提示。CI 的 `web-guided-init-e2e` job 安装 `[browser]` extra + Chromium 后单独运行。
