...
`compact_evaluation_profile_summary` (`discovery/engine.py:67`, caps: 20 core / 64 interests /
32 domains × 12 specifics / 12 recent-awareness / 8 evidence / 12 speculations; dislikes
...
domains / top-64 tags by weight; the tail beyond that is exactly where niche-content matches
...
- Degrades cleanly: no embedding service → field omitted (compact block alone, still ≥64 tags).
...
actually draws on (20 core traits, values, style, 64 interests), and the single-expression
