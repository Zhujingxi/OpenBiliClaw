...
`compact_evaluation_profile_summary` (`discovery/engine.py:67`, final caps: 20 core / 80 interests /
...
domains / top-80 tags by weight; the tail beyond that is exactly where niche-content matches
...
- Degrades cleanly: no embedding service → field omitted (compact block alone, still up to 80 tags).
...
actually draws on (20 core traits, values, style, 80 interests), and the single-expression
