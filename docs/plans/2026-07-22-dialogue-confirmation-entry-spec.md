...
**Created:** 2026-07-22(r4,codex 三轮 F22-F27 修订;与用户五轮讨论定稿)
...
4. **action 契约:结算表原子仲裁 + 两段提交(r4/F22/F23,替换 r3 的先查后写)**:
   ...
   - **两段提交**:①赢得仲裁(applied=0)→ ②执行副作用——`update_from_feedback` 的多段写(event→对象→rebuild 标记)以 settlement 为 receipt:event 写入 metadata 携带 `settlement_ref`,重试时已存在该 ref 的 event 则跳过 event 段,对象段/标记段本身幂等 → ③置 `applied=1` + CAS `payload.state`。崩溃于任意段:重试路径查到「settlement 属于本 turn_id 且 applied=0」→ 从②续做(各段幂等/去重),**故障注入测试逐段覆盖**(r4/F23)。defer 不进结算表(非终态,仅冷却记录)。
   - **卡片 turn 状态语义(F13)**:创建即 `status="completed"`、`payload.type="card"`,durable worker 不触发 LLM 生成(guard 测试)。
5. **锚快照/代次/卡片关联(r3/F17)**:anchor 携带单调 `generation` 与 **`origin_turn_id`**(建锚来源卡片,疑惑提问 turn 同理)、`ambiguous_count`(非 ambiguous 到达即清零,r2/F21);学习 payload 捕获 `{anchor_ref, anchor_generation}`,处理时 generation 失配 → 丢弃+WARNING。**卡片 payload 状态机完整化(r4/F25)**:`pending → {confirmed|rejected|deferred|discussing}`;`discussing → {confirmed|rejected|deferred}`(锚结算经 origin_turn_id patch)**或 → pending**(锚以 unrelated/TTL/replaced 释放时回 pending,卡片重新可操作,不卡死);discuss 重试幂等(已 discussing 返回现态);全路径释放测试。
...
7. **confusion 结算单一所有者 + 崩溃可恢复(r3/F14)**:durable side-effect 的直接 `resolve/defer` 移除,结算统一归锚处理器(学习队列内串行);分类器输出并入。**队列是内存的、失败即丢——补持久化恢复(r4/F24 逐轮 receipt)**:confusions 行增 `processed_turn_ids TEXT`(JSON 数组,上限 50 逐出最旧,与结算/处理写同事务)——**逐 turn 记录而非单水位**(单水位在 T1 失败 T2 成功时会永久漏 T1);12h 扫描取「clarifying 的全部 completed 回复 turn(晚于锚建立)」与 processed 集合比对,**缺口重放**(幂等);缺口测试:T1 处理失败被队列吞、T2 成功 → 扫描补 T1。假设卡片按钮与锚处理器的并发由对象级仲裁(不变量 4①)统一。
