...
**Created:** 2026-07-22(r5,codex 四轮 F28-F31 修订;与用户五轮讨论定稿)
...
4. **action 契约:结算表原子仲裁 + 两段提交(r4/F22/F23,替换 r3 的先查后写)**:
   - **新表 `card_settlements`**(`ref` 主键、`verdict`、`turn_id`、`applied INT`、`created_at`):仲裁 = **`INSERT OR IGNORE`**——插入成功者赢得该 ref 的结算权(DB 级原子,**无 TOCTOU**);插入冲突 → 读现行 verdict 返回 `already_settled` 并 patch 本卡片投影。跨连接并发测试(两连接同时 confirm/reject → 恰一赢)。
   - **两段提交(r5/F28 可接管化)**:settlement 行增 `apply_claim_at` 与逐段完成标志(`seg_event/seg_object/seg_marker INT`)。①赢得仲裁(applied=0)→ ②**原子 claim**:`UPDATE ... SET apply_claim_at=now WHERE ref=? AND applied=0 AND (apply_claim_at IS NULL OR apply_claim_at < now-5min)`(单语句;续做资格=claim 持有者,**原请求崩溃 5 分钟后任何重试请求可接管**,不再限定原 turn_id;同 turn 并发重试也由 claim 串行化,event 段无 TOCTOU)→ ③按段标志跳段执行副作用(每段完成置段标志)→ ④`applied=1` + CAS `payload.state`。**投影只认 applied=1**(applied=0 期间卡片显示"处理中",不谎报终态);故障注入逐段 + 接管测试。defer 不进结算表。
   - **卡片 turn 状态语义(F13)**:创建即 `status="completed"`、`payload.type="card"`,durable worker 不触发 LLM 生成(guard 测试)。
5. **锚快照/代次/卡片关联(r3/F17)**:anchor 携带单调 `generation` 与 **`origin_turn_id`**(建锚来源卡片,疑惑提问 turn 同理)、`ambiguous_count`(非 ambiguous 到达即清零,r2/F21);学习 payload 捕获 `{anchor_ref, anchor_generation}`,处理时 generation 失配 → 丢弃+WARNING。**卡片 payload 状态机完整化(r4/F25)**:`pending → {confirmed|rejected|deferred|discussing}`;`discussing → {confirmed|rejected|deferred}`(锚结算经 origin_turn_id patch)**或 → pending**(锚以 unrelated/TTL/replaced 释放时回 pending,卡片重新可操作,不卡死);discuss 顺序与补偿(r5/F30):①CAS pending→discussing ②建锚;②失败 → 回滚 CAS 至 pending(单写者);崩溃遗留「discussing 且无活锚」→ turn 列表读取时校正回 pending(兜底测试);discuss 重试幂等;全路径释放测试。
...
7. **confusion 结算单一所有者 + 崩溃可恢复(r3/F14)**:durable side-effect 的直接 `resolve/defer` 移除,结算统一归锚处理器(学习队列内串行);分类器输出并入。**失败不吞、有序重放(r5/F29,替换 r4 的集合缺口方案)**:学习队列对锚定轮处理失败时**不静默丢弃**——confusions 行记 `pending_replay_turn_id`(单值游标;处理串行、同锚同时至多一轮在飞,单值即够);**后续同锚轮次到达时先重放 pending_replay 再处理新轮**(顺序保证,杜绝 T2 先结算改锚导致 T1 永久丢/倒序改计数);12h 扫描仅兜底触发同一重放路径(幂等);锚以 replaced/TTL 释放时 pending_replay 随之清空+台账 dropped 行。测试:T1 失败→T2 到达→T1 先补放且顺序正确;锚释放清空;扫描兜底。假设卡片按钮与锚处理器的并发由对象级仲裁(不变量 4①)统一。
