...
**Created:** 2026-07-22(r7,codex 六轮 F37-F38 修订;与用户五轮讨论定稿)
...
3. **卡片 turn 进现有流 + 「一个大脑多个屏幕」(r3/F19 修正)**:对话认知流是**单一**的(单 `_history`、单认知语境——产品定调);`session` 仅是 **UI 归属标签**(决定哪个端显示哪些 turn)。**回灌:单一 history 回灌全部 `{chat, hypothesis, confusion}` scope 的 completed turn,不分 session**(认知连续;r2 的"各回灌各的"作废——那会造成双端大脑分裂);前端 turn 列表按各自 session 过滤显示。卡片 turn 落库 `session` 取产生端(用户 open → 请求端;系统抛出 → 附着的用户消息所在端)。dialogue 单例的固定 `_session` 字段保留为默认标签,durable 路径逐请求传 session。
   **抛出附着的确定性(r3/F18)**:拦截点=durable 用户消息接收处理内——同一处理流程中先 INSERT 卡片 turn 再处理用户消息;顺序稳定性:turn 列表排序改为 `(created_at, rowid)`(rowid 单调,替换随机 turn_id tiebreak,兼容既有数据);去重范围 **(ref, session)**(r5/F31):同 ref 同 session 已有 pending 卡片则不重复产;跨端 open 各自产各端卡片(投影由结算表统一);重启/重试测试覆盖。
4. **action 契约:结算表原子仲裁 + 两段提交(r4/F22/F23,替换 r3 的先查后写)**:
   - **新表 `card_settlements`**(`ref` 主键、`verdict`、`turn_id`、`applied INT`、`apply_claim_at`、`apply_claim_token`、`seg_* INT`、`created_at`):仲裁 = `INSERT OR IGNORE`(原子无 TOCTOU)。**冲突分流(r6/F32)**:插入冲突后读行——`applied=1` → 返回 `already_settled` + patch 本卡片终态;`applied=0` → **进入接管路径**(见两段提交),绝不在 applied=0 时 patch 或返回终态(此时对外呈现「处理中」)。跨连接并发测试(恰一赢 + 冲突方正确分流)。
   - **两段提交(r5/F28 可接管化)**:settlement 行增 `apply_claim_at` 与逐段完成标志(`seg_event/seg_object/seg_marker INT`)。①赢得仲裁(applied=0)→ ②**原子 claim + fencing(r6/F33)**:`UPDATE ... SET apply_claim_at=now, apply_claim_token=<新uuid> WHERE ref=? AND applied=0 AND (apply_claim_at IS NULL OR apply_claim_at < now-5min)`(单语句;5 分钟超时任何请求可接管);**此后每次段标志/applied 写入都带 `WHERE apply_claim_token=<自己的token>`——被接管的旧执行者段写失败即退出(fencing,杜绝双执行者)**→ ③按段标志跳段执行副作用——**event 段原子化(r7/F37)**:同一 SQLite 事务内「`UPDATE card_settlements SET seg_event=1 WHERE ref=? AND apply_claim_token=? AND seg_event=0`(原子占位,受 token+seg 双条件保护)+ `INSERT` event 行」——占位影响行数=0(被接管/已写)则跳过,旧执行者停顿后恢复因 token 失配必然=0,**查重-写入间隙不存在**;同库同事务保证占位与写同生共死。对象段/标记段天然幂等(跨存储段靠幂等,不依赖事务)→ ④`applied=1`(带 token)+ CAS `payload.state`。投影只认 applied=1;故障注入逐段 + 接管 + **旧执行者停顿-接管-恢复交错测试**。defer 不进结算表。
   - **卡片 turn 状态语义(F13)**:创建即 `status="completed"`、`payload.type="card"`,durable worker 不触发 LLM 生成(guard 测试)。
5. **锚快照/代次/卡片关联(r3/F17)**:anchor 携带单调 `generation` 与 **`origin_turn_id`**(建锚来源卡片,疑惑提问 turn 同理)、`ambiguous_count`(非 ambiguous 到达即清零,r2/F21);学习 payload 捕获 `{anchor_ref, anchor_generation}`,处理时 generation 失配 → 丢弃+WARNING。**卡片 payload 状态机完整化(r4/F25)**:`pending → {confirmed|rejected|deferred|discussing}`;`discussing → {confirmed|rejected|deferred}`(锚结算经 origin_turn_id patch)**或 → pending**(锚以 unrelated/TTL/replaced 释放时回 pending,卡片重新可操作,不卡死);discuss 顺序与补偿(r7/F38 加 attempt fencing):①CAS pending→discussing(payload 记 `discussing_at` + 本次 `attempt_token`)②同请求内建锚——**建锚入口校验卡片 payload 当前 attempt_token 与 state=discussing,失配即拒**;②失败 → 回滚 CAS;读取校正(discussing >5min 且无活锚)回 pending 时**清空 attempt_token**——停顿超 5min 的旧 discuss 请求恢复后建锚因 token 已清被拒(测试:「校正后旧请求恢复建锚被拒」);discuss 重试幂等;全路径释放+宽限+fencing 测试。
...
7. **confusion 结算单一所有者 + 崩溃可恢复(r3/F14)**:durable side-effect 的直接 `resolve/defer` 移除,结算统一归锚处理器(学习队列内串行);分类器输出并入。**失败不吞、持久有序重放(r6/F34)**:confusions 行记 `replay_queue TEXT`(JSON 有序小队列,上限 5,超限最旧 dropped+台账)。锚定轮处理失败 → 本轮入队;**存在非空 replay_queue 时,新到锚定轮先持久入队尾,处理循环严格从队头依序执行,失败停在原位**(T1 补放再失败,T2 仍持久保序等待,任何触发都从头续);12h 扫描兜底触发同一循环(幂等);**锚释放(replaced/TTL/结算/unrelated 解锚)时清空队列+台账 dropped**。测试:T1 失败→T2 到达持久入队→T1 再失败→T2 不越序;T1 成功后 T2 续;四种锚释放清空;扫描兜底;上限逐出。假设卡片按钮与锚处理器的并发由对象级仲裁(不变量 4①)统一。
