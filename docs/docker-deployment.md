...
Compose 启动 `api`、`worker`、LiteLLM 与独立 PostgreSQL。启动前必须在当前目录的 `.env` 生成四个本机 secret：
...
不要提交 `.env`。installer 会在跨进程锁内补齐上述四项、保留已有非空值，并以 `0600` 临时文件、`fsync` 和原子替换更新；`.env` 或锁文件为符号链接时拒绝继续。Provider key 只在 LiteLLM Admin 配置；`OPENBILICLAW_SECRET_KEY` 加密来源账户，`OPENBILICLAW_ACCESS_TOKEN` 保护业务 API。

权威 readiness 是 `GET /api/v1/system/readiness`。业务 API 只存在于 `/api/v1`；除首次 onboarding 例外外都需要 bearer token。当前静态 Web/扩展尚未重接，Task 22 前请用 OpenAPI/API client 验证后端。
...
```bash
docker buildx build --platform linux/amd64,linux/arm64 -t openbiliclaw-api:v0.3.4 .
```
...
>
> ```bash
> docker exec -it openbiliclaw-api openbiliclaw init
> ```
>
...
预构建路径必须同时保留 `docker-compose.prebuilt.yml` 与 `litellm/config.yaml`；后者与源码 Compose 使用完全相同的 retry/cache/logging policy。先在 `http://127.0.0.1:4000/ui` 配好三个 vNext 稳定别名；然后打开 `http://127.0.0.1:8420/setup/` 完成当前 legacy AI 配置与前置检查，再运行 `docker exec -it openbiliclaw-api openbiliclaw init` 完成初始化。想固定版本，把 compose 文件里的 `latest` 换成具体版本号（如 `0.3.152`）。
...
同样打开 `http://127.0.0.1:8420/setup/` 完成 AI 配置与前置检查，再运行 `docker exec -it openbiliclaw-api openbiliclaw init` 完成初始化。更新：`git pull && docker compose up -d --build`（Dockerfile 已做依赖分层，依赖没变时重建只需数秒）。
...
curl http://127.0.0.1:8420/api/v1/system/readiness
...
# 高级排查：直接编辑容器内配置
docker exec -it openbiliclaw-api vi /app/runtime/config.toml
```
...
```bash
docker exec -it openbiliclaw-api openbiliclaw models list
docker exec -it openbiliclaw-api openbiliclaw models add --kind chat
docker exec -it openbiliclaw-api openbiliclaw models add --kind embedding
docker exec -it openbiliclaw-api openbiliclaw models move <STABLE_ID> --position <1-10>
docker exec -it openbiliclaw-api openbiliclaw models probe <STABLE_ID>
```
...
```bash
# B 站认证登录
docker exec -it openbiliclaw-api openbiliclaw auth login

# 可选：启用本地 Ollama 作为独立 embedding provider
docker exec -it openbiliclaw-api openbiliclaw setup-embedding

# 手动触发内容发现
docker exec -it openbiliclaw-api openbiliclaw discover

# 查看推荐
docker exec -it openbiliclaw-api openbiliclaw recommend

# 查看用户画像
docker exec -it openbiliclaw-api openbiliclaw profile
```
...
# 查看容器日志
docker compose logs -f openbiliclaw-api
```
...
- 健康检查地址：`http://127.0.0.1:8420/api/v1/system/readiness`
...
# 查看应用日志文件
docker exec -it openbiliclaw-api cat /app/runtime/logs/openbiliclaw.log
```
...
```bash
# 备份数据库
docker cp openbiliclaw-api:/app/runtime/data ./backup-data

# 备份配置
docker cp openbiliclaw-api:/app/runtime/config.toml ./config-backup.toml
```
...
```bash
docker exec -it openbiliclaw-api openbiliclaw models add \
...
**Q: 启动时报 `container name "/openbiliclaw-api" is already in use`？**
...
```bash
docker rm -f openbiliclaw-api openbiliclaw-ollama
```
...
如果数据库出现问题，可以在容器内运行 `docker exec openbiliclaw-api openbiliclaw db-repair` 进行检查和修复。
...
```bash
curl -s http://127.0.0.1:8420/api/v1/system/readiness | python -m json.tool
