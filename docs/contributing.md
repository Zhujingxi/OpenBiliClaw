...
- **事件来源归属持久化** — [@Patrick5D](https://github.com/Patrick5D) 在 [#179](https://github.com/whiteguo233/OpenBiliClaw/pull/179) 中贡献 `events.source_platform` / `content_id` / `source_confidence` 顶层字段、统一来源解析优先级（显式来源 → metadata → 规范 URL → legacy fallback）与 schema v6 增量迁移，为按平台撤回数据重建画像奠定数据基础；主干在其实现上完成未知平台 slug 降级与 confidence 防升级加固，已合入主线。
- **实时 Star History 图表恢复** — [@OctoBored](https://github.com/OctoBored) 在 [#196](https://github.com/whiteguo233/OpenBiliClaw/pull/196) 中恢复 README 中/英文的实时 Star History 图表，替换已失效的静态徽章与临时提示；主干在合入时补齐 URL `&amp;` 转义，已合入主线。
