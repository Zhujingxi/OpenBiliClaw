...
7. Run configured Assistant/model work only through typed AI Runtime routes and budgets. Assistant exposes one canonical typed lifecycle to SSE and non-streaming adapters: turn/reasoning/tool/response completion or safe error. Native streamed runs preserve cancellation/resource bounds, expose no tool arguments/results, and never retry after visible output; disconnect cancellation needs no stop registry. Assistant context estimates the complete request, reserves about 20% for output/tools, retains newest complete turns that fit, reports approximate usage/exclusions, and never automatically summarizes or removes the persisted transcript. Only validated completion atomically persists the visible turn and sanitized tool summaries; reasoning is never persisted.
