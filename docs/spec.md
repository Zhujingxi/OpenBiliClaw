...
20. Hosts authenticate with password login for local clients and generated tokens for extension/plugin and agent access. Provider images are served through a local image proxy so provider CDNs never see user requests.
