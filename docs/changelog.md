...
- **安装与 Compose 切换到 vNext 双进程契约**：源码 installer 删除 legacy provider editor 与已移除 feature command 流程，改为要求用户提供 LiteLLM proxy、以 `0600` 原子持久化并幂等复用 access/encryption secrets，按依赖→migration→API+worker→queue readiness→`doctor`→public/protected check 顺序执行且失败非零退出；本地 PID/log 分离管理两个进程。源码与预构建 Compose 的 API/worker 现在显式使用完全相同的 app DB、Huey SQLite 路径和 data volume。README、agent/Docker 安装、FAQ 与 manual E2E 删除旧命令，并明确 static Web/extension 要到 Task 22 才完成 vNext wiring。
