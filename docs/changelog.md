...
- **Phase 1 试点 2：提取 `/api/health` 与 `/api/init-status` 到 `api/routes/health.py`**：`/api/health`（readiness 探针含 embedding、profile、LAN IP 与 degraded payload）与 `/api/init-status`（guided-init 状态与前置检查清单）从 `api/app.py` 内联装饰器迁移到 `api/routes/health.py` 的 `build_health_router(deps)` 工厂；新增 `HealthRouteDeps` 冻结 dataclass（embedding 探针、degraded 状态、init-coordinator 等闭包），路由注册顺序与外部行为字节级一致（路由契约与精确响应测试锁定）。本条系同层内部代码搬移，对外 HTTP 表面（路径、方法、响应体、content-type、状态码）完全不变，因此架构图按例外决定不强制更新。`docs/modules/api.md` 同步更新。
