...
- **LLM token diet 收口到评估 / 表达热路径**：`discovery.evaluate_batch` 坏输出不再整批退化成 N 次单条调用，而是在同一 worker 内二分拆批、递归子批 `<=5` 才逐条兜底，provider 限流 / cooldown / quota 仍原样传播不拆分；LLM caller bucket 补齐 `discovery.keyword*`、`discovery.x*`、`discovery.douyin*`、`runtime.bilibili_extension_search*`、`pool_purge*`、`api.sentiment*`，整条高频链路都能用 `[llm.discovery/evaluation/recommendation/soul]` 分层模型配置覆盖。内容评估画像改用 compact summary（20 核心 / 64 兴趣 / 32 域 × 12 specifics / 12 recent，长期避雷不裁剪），每条候选另带 top-256 兴趣池召回出的最多 3 个 `related_interests` 长尾兴趣，digest 覆盖 compact 画像和召回池；`scripts/run_profile_diet_ab.py` 新增真实 DB A/B replay gate，按同日 A/A 噪声包络做相对质量门，本轮 compact + recall 的 divergence 指标均落在实测 A/A 噪声范围内。推荐表达 / legacy 分类共用同一个 `compact_content_prompt_profile_summary()` 收口，`interests=` 内容相关替换保留；discovery eval `body_text` 进入 prompt 前截断为 1600+400，批量池文案截断为 1000+200。
