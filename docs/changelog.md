...
- **TypeScript 大重构稳定化**：补回移动 Web 迁移后缺失的 15 份 checked-in `.js` 运行时资产，并让 `npm run check:web-build` 在 clean checkout 可验证全部 18 份 TS/JS 对；修正 `saved-sync-runtime.ts` 与共享 core 的真实 task/focus 类型边界。插件恢复仅检查 `src` + popup 两个 strict runtime project，删除被后续 merge 误带回的 `tsconfig.tests.json`；`npm test` 与 root `test:js` 会先构建 popup 运行时，构建器改为原位覆盖以消除 Node 并行测试期间“先全删再逐个生成”的模块缺失竞态。extension source-text 契约与 Python clean-checkout 契约均改读 `.ts`，运行时行为测试继续读生成的 `.js`，因此 Python suite 不再隐式依赖 ignored popup 构建产物；popup 与 Web 模型状态机改以行为向量守卫，不再断言 Phase E 已删除的逐字节同步副本。四个大型移动视图暂以显式 `@ts-nocheck` 稳定边界恢复 strict gate，其余移动壳层、saved-sync adapter、extension popup/runtime 均保持真实 strict typecheck；每个边界都带 `TODO(types)`，后续按视图模型逐个收紧。本条不提升版本号；后端 API、配置与持久化格式均无变更。
...
- **TS 迁移 Phase B：`web/shared/` 模型配置三件套迁移到 TypeScript**：`src/openbiliclaw/web/shared/model-config-render.js`、`model-config-state.js`、`saved-sync-core.js` 迁移为 `.ts`（strict:true，opaque 后端 payload 用 `unknown` + 局部窄接口并标注 `// TODO(types):`；无显式 `any`）。运行时行为不变：`scripts/build-web.mjs` 使用 esbuild `transform` 做类型剥离（保留内联 `export` 声明与 CJK 字面量——前端契约守卫按源码文本断言 `export function <name>` 与中文文案），在仓库内原位发射同名 `.js` 供 StaticFiles 继续以 `/web/shared/*.js` 提供。三份 `.js` 现在是 checked-in distributable runtime assets，而 source map 仍仅为忽略的开发工件；因此 clean checkout 的 Python wheel/editable install、Docker build 与桌面 PyInstaller build 无需 Node/npm 也必然带齐运行时文件。web CI 在只读 runtime drift 与 JS 行为测试之前先执行 strict `npm run typecheck:web`，避免 esbuild 仅剥离类型时漏过类型错误；`tests/test_web_runtime_assets.py` 锁定该门禁及其执行顺序。extension CI 同样先严格检查 `src` 与 popup 两个运行时项目，再执行完整 Node 测试；移除会把 89 个既有测试文件及其局部浏览器 mock、legacy JS imports 一次性纳入 strict 编译却从未保持 green 的 `tsconfig.tests.json` scaffold，测试源码类型化改为后续按迁移切片渐进收紧，而不是放松生产源码或 popup 的 strict 门禁。`tests/test_typescript_ci.py` 锁定两个 strict 项目、CI 顺序及运行时测试边界。`npm run check:web-build` 以只读方式逐字节重建比较，CI 遇到缺失/漂移立即失败；同一测试模块另从 git index 构造 clean packaging tree、构建真实 wheel 并检查三份 `.js` 成员，同时锁定 Docker/PyInstaller 继续消费 tracked web tree。三份 `.ts` 的类型剥离输出与迁移前 `.js` 经同一 esbuild transform 后的输出逐字节一致，导出表也与 `tests/snapshots/*.exports.json` 基线逐一核对一致（31/8/16）。插件同步链路随源迁移：`extension/scripts/sync-model-config-state.mjs` 现从构建后的 `model-config-state.js` 生成 checked-in `extension/popup/popup-model-config-state.js`；`tests/js/model-config-parity.test.mjs` 漂移守卫继续断言 popup 副本等于 banner + Web 运行时构建产物，并保留全部 14 个 web ↔ extension 行为 parity 向量。本条不提升版本号；后端 API、URL 面（`/web/shared/*.js`）、桌面缓存版本指纹与持久化格式均无变更。
...
- **共享状态机补齐设置/向导需要的辅助能力**：`web/shared/model-config-state.js` 新增 `circuitView` / `circuitAnnouncementKey`（熔断状态 + 倒计时展示）、`hasUnverifiedChanges` / `unverifiedConnections`（自上次 exact probe 后 draft fingerprint 变化检测，保存前提示「存在未验证修改」）、`overrideLockFor` / `hasOverrideLocks`（`config.local.toml` 覆盖锁展示）、`createSingleConnectionDraft`（向导编辑单条 chat/embedding 记录的 lens，preservedFallbackCount 用于披露保留的 fallback 数量）、`prepareLocalOllamaEmbedding`（popup「一键启用本地 Ollama」动作的受守卫入口），并在 `applyProbeResult` 上写入 `probe.fingerprint` 作为后续变更检测的基线。`toModelConfigPayload` 现在对 `enabled=false` 的 Embedding route 序列化空 providers——后端对 disabled-with-providers 一律以 `embedding_disabled_with_providers` 拒绝；路由项仍保留在客户端状态里，同会话重新启用时恢复。
- **设置向导 `/setup/index.html` 对齐共享模块并补齐缺失步骤**：改用 `<script type="module">` 从共享 state 模块构建单连接 draft、从共享 render 模块导入 `escapeHtml`，替代此前的内联转义实现；当 snapshot 报 `migration.state != "none"` 时整页切换为迁移 interstitial、禁用保存并指引到完整设置页的迁移面板（决策 3），向导不再尝试内联解决 legacy 迁移；新增可选 Embedding 配置节，沿用同一 descriptor 渲染与 `keep/set/env/clear` credential 语义（凭据值输入处理器合并保留已选 action，输入 API Key / 环境变量名不再被重置为 keep）；保存前披露 `preservedFallbackCount` 条已存在的 fallback 连接会被保留；继续沿用 probe-before-save 与 revision-gated `PUT /api/model-config`。
...
- **保存链路统一切到平台中立 `/api/saved/{kind}` 家族**：插件 `popup-api.js` 删除 8 个 B 站专用 legacy 导出（`addToWatchLater` / `removeFromWatchLater` / `watchLaterStatus` / `fetchWatchLater` / `addToFavorite` / `removeFromFavorite` / `favoriteStatus` / `fetchFavorites`）；移动 `web/js/api.js` 删除 8 个对应 legacy saved helper 与 2 个孤儿 `/cognition-updates` helper（认知更新渲染由 `/api/profile-summary` 的 `has_more_cognition_updates` 分页承担）；桌面 `app.js` 删除 ENDPOINTS 表里的 `/watch-later` / `/favorites` 死条目与 `sourceCredentials` 拉取。legacy `/api/watch-later`、`/api/favorites` 路由仍保留在服务端供旧客户端使用，保存、列表、状态、同步与任务轮询在所有前端只走 `/api/saved/{watch_later|favorite}` + `/remove` + `/status` + `/sync` + `/api/saved-sync/tasks/{id}`。
- **桌面与移动保存逻辑收口到共享 `web/shared/saved-sync-core.js`**：新共享核心以 ES module 导出并同时挂载 `globalThis.OBCSavedSyncCore`，承载提交 fence、retained 列表状态、mutation registry、严格 saved API、durable task tracker / coordinator、焦点捕获恢复与 dialog focus controller。桌面删除旧 `assets/js/saved-sync-core.js`，`index.html` 用 `<script type="module">` 在 `app.js` 之前加载共享核心，后端 `_desktop_asset_version()` 把它纳入桌面缓存版本指纹；移动 `js/saved-sync-runtime.js` 改为薄适配器，向核心注入 `Date.now` / 定时器 / `document.hidden` / `activeElement` 等浏览器依赖。核心保持 surface-neutral：不含 CJK 文案，DOM 与计时器全部由调用方注入；presentation 只返回 `labelKey / detailKey / actionKey`，桌面在 `desktopSavedSyncPresentation` 里映射为桌面中文文案，移动端在自有视图模型里映射移动端文案。插件继续保留包内 `popup-saved-sync.js` 运行时，守卫断言插件产物不依赖 `/web/shared/`。
- **`/setup/` 向导收敛为单一有界请求助手并改用 init-status 门禁**：向导内全部 API 调用走同一个 `apiFetch`（同源 `credentials`、每请求 `X-OBC-Auth: 1`、AbortController 超时、结构化 `{status, details}` 错误、401 统一进入登录引导），整页不再调用 `/api/config`。B 站登录检查改为读 `GET /api/init-status` 的 `prerequisites.bilibili_logged_in`，且只在勾选来源包含 B 站时拦截，未勾选 B 站不再阻塞其它所选平台；模型保存顺序仍是先 exact probe 当前 draft 再携带 revision 调 `PUT /api/model-config`。停滞文案只说「可以继续等待，也可以先离开页面，进度会保留」，不再暗示可从向导取消初始化（向导没有接 `/api/init/cancel`）。
- **插件 popup 移除 GitHub star 实时数启动拉取与抖音 debug 中继**：popup 启动不再 fetch `api.github.com` 取 star 数（该域不在 `host_permissions`，请求本就不会成功），头部 Star 按钮与仓库链接保留。抖音 TEMP DEBUG 转发链整体移除：`src/background/debug-log.ts` 删除，`content/douyin.ts` 与 `background/dy-task-dispatcher.ts` 中全部 `debugLog` 调用点同步清除，不再向 `/api/sources/_debug/log` 发送任何内容；该后端路由保留为服务端专用。
- **插件模型配置状态机与 Web 共享实现行为对齐**：`extension/popup/popup-model-config-state.js` 的 `mapServerFieldErrors` 改用 own-property 检查写入字段错误桶，`__proto__` 等 prototype-like 稳定 ID 不再污染对象原型；`receiveRemoteSnapshot` 在远端 revision 与本地相同时直接返回克隆，等 revision 快照不再被当作远端更新（移除插件独有的 `{force: true}` 语义）。`tests/js/model-config-parity.test.mjs` 用 13 个行为向量（11 个共享 parity 向量 + prototype-like ID 与等 revision 两个独立向量）同时钉住 Web 共享与插件两份状态机。
- **三端模型编辑器文案改为中文优先**：桌面、移动与插件模型编辑器统一使用「上移 / 下移 / 未命名连接」，桌面与插件另有「拖拽排序 / 未设置模型」；`Chat / Embedding / Runtime / Primary / Fallback` 等技术术语保留英文，守卫测试逐端断言两套用词。
- **`openbiliclaw models` 恢复直接导入 click**：`cli_models.py` 与 `tests/test_cli_models.py` 改为直接 `import click`，不再依赖 typer 已移除的 `typer._click` 私有 shim（锁定环境升到 typer 0.24.1 后该 shim 缺失，导致 `openbiliclaw models` 导入即失败）；`main()` override 按 `TyperGroup` 签名重新标注，命令面与行为不变。
- **新增前端契约守卫**：`tests/test_frontend_contract_alignment.py` 用静态守卫加参数化真实挂载检查钉住：无前端请求 `reveal_keys=true`、无前端消费 legacy saved / cognition 路径、插件无 debug 中继且不依赖 `/web/shared/`、桌面共享核心先于 `app.js` 加载且纳入缓存版本、共享核心不含 DOM 依赖与 CJK 文案、setup 单一 CSRF 助手且零 `/api/config`、三端中文优先文案与受保护术语，并实际请求 `/web/shared/saved-sync-core.js` 与 `/m/shared/saved-sync-core.js` 验证两个挂载都可达；`extension/tests/packaging-layout.test.ts` 检查 Chrome 与 Firefox zip 布局并断言产物中无 debug relay。模块、配置、init、saved-sync、移动 Web 与 LLM 文档同步到当前前端契约；本条不提升版本号或修改 README 发布预告。
- **triage regression sweep：mypy 类型修复、release 脚本 Python 3.11+ 选择、`/api/config` 日志路径脱敏、删除 dead `ClaudeProvider`**：`sync-aggregate-release.sh` 在 `tomllib` 不可用时改探测 `python3.11+`，避免 macOS 自带 `python3`（3.9）拉发布包失败；`GET /api/config` 的 `logging.directory`、`logging.filename` 与 `logging.file_path` 改为不可逆脱敏输出——相对 `directory` 原样返回，绝对路径（如 `/srv/private/openbiliclaw/logs`）、`~` 前缀家目录相对路径、Windows 盘符绝对路径（`C:\Users\secret\logs`）、Windows 无盘符根路径（`\Users\alice\private\logs`）与 UNC 路径统一降为 basename，绝对 `filename` 同样降为 basename，根/卷根路径（`/`、`C:\`、`~`、`\`）与裸 `~user` 形式（如 `~alice`）脱敏为空且 `file_path` 回退为仅文件名，盘符限定形式（drive-only `C:` / `D:` 与 drive-relative `C:foo` / `C:private\logs`）先剥离盘符再按同一规则脱敏（drive-only 归为空、drive-relative 归为其 basename），不再泄露主机文件系统布局或盘符；`PUT /api/config` 改用无歧义协议——未修改时客户端回传 `file_path` 精确匹配脱敏 wire 形式即识别为 echo 并跳过，有意修改（包括改到与显示 basename 完全相同的相对路径）时省略 `file_path`、直接提交 `directory`/`filename` 即生效；桌面与插件设置页用**显式 dirty 意图标记**驱动该分支——字段真实 `input` 事件置位、配置渲染进表单与保存成功后复位，最终值相等不再被误判为未修改 echo（此前值相等启发式无法区分未修改 echo 与有意 basename 编辑；新增 `tests/test_api_app.py` POSIX root、Windows drive root、Windows rooted-no-drive、bare `~`、`~user`、Windows drive-only、Windows drive-relative 及 intentional-basename 碰撞回归，与桌面 `tests/test_desktop_web_logging_path_intent.py`、插件 `extension/tests/popup-settings.test.ts` 两侧客户端意图追踪回归）；桌面与插件设置页同步按「未修改发 `file_path`、已修改发 `directory`/`filename`」拆分保存负载；`openbiliclaw.llm` 删除未被任何生产代码使用的独立 `ClaudeProvider`（209 行）与其在 `__all__` 的导出，Anthropic 官方 / 自定义 Messages 网关统一由 `AnthropicCompatibleProvider` 承载，`tests/test_llm_providers.py` 同步收敛到现存 adapter；`mypy src/` 在 cli_models.py 类型漂移修复后回到 0 错误。本次为 maintenance 收口，不提升版本号或修改 README 发布预告。
- **triage regression sweep 复审修复：盘符限定 `logging.filename` 脱敏补齐 + release 脚本 Python 选择面向未来**：复审在 `882e62fe` 上发现 `/api/config` 仍把不含 `/` 或 `\\` 的 Windows 盘符限定 `logging.filename` 原样回显——`filename="C:backend.log"` 在 Windows 上会被 `PureWindowsPath("logs") / "C:backend.log"` 折叠为盘符限定的 `C:backend.log`，悄悄丢弃配置的目录，`filename="D:"` 同样带盘符穿透 wire 形式；本次把 `_safe_logging_filename_for_wire()` 改为与目录侧规则对齐——drive-relative（`C:backend.log` / `c:backend.log` / `D:private\backend.log`，大小写盘符均覆盖）先剥离盘符再按既有分隔符规则降为 basename，drive-only（`C:` / `c:` / `D:` / `d:` 等）归为空字符串，`file_path` 此时仅含脱敏目录与尾部 `/`，永不携带盘符；新增 `tests/test_api_app.py` 大写/小写 drive-only 与 drive-relative GET `/api/config` 回归。`sync-aggregate-release.sh` 的 `pick_python` 此前硬编码 `python3 python3.13 python3.12 python3.11`，在系统 `python3` 为旧版本、唯一合适版本化二进制是 `python3.14` 的环境下退出 1（尽管 `python3.14` 满足 `pyproject.toml` 的 `>=3.11` 要求）；本次改为通用版本化发现——优先使用已满足 3.11+ 的 `python3`，否则扫描 PATH 中所有严格匹配 `python3.N` 的可执行文件（跳过 `python3.*-config` 等非解释器），按版本号数值降序挑选最高合适版本；选择对 PATH 顺序确定性，并保留旧 `python3` 与无合适版本化二进制时的文档化错误信息；新增 `tests/test_aggregate_release_workflow.py` 可执行工作流回归，用隔离 PATH 下旧 `python3` + `python3.10` + `python3.12` + `python3.14` + 杂散 `python3.13-config` 验证选择 `python3.14`，现代 `python3` 优先，以及无合适解释器时的退出 1 与错误文案。`docs/modules/config.md` 同步补全盘符限定 `filename` 的 wire 合同。
...
- **初始化失败后的四端恢复不再被候选池假信号遮住**：画像是否已生成现在是桌面 Web、移动 Web、插件 popup 和 CLI 的统一权威状态；即使阶段 1 已留下 raw/pending/推荐计数，只要 `initialized=false` 就继续展示初始化恢复。CLI 的 `GuidedInitError` 终态会写入共享 `init_runs`，`/api/init-status` 用独立的 `start_mode` 与 `last_failure_reason/detail` 同时表达“当前只能在 Docker 宿主机启动”和“上次真实失败原因”，不再让 `unsupported_runtime` 覆盖 6 分钟模型超时。Docker 的 `/web`、`/setup/` 与 popup 直接展示并复制 `docker exec -it openbiliclaw-backend openbiliclaw init`，不会先发一个注定 409 的 POST；setup 重开会回到恢复步骤，桌面画像页、移动首页和模型设置入口也提供明确自救路径，内部 runtime event type 不再原样泄露到用户文案。
- **Typer 0.24 兼容性恢复**：模型 CLI 不再从 Typer 私有模块导入 Click，改为直接使用项目已声明的公开 `click` 包；避免新版 Typer 移除 `_click` 导出后，模型命令以及导入 CLI 的 API / 测试在启动阶段失败。
