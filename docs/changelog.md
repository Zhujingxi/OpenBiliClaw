...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排基础（issue #56）**：确认收藏与稍后再看的统一路由契约——所有动作先写 OpenBiliClaw 本地；收藏同步到各平台收藏 / 书签 / Saved / 专用播放列表，稍后再看优先平台原生能力、缺失时降级到收藏；自动同步总开关默认关闭，本地收藏页与稍后看页保留显式手动同步和逐项结果。首阶段已落地 canonical `source_platform:content_id` 身份及 `saved_items` / `saved_memberships` / `native_save_states` 本地表、带 marker 的旧 `watch_later` / `favorites` 单次事务迁移、normalized DAO 与旧 Bilibili wrapper 兼容读写；新增平台无关 `NativeSaveAdapter` protocol / capability router 与 `SavedSyncService`，自动 / 手动任务共用 UUID 和持久化状态，批量任务通过原子 claim 避免重复创建窃取 active row，并以原子唯一 `task_runner_id` 防止两个 service 并发跑同一 batch；task heartbeat、item claim、余项释放均按 runner token fence，heartbeat 抛错 / owner 丢失会被与平台 work 同步监控并立即取消 work，当前 item 按既有取消语义落库，第二 runner 无权释放 live owner 的后排 pending。generic state upsert 禁止建立 active owner，所有状态写边界验证显式 state machine、completion 只接收 terminal；runner 正常 / 取消退出释放余项，崩溃后轮询或手动创建按 5 分钟 task lease 清理 runner token 并回收。逐项执行另用 execution owner token 与 heartbeat；240 秒只作为调用方 response deadline，tracked watchdog 保持长调用 fence；延迟或同步吞取消的合法 terminal 结果仍持久化真实结果，malformed target / result 安全归一。空白 task/runner ID、adapter 非终态返回与全空白 item filter 均 fail closed，eligibility view 排除已有 owner 的 pending。本地状态 atomic insert-if-absent，单项失败不回滚 membership。生产平台 adapter、HTTP API 与三端同步 UI 仍按后续任务推进。
