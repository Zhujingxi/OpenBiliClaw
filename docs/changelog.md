...
- **浏览器插件模型设置升级为权威有序路由编辑器（阶段 11）**：popup 的 Models tab 改用自包含的 DOM-free state 与 controller，Chat primary/fallback 统一为最多 10 项的稳定-ID 有序列表，Embedding Provider 使用同一列表语义并共享唯一 model/维度/阈值/多模态设置；连接类型、preset、字段与 credential action 由 descriptor 驱动，窄窗按 sequential list→detail 操作。snapshot、descriptor、revisioned save、exact probe、迁移 resolution、dirty remote 与 conflict 均走独立模型 API，通用 `/api/config` 保存不再携带模型字段；初始 snapshot 被新 reload 取代时 descriptor 仍可独立安装。一键本地 Ollama Embedding 只创建或复用空的/单一 Ollama route，保留 Chat 与 credential 语义，遇到非 Ollama/多 Provider route、脏草稿、override 或 revision 冲突时明确拒绝覆盖。
