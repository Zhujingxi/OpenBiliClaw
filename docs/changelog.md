...
- **对话结算单队列独立验收修复 F1**：owner resolve 只终结自身 per-ref reservation，不再把全局 latest head 指回较早受理的跨 ref builder；确定性固定 `B1(A)→B2(B)→resolve B1→snapshot` 仍读取 `B2 reserved`。
- **对话结算单队列独立验收修复 F2**：`submit_and_wait()` 识别活跃 worker handler 及其 child task lineage，nested command 走真实 dispatcher inline 执行而不排到父 job 身后；普通继承 child 与父 handler 结束后的 detached child 仍无 mutation permit。
- **对话结算单队列独立验收修复 F3**：confirmed/rejected 卡片收到迟到 defer 时返回 `already_settled` 与真实终态，不再伪报 deferred，也不创建或延长该 ref 的确认 cooldown。
- **对话结算单队列独立验收修复 F4**：启动、pending 列表与主动 open 会扫描 `clarifying` 但无对应 durable turn 的 claim→create crash gap，经 worker 调用带 ask-turn/turn-existence fencing 的存储 CAS 恢复为 open、清除未发生的 ask cooldown 并释放全局唯一 slot。
- **对话结算单队列独立验收修复 F5**：删除“单一 settlement + fake dispatcher”的 100 路自证，改由安装在真实 `create_app` runtime 的 dispatcher 跑完 11 个 typed kind，并在同一总括用例固定跨 ref head 与 worker-child inline 两个 blocker 交错；十类 protected category 逐类调用各自生产 mutator/handler，核对 SQLite、锚、卡片、疑惑与 cooldown 均零旁路写。
