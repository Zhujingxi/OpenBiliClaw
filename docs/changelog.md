...
- **首启、安装、Docker 与桌面打包统一写入原生模型路由（阶段 14）**：`/setup/` 首页改为消费 connection-type descriptor 与脱敏 model snapshot，按“类型 → preset/OAuth → 字段”创建或编辑第一条稳定-ID Chat 记录，先 exact probe 再 revisioned `PUT /api/model-config`；fallback、Embedding route 与共享 settings 原样保留，409 会载入最新快照，credential 只提交 `keep/set/env/clear`。`config.example.toml`、agent bootstrap、Bash/PowerShell 安装器、Docker sidecar seed 与 frozen desktop helper 全部改用 schema-version-1 `[models]`；bootstrap 新增 canonical `--connection-type/--preset`、保留精确映射的 deprecated `--provider`、删除 module override，并逐 stable ID 检查 primary Chat 和每个 Embedding Provider。Docker 只在首份配置中 upsert `ollama-docker`，已有配置不覆盖；桌面包只在 route 可安全播种时用 typed renderer 更新本地 Ollama Embedding，远端顺序、凭据 provenance 与无关配置保持不变。
