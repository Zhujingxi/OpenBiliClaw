...
- **保留 Web journey 终验**：桌面与移动端 library 保存以 durable add 为权威，interaction signal 失败时仍显示已保存并只重试 signal；replacement Playwright suite 覆盖登录、desktop drawer、partial save、popup saved/onboarding retry，并由 CI 直接运行。三端已移除 dropped-feature stylesheet selectors，避免被删除的 delight、awareness、saved-sync 与 model-route UI 继续随包发布。
