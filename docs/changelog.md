...
- **设置页「当前 Cookie / 登录凭据」只读查看区不再伪装成输入框**：用户群实际反馈把该区当成 Cookie 粘贴框（readonly textarea 粘不进去，被当成 bug）。只读框改为虚线边框 + 置灰底色 + 默认光标，平台名旁新增「只读」标签，每行新增「复制」按钮（无凭据时禁用，成功弹 toast）；区块说明改为明确"只能复制、不能编辑或粘贴"，并指出 YouTube / 知乎走公开源或浏览器登录态接入、无需填 Cookie。真正的粘贴入口仍是下方 B站 / 抖音 / X 覆盖输入框，行为不变。已用 Playwright 对静态页验证 7 行结构、复制链路与只读样式。
