...
- **来源 task cancellation / fallback / 数据边界加固**：operation metadata 新增 optional fallback transport，B 站 search 明确为 direct primary，并只在 retained cooldown / DOM-fallback signal 下排入 browser queue；普通 TypeError/validation error 不触发 fallback。browser transport 的单一 timeout 覆盖 enqueue + poll，timeout 或 asyncio cancellation（含 enqueue thread 在途）会补偿为 durable `cancelled` terminal row，后续 claim 跳过且 completion 明确失败。task request/result 复用 frozen metadata finite-JSON 验证并以 token classifier 拒绝 `cookie_jar` / `authorization_header` 等 qualified secret container，保留窄 analytics allowlist。bootstrap/activity limit 端到端透传并在 normalization 后硬裁 N；知乎互动时间优先 retained `interaction_time`。真实 deferred registry provider、全部 browser builder、retained client/CLI 全操作 mapping 均由 SQLite 合同测试覆盖。HTTP/扩展仍未切换。
