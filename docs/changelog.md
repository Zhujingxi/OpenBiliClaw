...
- **修复小红书封面全灭：改由扩展在页面内采集封面字节**：2026-07 起 `sns-webpic-qc.xhscdn.com` 上线指纹级防盗链——新鲜 token 的封面 URL 用 curl / httpx 抓取一律 403（补齐 Chrome UA / Referer / Origin / Sec-Fetch 全套头无效），仅真实浏览器页面内 `fetch` 返回 200（CDN 给 CORS 可读字节）。服务端预取与 `/api/image-proxy` 实时抓取因此对小红书全部失效：7 月中旬后发现的笔记一张封面都进不了缓存,用户反馈「小红书内容都没头图」；我们自己实例靠 5-6 月存量缓存在撑所以未察觉。修复：扩展在提取笔记卡片时于页面上下文抓取封面（此刻 token 最新鲜），转 base64 挂 `cover_data`/`cover_content_type` 随既有 observed-urls / 搜索任务结果通道上报（每批 ≤12 张、单张 ≤1MB、4s 超时、best-effort），后端 `save_extension_cover` 校验（白名单 / `image/*` / 大小 / base64）后写入既有 `data/image-cache/`——缓存 key 本就剥离轮换 token，serve 端零改动全走缓存命中；已知候选去重跳过的笔记也先存封面，用户重新刷到旧笔记可就地治愈无封面存量。同期补齐链路可观测性（本次定位只能靠「慢取日志里 xhscdn 完全缺席」反推，`image_cache` 模块此前零日志）：`fetch_cover_bytes` 失败按 host 限频 WARNING 且 detail 携带真实上游状态码，image-proxy 失败补 DEBUG 关联行，xhs ingest 每批 INFO 汇报缓存封面数。
