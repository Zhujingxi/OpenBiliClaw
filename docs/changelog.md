...
- **PR #6 backend-first 架构恢复与 CI 修复**：重新以 `/api/v1` + feature/application + infrastructure adapter + LiteLLM/PydanticAI + fresh SQLite/Huey 的 v0.4 tree 为权威实现。修复 Python 3.11 下 zero-retry stream 被 `wait_for(anext(...))` 切换 consumer task 后跨 task 退出 AnyIO cancel scope：provider lifecycle 现由单一 producer task 持有，public generator 只经单槽 queue 转发 validated snapshot，关闭/取消会 cancel + join producer 并完成 durable `ai_runs` 终态。backup late-sync 测试只在 macOS 执行专属的第五次 payload-recycle `fsync` fault，Linux 仍覆盖其实际 final-file 与 directory durability 点；CLI 架构 policy 直接检查 Click command registry，不再受 Rich ANSI help rendering 影响。Firefox 开发工具升级到当前 `web-ext` minor，并覆盖其未及时提升的四个 vulnerable transitive parser/archive 依赖，使 `npm audit` 回到零告警且不把工具链带入运行包。
- **成功 continuation 与并发设置终审收口**：`job_runs.continuation_completed_at` 持久确认全部注册 callback 已完成，worker startup 与 live lifecycle sweep 都只重放未确认的成功 continuation，瞬时 callback/queue/DB 失败无需重启且确认后停止分发。retention cleanup 只删除已确认的 succeeded row，过期未确认成功会保留到 replay/ack 完成；failed/cancelled 清理不变，单条条件 DELETE 与 acknowledgement 依 SQLite write order 关闭竞态。`SettingsService` 保持完整 strict `UserSettings` 校验，但持久化改为 conflict-safe 默认 row seed 与 scoped top-level key upsert；onboarding completion 只单调更新自己的 key，跨 session 的 disjoint API/worker 更新不再整份替换或互相丢失。architecture/spec 同步首次 onboarding periodic gate，平台条件 policy 不再豁免 worker source composition。
- **首次 onboarding 前抑制产品维护任务**：worker 的 periodic eligibility policy 现在实时读取数据库中的 `onboarding_complete`；首次设置未完成时，`source_sync`、`profile_projection` 与 `feed_replenishment` 的 Huey tick 不创建 `job_runs`、不入队，避免后台失败污染用户可见的 onboarding progress。`cleanup` 仍可运行，显式 API/onboarding `schedule()` 不受影响；完成 onboarding 后各任务继续使用原 typed interval、时间桶、scheduled priority 与 dispatch 语义。
- **Production AI 与动态调度闭环**：feed replenishment 现在以 `keyword_generation` 规划需要输入的 source query，以 `obc-embedding` 的 `feed-v1` namespace 计算有界语义多样性，并只为 deterministic admission 结果通过 `obc-analysis` 调用 `recommendation_explanation`；逐条解释调用前后检查 job cancellation，取消不会提交部分 feed。删除无生产 caller 的单候选 `candidate_assessment` task/settings/UI/eval，dataset 改为真实 batch task。五个 generative task 的 alias 由 lane 固定，只有 `chat_response` 使用 `obc-interactive`。四个 durable job interval 全部持久化并在 Web/extension settings 暴露；per-minute Huey tick 动态读取 DB setting，以 job-specific 时间桶去重并保持 scheduled-maintenance priority。
- **vNext state-change and persisted-URL hardening**：cookie-authenticated source-task claim 虽保留 long-poll GET 合同，现也要求 same-origin CSRF proof；finite extension bearer claim 不变。Activity、content 与 source-task result URL 在持久化前 fail closed，并删除 userinfo、credential/signature query 字段及 credential-bearing fragment 字段。
- **安装与首次认证终审加固**：Docker credential stage、Compose start、verifier commit、一次性 disclosure 与显式 rotate 统一进入既有 lifecycle lock，并发安装不再可能运行 B 却持久化/披露 A。fresh configured API 的 onboarding 从第一步即要求 installer bearer 或 Web login；`web_password_enabled` 退出 browser PATCH/UI，避免当前 cookie browser 关闭唯一登录路径，丢失凭据统一通过成功的 `--rotate-access` 恢复。Docker 显式 `--litellm-admin-url` 会校验并持久化替换，未显式传值的 rerun 保留已有 custom URL。Windows `.env` 在写 secret 前应用当前 SID 独占、关闭继承的 DACL，发布与每次读取都复核 owner/rule，无法执行或验证即失败关闭。
- **Activity 重试幂等与 macOS backup payload 隔离**：`activity_events.id` 改为单条 SQLite conflict-safe insert；同一 deterministic UUID 的并发 extension/outbox 重试只保留一行并都成功返回，不再由 check-then-insert 在 commit 暴露 `IntegrityError` / HTTP 500。同 ID 不同 payload 的 losing retry 会在事务内重新读取首次持久化 event，响应与 profile signal 只使用该权威行，不产生未入库的 phantom evidence。macOS backup 删除可复用的固定 named payload slot；每次使用不可预测名称 `O_EXCL` 创建新 inode，核对 held/path identity 后在 snapshot 写入前 unlink，并在写入前后及发布前要求 held regular FD identity 不变且 link count 为零。预先打开 `.backup-00.tmp` 的攻击者 FD 因此永远不会成为 snapshot 写入目标；最终 `fclonefileat` no-replace、exact-byte/integrity 验证和 directory durability 保持不变。
- **多 scope bootstrap 公平性与来源质量门禁**：Bilibili、Douyin、YouTube 与 X 现在把一次公开 bootstrap limit 按稳定 scope 顺序均匀分配，避免首个满 scope 在最终裁剪时饿死后续 retained signal；总数仍受公开 limit 约束。新增 production direct-client mocked matrix，覆盖所有 advertised direct capability 及 empty/malformed/auth/timeout/rate-limit 分类，并在 CI 通过 `scripts/check-source-contracts.sh` 对七平台 vNext connector core 强制 80% branch coverage。

---

## v0.4.0：vNext 后端领域与持久化基础（2026-07-17）

### Current vNext authority

当前权威运行面是 `/api/v1`、fresh vNext SQLite、独立 Huey worker、generic
`/api/v1/source-tasks`、共享 TaskRunner chat、LiteLLM 和运维 CLI。Docker/source
installer 已拥有 migration 与 access/encryption secret 生命周期。现有 static
Web/extension 已通过 generated client、fetch-SSE 与 generic dispatcher 接入；下方较早的交付条目按
发生时状态保留，不得用来判断当前 API、CLI、配置或运行时权威。

- **首次登录默认可用**：fresh vNext 设置默认启用 Web 密码认证，与 installer 一次性生成并交付的密码保持一致；Docker/source 安装后的 setup UI 无需人工改库即可登录。该 enablement 现为 browser read-only，避免自锁。
- **扩展活动队列与小红书溢出加固**：被动活动在入 durable FIFO 前按后端合同限制 external ID/title/URL；后端对同一 payload 的 `400`、`413`、`422` 永久拒绝会写入本地 dead-letter 并继续后续 FIFO，认证、限流、网络及服务端失败仍原样重试。小红书每轮只 reserve 实际交给 service worker 的最多 20 条 URL，拒绝时释放该批，未提交溢出保留到下一轮而不再被误标为已上报。
- **首次安装访问与 manifest 设置闭环**：Docker/source installer 先只在 memory/runtime environment 暂存 Web scrypt hash、extension digest 与对应明文；migration、API/worker/Compose health 全部成功后才以私密 atomic replace 提交 verifier 并通过 `first_run_access` 交付一次明文，失败不提交也不暴露，丢失最终 event 可显式 rotate 恢复。prebuilt two-file bootstrap 同样使用 no-follow owner/mode 校验、flock、digest-only stage、Compose `--wait` 与最终 commit。CLI 对 `.env` 使用 no-follow descriptor、pathname identity、owner 与 `0600` 校验，shell/PowerShell wrapper 不再自行读取。Compose Admin URL 通过 `${LITELLM_PORT:-4000}` 插值；source 只在显式 `--litellm-admin-url` / env 输入通过 credential-free HTTP(S) 校验后持久化 Admin URL，绝不从 model API base URL 推导。custom `HOST`/`PORT` 一致驱动 command、mapping、health、probe 与完成 URL。`/setup` 与桌面账号区按 manifest credential schema 渲染；桌面和 mobile 首份画像都发送 `expected_revision=null`，direct/system 网络模式都清除旧 proxy URL。

- **Worker lease 与真实 chat streaming**：`job_runs` 增加 worker owner、per-claim fencing token、lease、durable retry deadline 与累计 attempt budget；claim 直接返回 token，checkpoint、feature transaction guard、heartbeat 和 terminal transition 必须匹配同一次 claim。API startup 不重置合法 running row，也不提前发布 delayed retry；worker lifecycle 在 deadline 后补发 crash-window 遗留的 undispatched retry，最后 attempt 的 lease expiry 原子终结为 failed。connector/AI timeout、connection、HTTP 408/409/425/429/5xx 才进入 Huey retry，cause chain 中的 permanent boundary 直接终结；periodic job 使用 scheduled-maintenance priority，显式触发默认 user-triggered。Chat 每轮加载最近 30 条 persisted typed history；zero-retry chat task 经同一 `TaskRunner.stream()` 实时输出 schema-valid delta 并最终验证，retry-capable streamed task 则隐藏 rejected attempt。router/service/responder 同 context 逐层关闭 child stream，客户端断开会主动 cancel/drain provider；recorder start cancellation 会等 transaction 完成后写一次含已知 usage 的 cancelled terminal 状态，完成后 assistant turn 保存对应 `ai_run_id`。
- **Feed/profile/source 合同终审加固**：Activity→ProfileSignal 对仍合法的长事件值做确定性 500 字符投影；每次画像 delta revision 获得新的 aware timestamp。Feed admission 在最终事务核对 latest profile revision，默认最多重算一次并在持续竞争时显式冲突，同时把既有 unseen entry 的 source/topics 计入 diversity cap。Bilibili public search/trending/related 改用不读取账户的匿名 client，bootstrap 仍要求 Cookie；Xiaohongshu、YouTube bootstrap、Zhihu 与 Reddit 不再发布或保存无 backend consumer 的凭据。browser completion 逐 item 通过对应 source normalizer 和 source identity 校验，nonsense/cross-source row 原子终结为 `result_mismatch`；URL-shaped result string 在 validation 与 `source_tasks.result_payload` 写入前统一剥离 `xsec_token` 和 credential query。Activity/Content URL 在领域入口执行同一防御。
- **Library 幂等与 backup 无 pathname staging**：`favorites` / `watch_later` 的重复保存改用 SQLite conflict-safe insert，并返回首次持久化的同一 membership；连续 HTTP POST 不再把唯一约束 `IntegrityError` 暴露成 500，首条 note/added_at 保持权威。SQLite backup 删除 private hard-link/staging cleanup 模型：main 与现存 sidecar 先以 no-follow FD 固定，SQLite connection 新增 FD 在 backup 前后必须只属于 pinned identities 且包含 main，并同步复核原 pathname。source snapshot 不再创建或 unlink named staging，消除 compare-before-unlink TOCTOU 与 32 槽上限；40 次连续备份、snapshot rollback 和 source-path adversarial swap 覆盖 replacement 保留及 owned inode 可恢复，同时不改变 final no-replace 与 directory-sync durability 边界。
- **Task 23 vNext 单一权威收口**：公开说明、安装与部署文档、模块索引、隐私说明和 Chrome Web Store 文案统一到 backend-first 架构；新增 Docker 首次运行、Web、extension 与 source install 四份可执行人工 E2E runbook。历史 provider route、Soul/awareness/probe、OpenClaw、native account save、desktop/self-update 和 legacy feature CLI 的 live-looking 规格、计划与模块文档退出当前文档集，只保留本次重构 master plan 与本日志中的历史记录。七个平台的 retained direct/CLI/browser transport 统一作为 `infrastructure.sources` 私有实现缝隙，feature use case 只依赖 connector contract；Web 与 extension 只消费 `/api/v1`、SSE 和 generic source-task 协议。
- **Reddit transport 收口为可运行的 extension 合同**：删除无法安全消费 vNext 加密账户 Cookie、且在 Docker 中依赖独立 home credential file 的 `rdt` backend、CLI adapter 与默认依赖。Reddit 的 bootstrap、search、trending、community 和 related 五个 retained operation 统一通过 durable browser task 执行；per-source settings 不再公开无消费者的 `backend`，manifest 不再发布 backend credential schema，generic configure use case 会在写库前拒绝 Reddit Cookie，避免产生永远不会使用的密文账户。
- **Feed 反馈语义与评分域对齐**：`impression`、`open` 与本地收藏/稍后看 telemetry 继续原子持久化；前两者维持 seen/watermark，全部 interaction 继续参与适用的去重，但不再伪造高置信正向画像证据。只有 `positive`、`negative`、`dismiss` 生成 evidence，interaction 响应以 nullable `signal` 如实表达。SQL read-side 在叠加 interaction adjustment 前按 `CandidateAssessment.score` 同样的 0..1 域夹紧 assessment weighted score，并继续以 position/entry ID 做确定性分页，不改写历史 feed/assessment 行。
- **Docker 产品 E2E 与反馈排序闭环**：新增 credential-free Compose override、schema-derived fake OpenAI provider 和单命令 product driver；真实启动 API、Huey worker、应用 SQLite、LiteLLM 与 PostgreSQL，只通过 generic Zhihu source-task 完成 onboarding/bootstrap/feed，并覆盖 profile、当前 feedback rerank、负反馈 avoidance 画像、语义相近的后续新候选降权、chat、embedding 与两类本地 collection add/list/remove。Compose 删除全局 `container_name`，由 project name 隔离并行/CI 环境，脚本每轮生成秘密、选择 loopback 端口且 trap 清理 volume/container。feedback evidence 不再使用 opaque content UUID，而从持久化 `ContentItem` 的 title/summary/topic 构造；当前 feed read 以 immutable assessment 加 interaction adjustment 稳定排序，历史 entry/assessment 不被改写。
- **LiteLLM 无 Redis 启动修复**：针对锁定的 LiteLLM v1.92 将未显式指定类型的 `cache: true` 解析为 Redis 并在无 Redis 的 Compose 中退出，内置单 proxy 部署现显式使用官方 `cache_params.type: local` 进程内缓存，保持无 Redis 架构并由配置回归固定。
- **Web/extension vNext 重接**：从确定性 OpenAPI artifact 生成 Web、popup 与 TypeScript client；Web 使用 HttpOnly cookie + CSRF，扩展使用 device-key→finite bearer，SSE 统一由 authenticated fetch parser 消费。`/setup` 承载 onboarding/来源 bootstrap，`/web` 与 popup 承载完整 nested settings，`/m` 保留 feed、joined library、profile edit、chat history、feed replenishment progress 和高频设置子集；产品面共享 LiteLLM alias health/Admin navigation。扩展后台改为单一 generic source-task claim/complete dispatcher，七来源页面采集统一归一为 `ActivityEvent`。provider editor、native save/saved sync、delight/通知、self-update 与 desktop 控件退出 active markup、manifest 和 service-worker graph。
- **保留 Web journey 终验**：桌面、移动端与 popup 的 library 保存以 durable add 为权威，interaction signal 失败时仍显示已保存并只重试 signal；完成后按钮不再重复发 signal，重渲染后的幂等 add 返回首次 membership 且不会制造新 evidence。replacement Playwright suite 覆盖登录、desktop drawer、favorites/watch-later、partial save，以及 popup 与独立 `/setup/` onboarding retry，并由 CI 直接运行。三端仅针对明确下线的 delight、messages/probe、MBTI/speculative/insight、saved-sync 与 provider/model-route editor 家族移除 stylesheet selectors，保留现行 feed、library、profile、chat 和 settings 样式。

- **Manifest 驱动的扩展任务运行时**：service worker 每轮读取 `/api/v1/sources`，manifest 与本地 executor 的交集决定哪些来源轮询；后端返回的 durable claim 按本地 executor 稳定能力校验，因此 transport mode 切换前入队的 browser operation 仍可排空，而新任务继续服从当前 manifest。claim deadline 在调用 executor 前验证，执行中到期通过 AbortSignal 取消 tab/message/listener 等待并只回写 typed failure；迟到 result 不会 success-complete，所有 timer、listener 与临时 tab 均有界清理。
- **来源 transport 热更新与迟到入队终态加固**：API 每次操作、长期 worker 每个 source/feed job 都重读持久化 settings；candidate registry 先 preflight 再 commit，多个 API process 不依赖本地 publish。mode switch 后新 enqueue 使用新 transport，旧 browser row 仍按 enqueue-time contract 排空；timeout/cancellation 会结构化等待 enqueue 得到确定持久化结果，并在 cleanup retry window 内同步重试瞬时失败的 terminalization。window 只决定是否发起下一次重试，已启动的 SQLite persistence 仍完整等待并由 busy timeout 有界；不留下 shutdown 后才落库的后台任务，失败日志只保留异常类名。

- **Browser-complete vNext 后端合同**：新增 `/api/v1/auth` status/login/logout/extension-token/revoke，installer bearer、same-origin HttpOnly cookie + unsafe-request CSRF 与 finite extension bearer 并存；Alembic `0002_auth_state` 持久化非秘密 session epoch，统一撤销 Web/extension session。Library list 以单次 join 返回 collection metadata + normalized `ContentItem`；profile PATCH 用 expected revision 把 narrative/五类 facet override evidence 与一个新 revision 原子提交；chat 增加按 conversation 隔离、升序、有界且不含 provider/AI-run metadata 的 history。`UserSettings` 收口为 sources/schedules/feed/profile/tasks/network/logging/access_control/jobs 九个 strict nested group，deployment facts 明确 read-only。七来源 manifest 公开 safe settings/credential/per-operation schemas，browser task 改为七种 discriminated request/result，account disconnect typed/idempotent 且不回显 credential。AI health 只从显式、credential-free `OPENBILICLAW_LITELLM_ADMIN_URL` 投影 optional Admin navigation。Runtime/OpenAPI 统一 `{error:{code,message}}` 及 401/403/404/409/422/429/500/503 response，不覆盖 success/security/SSE metadata。Docker/source/installer 文档同步 password hash、session secret、extension digest record 的私密 provisioning 边界。上述为后端与离线合同完成，不声称 Task 22 前端接线、真实浏览器/extension、native Windows 或 live provider 验证。
- **Browser contract review hardening**：extension origin 明确排除 loopback trust 与 CORS auth bypass，只能 device-key exchange finite bearer；login/exchange 使用分离、per-peer、bounded failure limiter。vNext auth 改为 environment-only；password state 对 fresh absent/first enable/unchanged 幂等，rotation、removal 的 `disabled` sentinel 与 re-enable 都和 auth epoch increment 原子提交，旧 session 无法在关闭/恢复密码后复活。Source/Docker installer 在 migration 或 Compose 前生成并幂等复用 mode-`0600` `OPENBILICLAW_SESSION_SECRET`，不打印值；两份 Compose 只向 API 转发 password hash、required session secret、extension digest records 与 public Admin URL，不向 worker 传 browser-auth material。Starlette 404/405 统一进入 `ErrorEnvelope`。API container 以 zero-I/O deferred holder 等待 schema-head gate 成功后才读取 source settings 和安装 registry；Bilibili/XHS/YouTube/Twitter/Zhihu 的 per-source schema 为空，Douyin 只保留已消费的 `mode`，Reddit 只保留已消费的 `backend`，global `UserSettings` 独占 enabled/weights/schedule/feed policy。worker 为真实 entrypoint 安装或复用 owned console/rotating-file sinks，应用 persisted levels，并在退出/失败时保留 host root policy、只清理自身创建的 sinks；proxy 与四个 CA env 的进入前状态也在正常/异常退出后精确恢复。Explicit profile edit 使用严格晚于上一 revision 的 fresh timestamp。全部合同仍不包含明文 password/device key，也不宣称前端或真实浏览器完成。

- **Installer/queue/backup stable-root 终审收口**：root guard 现在校验全部 complete record，并以每代 pending/committed 两条相同记录证明提交；只恢复 single pending generation 0 或同 root/instance/anchor 恰落后一代的 record，缺失 established record、早期损坏或跳代均失败关闭。Migration 前已 advance 而失败时，下一轮仅在 active lease 内把同实例、恰落后一代的 process record 重绑到当前 generation，再验明并停止原 API/worker pair。`.env`、installer 与 process state 的原子替换全程保留 temp FD，只对 FD `fchmod`，replace 前后核对 held inode，失败保留不可预测 temp 名而不 pathname-unlink；Windows 通过 `CreateFileW(CREATE_NEW)` 以 read/write/delete sharing 创建 temp，将 handle 转成 O_RDWR CRT FD 并持有跨越 `os.replace`。Windows logs 通过不共享 delete 的 `CreateFileW` handle 拒绝目录/final reparse；POSIX 保持 held-dirfd 路径。Queue POSIX proof 要求 connection 新增的全部普通 FD 都属于预先 pinned main/WAL/SHM identity set 且包含 main，拒绝并发 dup/无关 FD 冒充。macOS backup 从 locked held payload FD 以 `fclonefileat` no-replace 发布并验证 exact bytes + SQLite integrity；`.backup-00.tmp` 至 `.backup-31.tmp` 只复用 locked、零长度、单链接且 held/path identity 一致的槽，成功 descriptor-zero+fsync，失败非零槽有界保留，Linux anonymous payload 不变。
- **44a installer/operations 终审修复 crash gap、WAL 与 managed leaf 边界**：root guard 改为 append-only complete-record lease；generation 先 fsync guard、再原子写 installer record，下一轮在 held root/guard lock 内仅自动修复 record 恰好落后一代且 root/instance/anchor 全相同的 crash gap，其余 mismatch 失败关闭。Huey health 不再切 `journal_mode=MEMORY` 或经 FD pseudo-path 打开 WAL 库；POSIX 正常 pathname connect 后以 process FD delta 证明 SQLite main DB FD 等于预先 held inode，Windows 在稳定 canonical-root 边界内保持正常可写 health。`logs/api.log` 与 `worker.log` 逐 component no-follow 打开，并拒绝 junction/symlink、非普通文件和多 hard-link。Windows orphan recovery 只接受 root guard 内 non-reparse、普通、单链接、path/held identity 一致的对象，不使用 Unix `fchmod` 或声称 ACL 等价。Backup main/sidecar error path 不再 verify 后 pathname-unlink；32 个固定 O_EXCL mkdir staging slot 消除 count/create 竞态并给出无运行中 backup 时的显式清理指引。canonical checkout root 是信任边界；不承诺抵御恶意 same-UID 替换整个 root 或全部 Windows coordination objects。
- **安装、queue health 与 backup 末轮 TOCTOU 加固**：source installer 在 POSIX 上先对 held canonical checkout-root directory 取得有界 flock，再打开持久 root guard 和内层 lifecycle anchor；完整 `InstallationState` 与 anchor lease 在等待后、业务前、合法 generation advance 和退出时精确复核，所有等待共享一个不可重置的截止时间。首次 metadata publication 保留 temp FD、仅对 held FD 执行 `fchmod`，并在 hard-link no-replace 前后拒绝 inode swap；`data/vnext` 逐 component held-FD 遍历，bound anchor 多 hard-link 失败关闭。native Windows 未实现等价 ACL/descriptor orphan proof，因此不再尝试 Unix-mode recovery。Huey write probe 通过已验证的 `/proc/self/fd` 或 `/dev/fd` 连接 held inode，并用 memory journal 完成回滚写入，pathname swap+restore 不能把 probe 导向替换库。Backup 明确仅支持 Linux/macOS；staging cleanup 不在 identity check 后 pathname-delete，private hard-link set 按每个候选 parent 最多 32 个有界保留，达到上限后须在无 backup 运行时显式清理。
- **源码安装事务具备跨进程唯一所有权**：local prepare/start 现在由独立且限时的 lifecycle file lock 串行，锁覆盖 verified stop、migration、API/worker launch、process-state publication、doctor 与 protected readiness；`SKIP_START=1` 也先验证停止旧 managed pair，migration 不再与运行中实例交错。首次并发调用打开并等待同一 O_EXCL anchor；初始化崩溃留下的未绑定 anchor 只有在 held FD 通过普通文件、单链接、owner、私密 mode 与 pathname identity 检查后才会原位清空并重新绑定，恢复过程不执行 pathname unlink。Installer metadata 通过 fully-synced private temp + hard-link no-replace 原子发布，在进入业务临界区前持久绑定唯一 lock UUID/device/inode，竞争出现的 pathname 不会被 `os.replace` 覆盖；POSIX 通过 held parent dir FD，native Windows 通过无 `dir_fd` 的 direct-path 分支与 Python 3.11 reparse metadata，在取得锁后及退出前重读绑定并复核 pathname。绑定后 pathname 缺失或换 inode、复制 lock inode，以及 symlink/junction ancestor 都失败关闭，不能产生第二 holder；generation advance 也不会保存被篡改的 anchor 字段。安装器以 `0600` 持久化 root-bound UUID，并把 runtime state 绑定 canonical checkout root、instance ID 与单调 generation；复制/移动/篡改状态及 directory/FIFO 等非普通 state 对象会失败关闭。`.env` 每轮按 instance metadata 强制重绑定 root/app DB/Huey path，同时保留 access/encryption secret 与外部 LiteLLM connection。停止或失败回收只终止已验明进程，不再 pathname-unlink state；ownership-bound dead record 保留到下一次验明 ownership 的 publication，因此旧 cleanup 没有删除新 generation 的路径竞态。
- **Docker 安装成功条件覆盖真实 worker**：两份 Compose 为 worker 增加运行中 PID 1、Alembic head、Huey SQLite 完整性和真实 `CREATE`/`INSERT` 后回滚的可写事务联合 healthcheck；queue pathname 在 SQLite connection 前后都必须仍指向 held descriptor inode。Installer 在受保护 API probe 前后各解析一次 Compose JSON 状态；只有一次性 migration 以 0 退出且 API/worker 两次均为 `running/healthy` 才报告成功，probe 期间进入 crash-loop 的 worker 不再被 API readiness 掩盖。Docker `SKIP_START=1` 现在也先执行隔离的 `compose run --rm migrate`，prepared 结果不再虚称已迁移。
- **vNext SQLite backup 关闭 source TOCTOU 且保留 WAL commit**：backup 不再在 `lstat` 后按原 pathname 重开数据库；main 先由 `O_NOFOLLOW` FD 固定并以 device/inode 验证，再把 main 与存在的 WAL/SHM/journal 以 hard-link 组成随机 `0700` 私有 source set。SQLite online backup 前后复核原 source/sidecar presence 与 inode，完整 snapshot 只写 held、已 unlink 的 `0600` payload FD；macOS `fclonefileat` 或 Linux `O_TMPFILE` + `linkat(AT_EMPTY_PATH)` 才会原子 no-replace 创建最终名称，因此 observer 看不到半成品。Linux capability policy 拒绝 `AT_EMPTY_PATH` 时，仅在 `/proc/self/fd` 与 held inode 一致后以 `AT_SYMLINK_FOLLOW` fallback；directory sync 后先复核 parent pathname/held dir FD，再核对 final pathname/held FD。发布后异常不按 pathname 清理 final，避免误删替换 inode。Windows/缺少安全 primitive 的平台在创建或预留 destination 前失败关闭。
- **Onboarding SSE 跟随完整 durable journey**：`/{source_run_id}/events` 从应用库幂等 continuation 解析当前 `source_sync → profile_projection → feed_replenishment` 子任务，在 progress 中暴露 stage 与真实 child run ID；只有 feed 成功且 `onboarding_complete` 已持久化才发送成功 done，任一 child 失败/取消则传播其终态，断线或进程重启后仍可用原 root ID 续接。空来源选择在排队前以稳定 typed `422` 拒绝，避免默认全 disabled 时启动必败工作流。
- **vNext JSON 响应合同落到真实运行时校验**：events、feed、interactions、sources/status 与 library 路由不再用 `response_model=None` 或宽泛 `object` 绕过 FastAPI；每个保留操作都以具体 domain/transport schema 同时驱动响应校验和 OpenAPI，application service 若返回畸形值会 fail closed，而不会带着文档声明的 2xx 状态泄出不合法结构。
- **安装与 Compose 切换到 vNext 双进程契约**：源码 installer 删除 legacy provider editor 与已移除 feature command 流程，改为要求用户提供 LiteLLM proxy、以 `0600` 原子持久化并幂等复用 access/encryption secrets，按依赖→migration→API+worker→queue readiness→`doctor`→public/protected check 顺序执行且失败非零退出；本地进程状态以 PID、OS 高精度启动时间、可执行文件和 argv 指纹共同验明身份，重跑只停止仍匹配的旧进程，TERM 后限时等待并仅对同一身份升级 KILL。API 与 worker 在 queue、doctor 和 HTTP checks 两侧都检查存活，旧 queue 不能掩盖 worker 退出；部分启动、状态写入或 readiness 失败会终止并回收本轮全部子进程。源码与预构建 Compose 删除 Ollama/Embedding seed sidecar 与相关环境、volume、dependency，收口为 LiteLLM/PostgreSQL、一次性 `migrate`、API、worker；只有 migrate 写 Alembic，成功后才启动只读检查 schema head 的 API/worker，避免 fresh SQLite 并发 DDL。三个应用服务显式使用相同 app DB/data volume，API/worker 共享 Huey SQLite 路径。README、架构、spec、模块与 agent/Docker 安装文档同步该所有权，并明确 static Web/extension 要到 Task 22 才完成 vNext wiring。
- **公开运行时切换到薄 `/api/v1` 与运维 CLI**：删除 11k 行 FastAPI app 与 9k 行 feature CLI 的权威入口，改为 12 个 feature router、注入式 `ApplicationContainer`、集中 401/403/404/409/422/503 错误映射和只来自环境的 constant-time bearer gate。chat、onboarding/job progress 使用标准 SSE，扩展来源工作只走 generic long-poll claim/complete；其中同步 claim、job inspect 与 chat DB 写入使用有界 AnyIO worker，不阻塞事件循环且取消不遗弃租约/事务副作用。公开 job priority 收口为三个命名 lane，来源 enable/weight PATCH 合并完整七源 map 并拒绝非法嵌套值，`onboarding_complete` 只由工作流推进；startup 失败也执行脱敏 cleanup。OpenAPI 以稳定 `v1_*` operation ID 可重复生成，frozen metadata 暴露正确 JSON-object schema 且无 secret default/example。CLI 仅保留 `serve`、`worker`、`doctor`、`eval`、`db migrate/backup`，Compose 的 `api`/`worker` 使用同一正式入口与 vNext readiness。现有静态 Web 继续挂载但尚未重接，新 API 不保留 legacy endpoint/command 兼容。

### Historical delivery sequence

以下条目记录 v0.4.0 构建过程中的阶段性边界；其中“尚未接入”“legacy 仍权威”等
状态已被上方 Current vNext authority 与较新的交付条目取代。

- **Task 20 复审加固 durable workflow**：DB→Huey 改用 immediate TaskWrapper enqueue 与 `job_runs.dispatched_at` publish-then-marker；普通重试 reconcile undispatched row，worker startup 重发全部 pending row，覆盖 queue failure、enqueue/marker 与 dequeue/claim 两个崩溃窗口，重复消息由原子 claim 消解且 restart-safe。四个 handler 的 progress checkpoint 保持独立可见，而 cancellation running guard 与 activity、profile revision+consumed ledger、content+assessment+feed entry、terminal cleanup 分别共用一个 UoW；guard、cancel、checkpoint、terminal/retry 和 recovery 都以条件 SQL write 开始，不再 SELECT 后 ORM flush 升级 SQLite 锁。写序与有限 busy timeout 保证 cancel 先则零后续 effect、guard 先则 feature commit 先于 cancelled，等待耗尽显式失败。`source_sync_interval_minutes` 通过每分钟 tick + DB 幂等时间桶生效。画像增加独立 `profile_consumed_evidence`，与 revision 原子提交并携带 expected base revision 拒绝陈旧 AI delta。Feed 在 batch 前排除同 revision 已评估和历史 admitted/interacted/dismissed 内容，有界扩量补足新候选，持久化全部评估，并把 topic cap 收紧为任一 topic 饱和即拒绝。两份 Compose 让 backend/worker 显式共享 vNext DB、Huey 文件继续隔离；真实 SqliteHuey dequeue-loss recovery、双写序并发、迁移 parity、并发/回滚、二次补池与四类持久化边界取消均有离线回归。
- **vNext 用例与四任务 worker 落地**：新增证据活动/画像、加权多来源 Feed、反馈、本地双集合与直接 interactive TaskRunner chat 用例；Feed 从严格 `UserSettings` 读取 enable/weight，以稳定 largest-remainder 精确分配有界候选预算，并只做一次 typed batch assessment。新增独立 `data/vnext/huey.db` 的四个 priority/retry/lock/periodic Huey task 与最多四线程 consumer；Huey result 只作 durable transport，应用库 `job_runs` 独占产品状态、幂等、取消和重启恢复。production worker 固定构造七个 built-in connector，不加载动态 source plugin；direct client/DB 密文解密均延迟到首次真实调用，默认 disabled 启动不触发 live source，缺少账户/secret/Cookie 以安全 typed 配置错误失败。源码/预构建 Compose 新增真实 worker 服务，legacy backend 保留；公开 HTTP API 与前端切换仍由后续任务完成。
...
- **类型化 AI 统一经 LiteLLM**：新增 PydanticAI `TaskSpec` / `TaskRunner`、画像增量/关键词/候选评估/推荐解释四个可复用 typed task、四组 task-specific offline invariant evaluator（画像 evidence/change、关键词 uniqueness/relevance、候选 identity/score/topic、推荐 grounding/length/concept）及推荐解释显式 eval 用 `LLMJudge` rubric、专用 embedding 与分级 alias health；CI 接受语义有效但不等于 expected output 的答案且不运行 live judge。中文推荐 grounding 改用排除通用话术的重叠 CJK 2/3-gram，并要求 shared trigram coverage 或两个独立 unit：允许“这个视频很符合你对实用建模教程的兴趣。”等已评审改写，拒绝无关泛化文案和仅顺带提到“建模”的混合幻觉；该 deterministic guard 不声称覆盖所有幻觉，主观事实忠实度仍由显式 `LLMJudge` eval 负责。应用只认识 `obc-interactive`、`obc-analysis`、`obc-embedding`，LiteLLM 独占 provider routing/fallback、网络重试、限流和缓存，OpenAI transport retry 明确为 0，BYPASS 映射到官方 `cache.no-cache` 请求体。复审后收紧 provenance：画像输入改为 evidence UUID+content，所有 facet 引用必须来自输入；candidate 不让模型生成 application row ID，并强制复制 content/profile identity；四任务都有参与 output retry 的语义 validator。`ai_runs` 从 ORM、未发布 Alembic 与 repository API 结构性删除 output payload，只保留 task/model/status/timing/usage/error class。源码与预构建 Compose 共用 `litellm/config.yaml`，Admin 默认 loopback，固定 upstream `v1.92.0` tag（不宣称签名验证）；installer 以跨进程锁、symlink 拒绝、同目录 `0600` 临时文件、`fsync` 和原子替换生成基础设施密钥。legacy backend 服务与命令保留，typed AI 尚未接入生产 composition/use case。
- **七平台能力连接器与通用浏览器任务合同**：显式区分稳定 capability 与 concrete operation，并为每个 operation 声明 auth、result kind、direct/CLI/browser transport；B 站不再虚报 native explore，X retained likes/bookmarks 组成真实 bootstrap。现有 Bilibili、Douyin、YouTube、X、Reddit read client/CLI 与 browser queue 都有 production adapter/builder。generic queue 只接 browser operation，支持 bounded async wait、并发单 owner claim、并行 completion 幂等/冲突；payload/result 会递归拒绝 singular、plural、qualified、nested credential container 且不回显值。知乎按 retained flat `author/summary/interaction_action` 归一化并以 `content_type:id` 保持跨类型身份稳定。闭合 settings mode 拒绝任意字符串。`import-linter` 保持 feature→infrastructure 门禁。本项尚未新增 HTTP route、替换 legacy endpoint 或重接扩展 dispatcher，当前生产来源路径不变。
- **来源 task cancellation / fallback / 数据边界加固**：operation metadata 新增 optional fallback transport，B 站 search 明确为 direct primary，并只在 retained cooldown / DOM-fallback signal 下排入 browser queue；普通 TypeError/validation error 不触发 fallback。browser transport 在 enqueue 时持久化由 execution timeout 推导的绝对 request deadline；claim/complete 的 deadline/lease 条件与新 lease timestamp 均在原子 SQL 内按 SQLite 数据库时钟求值，写锁等待后不会使用陈旧 Python 时间。timeout 或 asyncio cancellation（含 enqueue thread 在途）进入独立、抗二次取消且与 SQLite busy timeout 对齐的有限 compensation window，成功写 `cancelled`，清理超界/失败则由 deadline 保证不可再 claim，并在 snapshot/claim 时写 `abandoned`；超界的 late enqueue 由 done callback 取回 outcome，显式吞掉 `CancelledError`，其余 `BaseException`（含 group / `SystemExit` / `KeyboardInterrupt`）只记录类名，不泄露 message/value、不会逃逸回调或触发未取回 task exception。总返回可超过 execution timeout，不声称返回前一律已写终态；原始 TimeoutError/CancelledError 不会被 cleanup 异常替换。task request/result 复用 frozen metadata finite-JSON 验证并以 token classifier 拒绝 `cookie_jar` / `authorization_header` 等 qualified secret container，保留窄 analytics allowlist。bootstrap/activity limit 端到端透传并在 normalization 后硬裁 N；知乎互动时间优先 retained `interaction_time`。真实 deferred registry provider、全部 browser builder、retained client/CLI 全操作 mapping 均由 SQLite 合同测试覆盖。HTTP/扩展仍未切换。
...
- **模型配置重构收口并删除 legacy runtime 死代码（阶段 15）**：生产配置、runtime、API、CLI、bootstrap 与独立评估/优化脚本统一只消费 `Config.models`，经 descriptor registry、`ModelConfigService` 和原生 ordered Chat/Embedding factories 构造同一个 `RuntimeModelBundle`。删除旧 `LLMConfig` / provider buckets、`LLMRegistry`、vendor provider subclasses、旧 builder 与 module override；DeepSeek / OpenRouter 继续作为统一协议 adapter 的 presets。`RuntimeContext.llm_registry` 仅保留为指向 `OrderedLLMRoute` 的字段名兼容别名，不承载旧类型或构造行为。legacy `[llm]` 只在加载时生成只读迁移候选，legacy `/api/config` 只返回无凭据投影并忽略模型写入；普通配置保存保留原始 authority，只有模型 service 可权威替换。仓库脚本统一委托公开 runtime bundle builder，402 归一为固定安全 rate-limit 错误，模块、架构、CLI、安装与部署文档同步到当前数据流；本条不提升版本号或修改 README 发布预告。
- **阶段 15 终审关闭配置/安全边界**：OpenAI、Gemini 与 DashScope Embedding transport 最多尝试 3 次，Ollama 最多 2 次，终态失败及 DashScope HTTP 200 error envelope / malformed JSON 映射为无上游原文或 raw cause 的 typed failure，connection 等类别可正确打开 circuit；`PUT /api/config` 在 canonical writer 内重读含 local/env 覆盖的 effective 模型 authority，不再用排队前快照发布陈旧 runtime 或丢失 `config.local.toml` route。权威模型校验统一执行 Chat 并发 `1..16`、timeout 至少 10 秒、Ollama `num_ctx >= 0`、Embedding 维度非负、阈值为 `0..1` 有限数值等不变量；原生解析与 legacy 迁移都把超大数值转为稳定 validation/parse issue，不再抛出 `OverflowError`。`config-show` 与 API 共用 proxy userinfo 脱敏。Bash/PowerShell 安装器的恢复指引改为关闭回显的交互提示，不再打印携密钥/Cookie 的 argv 示例，并保留原始 runtime mode、已知 connection type/preset、完整 Chat/Embedding 顺序和其它非秘密选择，只询问未决项；已链接的 agent 部署文档改用 connection type/preset、可重复 Embedding endpoint 与安全 secret 恢复路径。本条不提升版本号或修改 README 发布预告。
- **首启、安装、Docker 与桌面打包统一写入原生模型路由（阶段 14）**：`/setup/` 首页改为消费 connection-type descriptor 与脱敏 model snapshot，按“类型 → preset/OAuth → 字段”创建或编辑第一条稳定-ID Chat 记录，先 exact probe 再 revisioned `PUT /api/model-config`；fallback、Embedding route 与共享 settings 原样保留，409 会载入最新快照，credential 只提交 `keep/set/env/clear`。`config.example.toml`、agent bootstrap、Bash/PowerShell 安装器、Docker sidecar seed 与 frozen desktop helper 全部改用 schema-version-1 `[models]`；bootstrap 新增 canonical `--connection-type/--preset`、保留精确映射的 deprecated `--provider`、删除 module override，并逐 stable ID 检查 primary Chat 和每个 Embedding Provider。复审修正 legacy 单 Embedding alias 丢弃 fallback/ID 冲突、route 选择前过早复用 credential、同一 legacy 来源跨 Chat/Embedding 重复消费及首个 Embedding 探测异常提前终止；显式 flag credential/cookie 仍覆盖复用值，其中只传 `--embedding-api-key` 会更新现有有序 Embedding route 内全部支持 credential 的 Provider、保留无凭据类型和全部路由元数据，无适用 Provider 时在任何 route/reuse 配置或 cookie-file 写入前明确失败。窄屏 setup 改为可返回的「列表 → 详情」，切换类型/preset 保留适用的用户编辑字段与稳定名称并清除无效字段。Docker 只在首份配置中 upsert `ollama-docker`，已有配置不覆盖；桌面包只在 route 可安全播种时用 typed renderer 更新本地 Ollama Embedding，远端顺序、凭据 provenance 与无关配置保持不变。
- **CLI 模型配置切换到原生有序路由（阶段 13）**：新增 `openbiliclaw models list/add/edit/remove/move/probe`，Chat primary/fallback 统一为最多 10 项的稳定-ID 有序列表，Embedding 支持最多 10 个 Provider 并强制共享同一 model/维度/阈值/多模态设置。命令按 connection-type descriptor 选择 compatible preset 或独立 OAuth 类型，公开输出只显示脱敏 credential source；离线 circuit 状态明确为 `unknown`。写入直接调用 `ModelConfigService`，使用 revision guard、一次稳定-ID rebase、封闭 migration resolution 与 exact single-ID probe，不复制 TOML/validator 或 API 私有 helper。`init` 和 `setup-embedding` 同步改用原生 descriptor-driven 编辑器，不再写 legacy Provider/module override，也不在配置阶段安装、启动或探测真实 Provider。正式复审进一步保证 Chat 引导按稳定 ID 保留并发新增的 fallback 与完整顺序，所有引导入口统一经过不保留 inline secret traceback 状态的安全边界；`init` 只对 `models.*` / legacy `llm.*` 错误打开编辑器并在认证前复验，非交互 `setup-embedding` 固定返回安全指引与非零退出码。二次复审把 `--api-key` 在进入 Click/Typer 参数状态前换成一次性 opaque handle，命令入口 claim 后由 context close 与外层 group 双重按 handle 清理，框架 traceback、closure 与 `ctx.params` 不再保留原值；guided Chat 的首次 add/edit 意图在冲突重放中保持不变，目标被删除或 ID 被并发占用时在第二次保存前安全停止。最终安全复审把 `args=None` 统一为可回写的 `sys.argv[1:]` 命令视图，并以 root/direct subgroup 显式作用域在识别子命令前清理 `--api-key`；因此 malformed model 子命令与提前解析失败也不会在 argv、traceback 或 parser context 中保留原值，同时顶层其它命令的同名 option 保持原语义。末轮复审补齐 Click 对 dash-prefixed 分离值的真实消费语义；sanitizer 从 token 生成前就登记 handle，并在任意 BaseException 下清理 partial vault、脱敏 argv/traceback locals。`args=None` 在安全回写 `sys.argv` 后继续以 `None` 交给 Click，保留 Windows argument expansion。最终窄复审改为按选中子命令的真实 Click 参数语法定位 credential：其它 option 消费的形似 key 值和 `--` 后的字面参数不再误清理，未知子命令仍保守清理到 terminator；vault 的原值绑定也完整位于 `try/finally` 内。Click group 在子命令前消费单个 `--` 的有效写法也纳入同一保护，而 leaf command 自己的后续 terminator 边界保持不变。
- **阶段 12 终审补齐移动 Models 的错误读取与可访问加载状态**：数值与服务端字段错误现在对稳定连接 ID 和字段名都只读取 own property，`constructor` 连接的真实 `name` 错误可正常展示，缺失映射不会回落到对象原型。编辑器的锁定/inert 与 `aria-busy` 分离：首次加载、在途加载和重试为 busy，settled-not-ready/error 继续锁定但不再误报忙碌，ready 后解锁；实时状态和重试入口位于 inert 边界之外，加载完成或失败时辅助技术仍可感知。销毁后的 readiness 通知继续由 disposal guard 丢弃；补充 dependency-free Node 行为与静态契约回归，未启动浏览器、服务端或真实 Provider。
- **阶段 12 二次正式复审关闭移动 Models 的恢复与数值草稿死角**：完整加载被较新 reload 取代后，即使胜出的 reload 失败、旧请求只以 stale 完成，移动端也会把“已结束但未 ready”呈现为可重试错误并继续锁定；显式重试成功或迟到的胜出 reload 会自动隐藏重试、同步状态并仅在 snapshot/descriptor 都 ready 时解锁。移动端在共享 serializer 之前统一校验 Chat concurrency/timeout、每个 Chat connection 的 `num_ctx`、Embedding 输出维度与相似度阈值，空输入保持为空而不暗转 `0`，非法草稿保留字段/路径反馈并阻止保存与 exact probe；显式 `0` 仍是 `num_ctx`、输出维度和相似度下界的合法值。错误会随每次草稿 mutation、远端放弃和保存成功重新推导；Pydantic `422 detail` 只映射安全路径/固定消息，并按稳定 ID 关联连接且防御 `__proto__` / `constructor` 键污染。补充 dependency-free Node 竞态、生命周期、边界与脱敏回归，未启动浏览器、服务端或真实 Provider。
- **阶段 12 正式复审补齐移动 Models 的资源恢复与精确草稿投影**：新增 production-used `mobile-model-settings-controller.js`，分别跟踪 snapshot / descriptor readiness；Saved Sync 先收到 reload 不再跳过 descriptor，失败资源可由显式按钮或重新进入 Models 单独重试，重叠 full load 与较新 reload 保留最新 snapshot 和当前 descriptor，两项 ready 前编辑器保持锁定。model / preset / credential / Embedding 共享设置和 migration mutation 会同步刷新列表、probe health 与错误投影，纯 Back 导航保留服务端错误；Runtime 非法 concurrency/timeout 在 PUT 前就地拦截；shell 替换设置按钮后关闭 overlay 会聚焦当前 live opener。补充无浏览器、无真实 Provider 的 Node controller 行为套件，并澄清只有浏览器插件提供 Ollama route 快捷动作，桌面 Embedding repair 与该动作彼此独立。
- **移动 Web 模型设置升级为权威有序路由编辑器（阶段 12）**：顶部设置 overlay 保留 Saved Sync，并新增与其独立保存的 Models section；Chat / Embedding / Runtime 使用触摸友好的 sequential list→detail，稳定 ID 列表最多 10 项，以 Move Up / Move Down 排序，Back 返回列表不丢草稿。移动端直接复用桌面 `web/shared/model-config-state.js`，消费 descriptor 分组搜索、preset 条件字段、显式 `keep/set/env/clear` credential、Embedding 共享设置、override/migration/字段错误、revisioned save 与 exact probe；通用 `PUT /api/config` 只发送 Saved Sync，模型只走 `PUT /api/model-config`，不提供一键 Ollama。snapshot 与 descriptor 使用独立 latest-request gate，保存锁使旧 probe 与 GET 失效，迟到 snapshot 只记录远端更新；`config_reloaded` 对 clean draft 自动 hydration、对 dirty draft 保留本地修改。关闭/重开生命周期现在把 DOM 查询限定在当前 overlay，并丢弃旧实例迟到的加载、保存和 probe 回写，避免复用 ID 时污染新设置层。自动化契约覆盖模型 API、列表/详情、触摸排序、竞态、dirty 导航、焦点恢复与既有 Saved Sync 同意文案；未启动浏览器或真实模型服务。
- **浏览器插件模型设置升级为权威有序路由编辑器（阶段 11）**：popup 的 Models tab 改用自包含的 DOM-free state 与 controller，Chat primary/fallback 统一为最多 10 项的稳定-ID 有序列表，Embedding Provider 使用同一列表语义并共享唯一 model/维度/阈值/多模态设置；连接类型、preset、字段与 credential action 由 descriptor 驱动，窄窗按 sequential list→detail 操作。snapshot、descriptor、revisioned save、exact probe、迁移 resolution、dirty remote 与 conflict 均走独立模型 API，通用 `/api/config` 保存不再携带模型字段；snapshot 与 descriptor 分别执行 latest-request ownership，snapshot-only reload 不丢弃仍有效的 descriptor，而重叠的完整加载只安装最新 descriptor。加载期间产生的本地编辑保留为草稿并提示远端版本；save 会使旧 probe 失效，完成后按统一 operation state 恢复控件。一键本地 Ollama Embedding 只创建或复用空的/单一 Ollama route，且仅在 snapshot 与 descriptor 都赢得当前加载、期间没有本地编辑或其他 save 时发出 PUT；非 Ollama/多 Provider route、脏草稿、override 或 revision 冲突均明确拒绝覆盖。
- **桌面模型设置升级为权威有序列表/详情编辑器（阶段 10）**：设置页新增 Chat / Embedding / Runtime tab；Chat 的 primary/fallback 使用同一种稳定-ID 记录，支持最多 10 项的新增、删除、拖拽/按钮/键盘排序与选中项 inspector，Embedding Provider 使用相同有序列表并共享唯一 model/维度/阈值/多模态设置。连接类型改由 descriptor API 驱动的可搜索纵向分组列表，OpenAI/Anthropic API-key 家族各自归入 compatible 类型 presets，每个 OAuth 类型独立显示；跨 OAuth 边界会确认并替换不兼容 credential 语义。窄屏使用 list→detail；模型保存、exact probe、迁移 resolution、按稳定 ID 映射的字段错误和 revision reload/conflict 均走独立模型 API，通用保存不再提交 legacy `llm`。新增/切换 Codex OAuth 时 `keep` 会安全解析为导入引用，反向误留 OAuth 引用仍失败关闭。
- **阶段 10 复审关闭模型编辑竞态与迁移位置漂移**：legacy `add_to_chat_route` 省略位置时改由后端在全部确认删除后，按 issue 顺序占用最高一组剩余 tail slot；显式 one-based position 继续支持，并按删除后的最终 route 做越界/冲突校验，Desktop 不再发送会因同期删除而过期的客户端位置。Desktop exact probe 以 generation + revision + kind + stable ID + draft fingerprint（Embedding 含共享 settings）验收完成结果，重排/切换只把结果挂回原 ID，编辑或 revision 变化丢弃旧结果且不污染另一 inspector；初始 snapshot 与后续 reload 共用 latest-request generation，descriptor 即使在初始 snapshot 被取代时仍独立安装；模型 PUT 在途锁定完整编辑器并拒绝二次保存/草稿 mutation，开始保存即失效全部已发出的 GET snapshot，完成 GET 仅在 generation 仍为最新且保存锁已释放时应用，旧 reload 的成功或失败均不会覆盖成功响应；失败保留草稿、成功只接受响应 snapshot。Embedding Provider route 由 local override 锁定时 enabled 同步锁定，共享 settings 仍可保存；连接类型重绘后恢复选中 option 焦点。该阶段保留的插件 legacy 模型 tab 已由下述阶段 11 权威编辑器迁移完成。
- **模型配置获得独立、可演进的权威 API（阶段 9）**：新增严格且拒绝额外字段的 `GET/PUT /api/model-config`、descriptor-driven `GET /api/model-connection-types` 与 exact `POST /api/model-config/probe`；公开 snapshot 只携带凭据状态、迁移/override、按稳定 ID 绑定的安全探测时间/revision 和 live circuit 摘要，Embedding 共享 model/维度/阈值/多模态设置也属于精确探测身份。保存沿用 revision/authority guard、原子文件/runtime transaction 与唯一 `config_reloaded`；stale 写返回最新 snapshot，字段/schema 错误不回显提交值。成功探测当前持久化记录只关闭同 ID/capability/current-revision circuit，edited/unsaved draft 既不动 live circuit，也不覆盖最近 exact summary。legacy `GET /api/config` 仅保留 primary + first fallback 的非权威、只读、无凭据投影；原生 `[models]` 下旧 `PUT /api/config` 的 `llm`/reset 被忽略并返回 `model_config_not_updated`，其它通用设置仍可保存。旧 probe-service 仅保留 network proxy。
- **阶段 9 复审补齐 probe、lifecycle、degraded 与 init 竞态边界**：exact probe 在 maintenance gate 准入后重查 init，并在取得短 model path lock 后再次重查 init，再把 `keep` credential 绑定到请求 revision；因此 probe 排队等待慢保存期间启动的 init 会在 credential/network 前返回 `409`。网络期间不持配置锁，完成后重读 revision/record/shared settings，stale A 不会借用 B secret、写入 B history 或关闭 B/live circuit。模型 API 改由显式 app lifecycle coordinator 执行「无事件发布完整 graph → 清退 registry 中除 `guided_init` 外的旧 graph 工作 → 重启新 graph app loops → 清除 degraded → 单次 `config_reloaded`」；已异常/取消的旧 loop 只作为 cleanup 结果，保存调用方取消仍传播并触发 rollback。restart/swap 失败恢复旧字节与完整 normal/degraded runtime identity，按旧 ownership 重建等价 app loops，但已清退 detached 旧 one-shot 不会复活；直接 `RuntimeContext.swap_model_candidate()` 兼容行为不变。guided-init `try_start` reservation 与模型 canonical precommit guard 现共享同一 path writer，消除 handler 初检后 init 与配置 commit 交错窗口。
- **阶段 9 lifecycle 终审关闭 guided-init 完成与模型 cutover 竞态**：`RuntimeContext` 新增稳定 background lifecycle lock，所有 public stop/restart 在一次 ownership 内串行 slot clear、registry drain、loop creation 与 post-reload one-shot scheduling，仍明确排除 `guided_init`，因此并发 restart 不会各自创建一套 loops、覆盖 app slots 后留下 orphan registry task，reload event 只对应最终 slots。模型事务在 backup/replace/publication 前通过 app coordinator 的可选 async capability 等待 settled runtime/task 快照；canonical writer → lifecycle lock 保持单向，等待时取消不改磁盘/runtime。写盘后的取消仍由 shielded rollback 重新取得 lifecycle ownership，恢复旧 graph 与等价 loops 后再传播 `CancelledError`。
- **生产模型调用统一切换到原生 ordered runtime bundle（阶段 8）**：`RuntimeContext` 只发布一个不可变 `RuntimeModelBundle`，在锁外完整构造 Chat/Embedding adapter、service 与 Soul/Dialogue/Discovery/Recommendation/controller consumer graph，再在短锁内一次性切换；旧在途调用继续使用旧 route，失败或配置事务 rollback 恢复精确旧对象身份。稳定 LLM gate 只在成功 publication 时按 `models.chat.concurrency` 更新，模块 override 与旧构造参数已删除；API runtime、CLI、OpenClaw、health 和 Ollama 判断统一读取 `Config.models`。`llm_usage` 新增稳定 connection ID/type/preset/position，并自动迁移旧库和建立连接时间索引；`openbiliclaw cost` 新增 connection 视图，同时保留 provider 汇总。阶段 9 已完成权威模型 API 与 legacy 写保护，阶段 13 已完成 CLI setup/save 迁移。
- **阶段 8 runtime composition 复审修复**：guided-init readiness 改为按 `OrderedLLMRoute.connections` 顺序探测真实 adapter，primary 失败会继续 fallback，`/api/init-status` 与 `/api/init` 不再调用缺少 connection ID 的 legacy `get()`。模型 candidate 在 canonical rebase 后会用当前 live config 同步 restage 完整 consumer graph，保留 candidate 构造期间普通 writer 已发布的 scheduler/source 等非模型设置；restage 失败保持磁盘/runtime 原样。CLI 与 OpenClaw `SoulEngine` 都接收同一 bundle 的精确 Embedding service，避雷确认可走共享 semantic pool cleanup。Ollama general startup 明确为 Chat-first 单托管 daemon 策略；Embedding repair 的 not-running start、provider-error restart 与 model-path migration gate 改用被诊断 provider 自己的 root，Chat `11434` + Embedding `11435` 时绝不误操作 Chat daemon。
- **阶段 7 endpoint 安全边界补齐 base/local 整数组遮蔽场景**：`ModelConfigService.read()` 现在分别对 base persistence 与 base+local effective 模型执行 endpoint-only 检查，local 中安全的完整 Chat/Embedding 数组不再能遮住 base 中带 userinfo/query 等不安全 endpoint。初始 split 与 canonical rebase 后同样在 candidate build/render 前显式检查 persisted endpoint，并继续对 effective 做完整 validation；base 缺 credential、由 local 合法补齐的配置仍可读取和保存。错误保持字段化、无原 URL/credential/cause/context，阻断路径不构造 candidate、不写盘也不 swap runtime。
- **阶段 7 模型配置复审补齐四条安全/事务边界**：所有已填写的原生 Chat/Embedding endpoint 现在共用一条 secret-safe HTTP(S) validator，并在公开 snapshot、保存、credential/proxy callback 与 SDK 构造前拒绝 userinfo/query/fragment、控制/空白字符、反斜线和非法 host/port；磁盘旧值不安全时读取字段化失败且不回显。base persistence 与 base+local effective authority 分离，local-only / base+local legacy 会从真实合并值生成迁移报告并阻断未转换的 local legacy 保存，native precedence 不再产生错误 provenance。新增 `config_write.py` canonical in-process path boundary，模型 candidate build 后提交前即时重读：无关普通配置变化从最新原字节 rebase，模型 authority/revision/provenance 变化返回 conflict；同一 async ownership 覆盖 swap/rollback，同步 writer 在他方事务中快速失败。model-only source scanner 还会识别 multiline basic/literal string，不再把字符串里的 `[models]` / `[llm]` header 当成真实 table。该协调不包含跨进程文件锁；即时重读只覆盖读取前已可见的外部变化，外部 writer 仍可在重读到替换的窄窗口竞争。
- **模型路由保存获得独立事务边界（阶段 7）**：新增 `ModelConfigService` 的脱敏 snapshot、revision guard、stable-ID 增删改移、exact draft probe 与 `keep/set/clear/env` credential action；首次 legacy 保存先建立不覆盖、保留 mode 的 pre-refactor backup，再用同目录 temp、文件/目录 fsync 与 `os.replace` 原子落盘，最后短锁交换预构造的 `RuntimeContext` candidate。写入、swap 或 swap 期间取消会恢复旧字节、mode 与精确 runtime 身份并保留取消语义；`config.local.toml` shadow path 保持只读且不烘焙。model-only source editor 用 `tomllib` 识别并去重 `[models]` / `[llm]` authority，未知 table、边界注释、空白与 CRLF 原字节保留，inline/dotted 歧义失败关闭。该阶段留下的 production composition 已由本版本阶段 8 条目完成；模型编辑 API/UI 仍后续迁移。
- **Embedding 图像能力检查不再吞掉程序错误**：`EmbeddingProtocolAdapter` 的原生 model checker、`OrderedEmbeddingRoute` 的能力 property 与 direct `EmbeddingService` 的 checker 现在只把明确返回 `False` 视为不支持；`ValueError` / `TypeError` / `RuntimeError` / `AssertionError` / 取消会原样传播，不再伪装成 `image_capability` fallback，也不调用后续 Provider、写 circuit/cache 或设置不可用原因。
- **Embedding route 复审加固**：原生维度为 `0` 的多模态 exact probe 现在要求同一 Provider 的文本与固定图片向量长度一致，不一致会以安全 `config_error` 打开精确 ID+revision 的永久 circuit。Chat/Embedding 的普通并发成功只能清除 timed/transient 状态，不再覆盖调用期间新打开的永久保护；只有对应 exact probe 成功可关闭永久态。fallback 和产品降级只处理类型化 Provider 失败及明确识别的 transport 失败，取消、调用方错误和未知编程错误会传播且不写 circuit/cache/不可用原因。
- **Embedding 共享设置接入稳定 ID 有序路由（阶段 6）**：新增 `llm.embedding_route.OrderedEmbeddingRoute` 与原生 `build_ordered_embedding_service()` 构造入口，1–10 个 Provider 严格按 `models.embedding.providers` 数组位置执行，允许同类型 endpoint 由不同稳定 ID 独立存在，并保证 Provider 内 transport retry 完成后才 fallback。所有 adapter 必须持有同一个不可变 `EmbeddingModelSettings` 对象；空、非数值、布尔、非有限和错误 shape 向量只在当前调用 fallback，非零共享维度不匹配才打开 `(provider ID, revision)` 永久 `config_error` circuit，失败精确探测不会削弱已有保护，成功只关闭目标 revision。`probe_provider(id)` 绕过目标 circuit、只调用该 ID、不走 fallback、不读写 cache/config；多模态开启时使用仓库固定 1×1 PNG 与 `image/png`，报告观测维度但不声称证明不同 endpoint 的远端权重一致。`EmbeddingModelSettings.cache_namespace()` 只散列 model/维度/阈值/多模态开关，Provider ID/顺序变化可复用缓存而任一共享设置变化会隔离；`EmbeddingService` 注入原生 route 后从该 settings 派生全部产品参数，拒绝缓存无效/错维向量，route 耗尽仍返回 `[]` 并只保留固定安全原因。阶段 8 现已将该 route 接入 production bundle。
- **Chat 从单备选升级为稳定 ID 有序路由（阶段 5）**：新增 `llm.route.OrderedLLMRoute` / `CircuitTable`，允许同一种连接类型以不同 connection ID 重复出现并严格按 `models.chat.connections` 数组位置尝试；Provider 内 transport retry 完成后才 fallback，整条 route 共用一个总 deadline，剩余时间耗尽后不再启动新连接。rate-limit 优先读取适配器安全解析的 `Retry-After`（缺失时 60 秒），auth/model-not-found 保持 open 到 revision 变化或 exact probe 成功，timeout/connection/5xx 按 15/30/60/120/240/300 秒冷却，invalid-response/moderation 只在当前请求 fallback。Circuit 由 `(connection ID, revision)` 精确隔离；查询、失败和成功不会跨 revision 删除状态，失败 exact probe 也只会保留或延长仍 open 的保护，成功只关闭当前 revision。耗尽错误只携带固定摘要的 connection ID/type/preset/position/failure kind，不保存上游异常或 secret；`LLMResponse` 在保留 provider/model 计价字段的同时补齐四个 connection metadata。`LLMService` 的普通、结构化、多模态和工具路径现只走一个全局 `complete()`，模块 caller 仅用于并发/usage；阶段 8 现已删除 module override 构造数据并完成 production composition。
- **模型连接记录可安全构造协议 Adapter（阶段 4）**：新增 `llm.connection_factory`，以稳定 connection/provider ID 命名 Chat 与 Embedding adapter；OpenAI、DeepSeek、OpenRouter 与 custom preset 统一构造同一个 `OpenAIProtocolProvider`，每个实例使用 frozen options 隔离 API mode、reasoning body 与 attribution headers，不再通过可变子类状态切换 DeepSeek 请求；hook 元数据不进入 repr，env credential 来源在 runtime options 构造时复制成只读快照。新 protocol adapter 在 SDK catch 边界把 raw 400 转成不含原文的私有布尔信号，以保留 Responses temperature 和 Chat / Responses JSON format 降级；无关 400 是不重试的固定文本错误，原生 `openai.APITimeoutError` 归一为可重试 timeout。Anthropic 官方和 custom gateway 统一构造 `AnthropicCompatibleProvider`，按 timeout/connection/5xx 与 429/auth/其他 4xx 区分重试，并把 cache read/create 纳入 total usage；Gemini 与新 protocol adapter 的未知上游异常只产生固定 ID 文本且不保留 secret-bearing cause/context。OpenAI / Anthropic custom URL 在任何 proxy/client callback 前由共享 HTTP(S) validator 拒绝 userinfo、query/fragment、控制字符、反斜线、外层空白、无效 host/port，合法 path/port 原样保留；Gemini 保持原生 SDK，Ollama 保持 `num_ctx` 与强制直连。inline / 精确 env / Codex OAuth / descriptor 允许的 none credential 只在构造时解析，Codex token lookup 前严格拒绝任何非精确 OpenAI 官方 scheme/host/port/path/query/fragment/userinfo endpoint。Embedding provider 记录仍不含 model/setting override，adapter 持有调用方传入的同一个不可变共享 settings 对象。legacy `build_llm_registry()` / `build_embedding_service()` 构造 API 与行为继续保留；阶段 4 当时尚未包含 ordered route/circuit，现由阶段 5/6/8 补齐路由与 production composition，模型编辑 API/UI 仍后续迁移。
- **模型连接配置领域基础与 Embedding schema 校验修正**：新增标准库 `model_config/` 领域层，以不可变 tuple schema 表达有序 Chat route 与共享模型设置的 Embedding route，并提供代码内、JSON-safe 的 connection type / preset / field descriptor registry、字段化验证问题和无密钥 DeepSeek 默认值；修正 typed `EmbeddingProviderConfig` 被硬编码候选字段误判为携带 provider-level `model` 的问题，raw provider 显式写入 `model` 仍会被拒绝。本阶段不接管 TOML、legacy `[llm]`、runtime adapter/route、API 或 UI。
- **版本化模型路由持久化且不静默迁移**：`model_config` 新增严格的 `schema_version=1` 原生 `[models]` parser、确定性 renderer 与 secret-fingerprint revision，`Config` 暴露原生模型值和不落盘来源元数据；普通 `save_config` 会按目标文件的 parsed raw table 保留既有 `[models]` / `[llm]` 未知嵌套字段、array-of-tables 顺序、凭据以及 `config.local.toml` 分层，只有显式 `models_authoritative=True` 才写原生 `[models]` 并移除 legacy 段。本阶段仍不做 `[llm]` 迁移，也未切换 runtime adapter/route、API 或 UI。
- **模型配置普通保存改为失败关闭与字段级覆盖保护**：已有配置无法读取或 TOML 损坏时，`save_config` 现在会在写入前以 secret-safe `ConfigError` 中止并保留原字节；环境变量和 `config.local.toml` 改为记录精确 `override_paths`，只阻止覆盖字段回写，其他显式 legacy 编辑、未知扩展和并发新值继续保留。原生与 generic renderer 共用 TOML basic-string 编码并正确转义 DEL 等控制字符，同时 renderer 仅接受精确整数 `schema_version=1`。
- **Legacy 模型配置获得确定性只读迁移报告**：仅有 `[llm]` 时，loader 现在按显式 default → fallback 顺序在内存构造 `Config.models`，并以稳定、跨 Chat/Embedding 唯一的 ID 保持重复加载 revision 不抖动；OpenAI 官方/自定义、DeepSeek、OpenRouter、generic compatible、Anthropic 官方/自定义、Gemini、Ollama 与 Codex OAuth 均按新 connection type/preset 映射，Codex 只引用 `credential_ref=codex`。未路由 credential、模块覆盖、未知/翻译污染值、无效 auth 和不同空间的 Embedding fallback 进入不含 secret 的封闭 resolution report；缺失/未知 choice 与非法 position/shared-settings payload fail closed。`[models]` 共存时完全忽略 `[llm]` 值和凭据；启动与普通保存仍不写迁移结果，后续阶段已经完成事务 backup、runtime route、API、四端 UI/CLI 编辑。
- **Legacy 模型迁移安全边界加固并拆分职责**：raw legacy 字段改为精确类型检查，数字、布尔、列表与 table 不再被静默字符串化或截断；统一 endpoint inspector 只按规范 HTTPS host/path/default port 识别官方服务，并在 userinfo、query、fragment、异常端口/路径或控制字符出现时阻断且不把原 URL 留进候选、issue 或 `repr`。Embedding fallback 现在要求显式启用、Provider 可用，并逐项核对有效 model、维度和 model 级 multimodal capability；resolution 确认移除会真正删除不可用 route connection，最终结果还须通过 `validate_model_config()`，因此不会返回部分或无效配置。公开 `migration.py` facade 保持兼容，内部拆为 inspection、Chat、Embedding、mapping、resolution 与 DTO 模块。
- **Legacy 模型迁移复审改为全 route 兼容证明**：`apply_shared_embedding_settings` 现在以私有、脱敏 capability 元数据校验最终保留和待加入的每个 Embedding Provider；固定空间 Ollama、能力未知远端或 endpoint 不可用时均失败关闭，不再只验证新 fallback。显式空白 endpoint、首尾空白与带非规范端口/路径的官方 DNS 末尾点别名会阻断，规范末尾点则先归一化；同一映射路径不再重复读取 credential，重复 credential/auth 诊断按完整语义复用稳定 issue，嵌套临时文本也不再进入 `repr`。
- **Legacy Embedding 借用配置保留源字段 provenance**：fallback-enabled 的 Embedding primary 借用 `llm.<provider>.api_key/base_url` 时，malformed issue 现在继续指向真实 Provider 字段，不再伪造 `llm.embedding.*` 决定；同一源问题同时影响 Chat fallback 与 Embedding 时只需一次确认，并原子移除两个无效 route 项。专属 Embedding 字段仍生成独立 issue，语义去重不会跨源字段合并。
