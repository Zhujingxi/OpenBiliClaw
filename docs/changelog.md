...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排基础（issue #56）**：确认收藏与稍后再看的统一路由契约——所有动作先写 OpenBiliClaw 本地；收藏同步到各平台收藏 / 书签 / Saved / 专用播放列表，稍后再看优先平台原生能力、缺失时降级到收藏；自动同步总开关默认关闭，本地收藏页与稍后看页保留显式手动同步和逐项结果。首阶段已落地 canonical `source_platform:content_id` 身份及 `saved_items` / `saved_memberships` / `native_save_states` 本地表、带 marker 的旧 `watch_later` / `favorites` 单次事务迁移、normalized DAO 与旧 Bilibili wrapper 兼容读写；新增平台无关 `NativeSaveAdapter` protocol / capability router 与 `SavedSyncService`，自动 / 手动任务共用 UUID 和持久化状态，批量任务通过原子 claim 避免重复创建窃取 active row，generic state upsert 禁止建立 active owner，所有状态写边界验证显式 state machine、completion 只接收 terminal，task-starter 普通异常或 cancellation 都会释放 owner；runner 以 task heartbeat 保护尚未逐项 claim 的 pending，正常 / 取消退出释放余项，崩溃后轮询或手动创建按 5 分钟 task lease 回收，因此长串行 batch 不会误窃取、部分 batch 也不会永久卡住。逐项执行另用 execution owner token 与 heartbeat；240 秒只作为调用方 response deadline：若 adapter 延迟 / 吞掉 cancellation，tracked watchdog 会继续 heartbeat 到真实 coroutine 结束，禁止陈旧恢复与重试产生重叠平台写入；延迟或同步吞取消的调用若返回合法 terminal 结果仍持久化真实结果，malformed target label / result 安全归一为 `failed/invalid_adapter_result`，只有取消 / 异常终止才使用预备的 timeout / interrupted。空白 task ID、adapter 非终态返回与全空白 item filter 均 fail closed，eligibility view 也排除已有 owner 的 pending；路由目标以 router 决议为准。本地状态首次创建改用 atomic insert-if-absent，重复 / 并发本地保存不会降级既有 active 或 terminal 状态。单项失败不回滚本地 membership，adapter 异常只落安全错误码而不暴露响应正文。跨平台同裸 ID 可并存，移除迁移项会同步清理旧表，native state 不允许脱离本地 membership 成为 orphan。生产平台 adapter、新的 platform-neutral HTTP API 与三端同步 UI 仍按「通用基础设施 + B站首适配」后续任务推进，其它平台在契约实测稳定后分计划接入。
- **原生保存自动同步配置契约（issue #56）**：新增默认关闭的 `[saved_sync].auto_sync_enabled = false`，支持 TOML 加载 / 保存、`GET/PUT /api/config` 和 `openbiliclaw config-show`；API 允许在部分更新中省略该段，但显式 `null`、字符串或数字都返回 422，只接受真实布尔值。该开关目前仅提供后续同步服务所需的配置边界，不代表平台 adapter、同步路由或桌面 / 移动 Web / 插件设置控件已完成。
