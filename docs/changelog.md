...
- 插件头部窄宽度对齐修复 + GitHub Star 引导：side panel 默认窄宽（<460px）下头部不再把操作图标挤到品牌下方、右浮成空一截的第二行；改为**始终单行**布局（品牌左、图标右、整体垂直居中），窄屏隐藏装饰性 eyebrow、状态徽标仅在空间不足时紧凑换到标题下、图标压到 30px，宽屏（≥460px）维持原样。并在 hero 下方新增**可关闭的「给个 GitHub Star」引导条**（金色渐变、★ 图标，点击开 repo、`localStorage` 记住关闭不再打扰），宽屏显示「去点 Star →」CTA、窄屏自动收起。用 chrome-devtools 在 360 / 430 / 560px 实测三档对齐；`tests/popup-layout.test.ts` 改判新单行布局并加 Star 引导断言。
