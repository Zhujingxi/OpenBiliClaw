...
- 新增 pool distribution snapshot 基础模型：`PoolDistributionSnapshot` 汇总候选池总量、平台族数量 / 缺口和 topic/style/franchise 饱和方向，并通过 `Database.get_pool_distribution_counts()` 复用 fresh、非 dislike、未推荐且可打开的候选统计口径；默认饱和阈值为 topic `pool_target_count // 20`、style `pool_target_count // 8`、franchise 10，且 `source_deficits` 明确保持为平台 / 来源缺口信号，不混入内容轴。
