...
- **小红书来源接入状态改用真实登录 Cookie 布尔态**：插件 `cookie-sync` 现在监控 `xiaohongshu.com` 的 `web_session`，只向 `/api/sources/xhs/login-state` 上报 `logged_in` 布尔值（不上传 cookie 值，`a1` / `webId` 仍视为游客设备 cookie）；后端把状态与时间戳存入 `auth_state`，`/api/sources/status` 在 72 小时窗口内以该登录态决定小红书 `ready/missing`，`xsec_token` 缓存仅作为 detail 提示，不再让新登录但尚无内容令牌的账号显示「需要登录」。
