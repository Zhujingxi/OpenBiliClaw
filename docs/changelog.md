...
- **阶段 9 复审补齐 probe、lifecycle、degraded 与 init 竞态边界**：exact probe 在 maintenance gate 准入后重查 init，并在短 model path lock 内把 `keep` credential 绑定到请求 revision；网络期间不持配置锁，完成后重读 revision/record/shared settings，stale A 不会借用 B secret、写入 B history 或关闭 B/live circuit。模型 API 改由显式 app lifecycle coordinator 执行「无事件发布完整 graph → 重启新 graph tasks → 清除 degraded → 单次 `config_reloaded`」；restart/swap 失败恢复旧字节、完整 normal/degraded runtime identity 与旧 graph task ownership，直接 `RuntimeContext.swap_model_candidate()` 兼容行为不变。guided-init `try_start` reservation 与模型 canonical precommit guard 现共享同一 path writer，消除 handler 初检后 init 与配置 commit 交错窗口。
