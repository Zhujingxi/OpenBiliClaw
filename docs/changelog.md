...
- **Eval reason 减肥,砍评估输出 token（eval-reason-diet）**：批量 / 单条内容评估 prompt 的 `reason` 契约改为——`score` 严格低于固定 `0.5` 的条目 `reason` 写空串 `""`（这类候选必低于所有 admission 门槛、会被直接丢弃,写理由是纯浪费输出 token,而输出 token 成本是缓存输入的 5–10 倍;典型 discovery batch 里低分候选占多数,预期省 30–50% 评估输出),其余 `score >= 0.5` 的条目写一句口语化、可直接展示给用户的中文,不超过 30 个字（可经 delight 兜底原样露脸）。0.5 是 baked 进两个 system prompt 的静态常量而非运行时 `admission_min_score`,既守 prompt-cache 前缀 byte-identical 铁律,又严格低于 0.60 默认 / 0.58 explore 全部准入路径;`test_prompt_builder_system_messages_are_call_invariant` 保持 system message 调用不变。解析零改动：`_evaluate_batch_once` 与单条路径本就把缺失 / 空 `reason` 归一为 `""`,eval 缓存不因空 `reason` 拒绝,delight 兜底链 `.strip()` 跳过空 `relevance_reason` 回落 topic / 通用文案（全链路补齐回归测试）。真实 provider 的 A/A 噪声包络 + A/B replay 质量门由发布前人工跑,门未过不合并。规格与实施计划见 `docs/plans/2026-07-18-eval-reason-diet-{spec,plan}.md`。
...
- **Discovery eval embedding 预过滤进入 shadow rollout**：`[discovery].eval_prefilter_mode` 新增 `off` / `shadow` / `enforce` 三档，默认 `shadow` 只记录 `prefilter-shadow` would-filter 候选并继续送 LLM；确认 would-filter 中几乎没有高于 admission 门槛的内容后可切 `enforce`，由本地 embedding 相似度提前缓存低分并跳过 LLM。`explore` 候选始终豁免，enforce 单批过滤超过 50% 时自动 fail-open。
- **LLM token diet 收口到评估 / 表达热路径**：`discovery.evaluate_batch` 坏输出不再整批退化成 N 次单条调用，而是在同一 worker 内二分拆批、递归子批 `<=5` 才逐条兜底，provider 限流 / cooldown / quota 仍原样传播不拆分；LLM caller bucket 补齐 `discovery.keyword*`、`discovery.x*`、`discovery.douyin*`、`runtime.bilibili_extension_search*`、`pool_purge*`、`api.sentiment*`，整条高频链路都能用 `[llm.discovery/evaluation/recommendation/soul]` 分层模型配置覆盖。内容评估画像改用 compact summary（20 核心 / 64 兴趣 / 32 域 × 12 specifics / 12 recent，长期避雷不裁剪），每条候选另带从画像块外长尾（权重 65..256 名）召回的最多 3 个 `related_interests` 兴趣名（画像未满 64 兴趣时零开销），digest 覆盖 compact 画像和召回池；`scripts/run_profile_diet_ab.py` 新增真实 DB A/B replay gate，按同日 A/A 噪声包络做相对质量门，本轮 compact + recall 的 divergence 指标均落在实测 A/A 噪声范围内。推荐表达 / legacy 分类共用同一个 `compact_content_prompt_profile_summary()` 收口，`interests=` 内容相关替换保留；discovery eval `body_text` 进入 prompt 前截断为头 200+尾 100，批量池文案同为 200+100。
