...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排 / API 基础（issue #56）**：所有保存 local-first；favorite 写平台收藏，watch-later 优先原生、缺失回退收藏，自动同步默认关闭。canonical identity、normalized 保存表、capability router、durable sync service、B 站 adapter、平台中立 `/api/saved/*` 与 task polling 已落地；`RuntimeContext` 在热重载时把当前 B 站 client 原子注册到新 service，后台 sync 统一纳入 `BackgroundTaskRegistry`。严格 API 边界拒绝非法 list kind、identity/key/UUID/selection，缺 membership 返回逐项 `failed/not_saved_locally`；自动同步立即返回 pending，手动同步无视开关，轮询保留 `login_required/rate_limited/failed`，旧 B 站端点继续 local-only 并增量返回 identity/sync 字段。task 由 runner / task heartbeat / execution / item heartbeat 分层 fence；detached watchdog 继续保护 late completion，平台失败不回滚本地。B 站 adapter 使用 `SESSDATA + bili_jct` 前置校验、application-aware 正整数 aid、实例内 exact-title folder 单飞和 operation-specific 脱敏 code；四端 UI 仍属后续任务。
