...
- Landed Phase D personalized feeds: Bilibili's cookie-authenticated `rcmd` API is now a declared `platform-personalized` channel and joins replenishment as exploit-only supply; anonymous connections never request it. Provider feed acquisition round-robins available channels, candidates retain `bilibili:rcmd` provenance without exploration attribution, and accepted feedback appends attempts plus satisfaction/correction outcomes to a standing `channel-bilibili-rcmd` hypothesis so per-user yield uses the existing Thompson/Beta journal. Channel-level posterior allocation and other providers' personalized feeds remain deferred.

- Landed Phase D plugin-assisted access: Bilibili now declares a frozen cookie recipe (`SESSDATA` + `bili_jct`) in its provider manifest; authenticated recipe/material API routes let the generic extension request only recipe-derived site permissions, collect declared cookie/storage artifacts, and submit them through the existing manual form, verifier, and credential vault. Credential slots are provider/account-scoped and Composition idempotently rehydrates/re-verifies default-account connections after restart. Browser-executed fetch/signing, refresh scheduling, and multi-account remain deferred.

- Landed Phase C Assistant correction hardening: the model-visible `propose_profile_revision` tool can create only deterministic, scoped, expiring pending actions for an existing claim or `exploration.disabled`; it has no direct profile mutation capability. Approval through the existing pending-action confirmation path invokes canonical `EditProfile`, projects normal SET corrections as same-kind trust-1.0 user-statement claims with the corrected value, and persists/indexes statement evidence plus accepted claims through the shared Understanding/C2 hooks. REMOVE stays removal-only, exploration approval activates the sole exploit-only override, and `POST /v1/content/actions/reject` durably rejects pending/expired proposals without touching the profile or evidence ledger. Completed confirmation replay remains valid after expiry.

- Landed Phase C semantic retrieval: schema V10 adds a model-specific float32 embedding index for evidence summaries, accepted profile claims, and candidate metadata. Text hashes avoid unchanged recomputation; stale-model rows are ignored; bounded pure-Python cosine recall fails open when embeddings are absent or unavailable. `RecommendationProfile` v2 exposes opaque semantic claim views, and the new adjacent arm recalls candidates close to weak/emerging claims while excluding established-interest matches. LLM reranking and ANN remain deferred.

- Landed Phase C event-triggered profile re-synthesis: explicit corrections, contradictory stable-interest/avoidance evidence, and the existing 180-day evidence-staleness boundary target only affected claims through the canonical proposal/evidence/decision transaction. Each invocation is capped at 25 claims and retained claim evidence at the latest 64 IDs; stale links are excluded from replacement proposals. Trust 1.0 is reserved for high-trust preference/profile-edit statements while authenticated behavior projects to at most 0.6, so behavioral inference cannot overrule a statement. Contradictions recompute confidence, drift refreshes or retires, every result is ledger-audited, and best-effort post-commit failures never misreport the durable source operation as failed. Trigger evaluation remains in-process, never a scheduled full-profile rebuild.

- Landed Phase B evidence discipline: server-resolved exploration arm/hypothesis provenance is copied into immutable feedback observations; viewport exposure gates dismissal failures while attempts remain auditable; exploration likes/saves create durable confidence-0.2 Understanding proposals that stay pending behind the existing confidence gate. Only the explicit `exploration.disabled=true` profile statement bypasses allocation for exploit-only episodes, removable through the profile API/CLI; passive feedback cannot set it. Web cards report viewport exposure with `IntersectionObserver`, and seeded uniform priors retain cold-start exploration.

- Added Phase B RecommendationBrief in shadow mode: the configured `recommendation.brief` model route proposes strict expiring intent, evidence-cited hypotheses, retrieval/inspection plans, qualitative slate relationships, ask/abstain/recommend action, and stop conditions. A deterministic compiler rejects undeclared or personalized-exploration channels, over-budget inspection, expiry errors, and inconsistent questions (percentage guidance is unrepresentable at schema level); each attempt journals typed output, diagnostics, compiler inputs, context hash, and agent/model versions. Failures no-op and live replenishment remains pinned to the existing uncertain-intent allocator until shadow evidence supports a later switch.
...
- Implemented Weibo anonymous public search behind the existing provider contract. The transport obtains a generated visitor `SUB` through Weibo's visitor passport flow, keeps it only in memory, and never accepts or replays user cookies. Live verification drove three corrections to the ported design: anonymous keyword search moved from the dead `100103type=1` container to `100103type=64` (hot sort); upstream randomly soft-blocks visitor cookies (ok=0 "这里还没有内容", or ok=1 with total=0 for queries that have results), so the transport re-bootstraps a fresh visitor cookie with a 1s decorrelation delay, bounded at six attempts; and FETCH is not advertised — every anonymous detail-by-id endpoint (statuses/show, statuses/extend, 230413 container, ajax show) is login-walled or unreliable as of 2026-08, so search previews carry full title/body/author/timestamp and canonical `https://weibo.com/status/<bid>` links instead. Added hermetic visitor/transport/provider tests, production composition wiring, anonymous config enablement, and the opt-in `l1weibo` layer.

- Implemented the Linux.do anonymous provider behind the existing contract: real Discourse JSON transport (`/search.json` topics-as-items, `/t/<id>.json` fetch with `post_stream.posts[0]` body extraction), allowlist normalization, cooked-HTML text extraction, typed error mapping, canonical `https://linux.do/t/topic/<id>` identity, and production composition wiring. **Not enabled and no live e2e layer**: linux.do Cloudflare-challenges every content-bearing endpoint for non-browser clients — verified 2026-08-14 with default and browser User-Agents from a residential IP (403 "Just a moment…"; only `/site.json`-class metadata passes). The transport stays wired for networks/cookie paths where the challenge does not apply (manual-cookie upgrade form retained); enablement waits for a viable path.

- Enabled V2EX public reads through the existing official `www.v2ex.com/api` transport and global `builtin.anonymous` access path, with no user token required. Added an opt-in `l1v2ex` real-stack layer covering the hot feed at the UI's default page size and dynamic topic-detail fetch; enabled V2EX in the E2E and Docker profiles. V2EX has no official full-text search API: the existing SEARCH path only filtered `/api/topics/hot.json` client-side, so SEARCH is no longer advertised; official feed, fetch, creator, and projection capabilities remain.
...
- Completed L7 agent-driven browser testing against the live Vue/FastAPI stack. Web recommendation cards now submit server-authoritative `liked`/`dismissed` feedback with stable shown IDs and visible expired-delivery failures; SPA HTML is revalidated after rebuilds while fingerprinted assets remain immutable. The L7 pass also repaired detached native `fetch` calls, browser JSON source-permission validation, invalid Assistant conversation IDs, and untyped Assistant model failures.

- Completed L6 real Docker deployment testing. Compose now runs `BAAI/bge-small-zh-v1.5` in a healthchecked Infinity Torch/server sidecar while the application image remains model-free; runtime-mounted model keys and generated API bearers are stored in the vault and only opaque references enter persistent config. Fixed three real deployment blockers found by build/boot/flow/restart tests: Infinity must disable its broken optional BetterTransformer path, non-loopback Docker binding had no vault-backed bearer configuration, and `/v1/sources/connect` replaced provider-owned form field IDs with an unusable generic credential field. The containerized anonymous refill/feed/feedback loop, in-container check, authenticated SPA serving, feedback/profile/feed persistence, isolated volume teardown, and explicit client-resubmit-after-restart access behavior are verified.

- Completed L5 real-stack Application testing through a live `openbiliclaw serve` process and `/v1` HTTP only. Recommendation feed delivery now atomically marks selected candidates shown and returns a stable `shown_id`; feedback rejects unknown/mismatched deliveries, transitions shown candidates to interacted, and preserves idempotent observation learning. The live loop verifies typed errors, unauthenticated loopback reads, supervised refill, ranked/reasoned feed, feedback, content-linked preference processing through the scheduled real-Kimi analyzer, and feed/profile persistence across server restart.

- Completed L4 real-stack recommendation testing over live Bilibili refill and the accumulated default profile. Content preferences now feed discovery/recommendation projections, planned topics survive into candidates, profile avoidances reach prefilter and selection, refill honors `maximum_items`, repeated deterministic candidates are observable successful no-ops instead of invalid state retransitions, host job health comes from the runtime supervisor, and the public model-free feed joins durable expression reasons while preserving rank order and named score contributions. The current baseline remains honestly non-personalized: profile influence shapes queries/topics and hard exclusions, not model scoring. Recommendation discovery is text-query based, so no speculative semantic index/table was added.
- Completed L3 real-stack understanding testing. Composition now constructs and exposes the configured native `EmbeddingService`; BGE small Chinese queries receive the model-card retrieval instruction while documents remain unprefixed. Real Bilibili titles passed a 512-dimensional semantic-ranking smoke. The original native output-tool attempt failed because Kimi's default thinking conflicts with forced `tool_choice = "required"` (not because Kimi lacks tool calls), and the original `ProposalBatch` asked the model to generate deterministic hashes/timestamps it cannot compute; the routed preference analyzer now uses PydanticAI prompted, model-friendly drafts that the application converts into canonical proposals with verified evidence references. Real profile derivation, restart persistence, incremental processing, inspection, and correction passed. Durable semantic-index ownership is deferred to the L4 Recommendation consumer rather than invented in Understanding. Live Bilibili search rows without a stable BVID are now skipped instead of invalidating an otherwise usable page.
- Completed L2 real-stack observation testing against live Bilibili content and the persistent E2E database. The production facade now has verified coverage for content-reference landing/deduplication, view/like/feedback idempotency, restart durability, deterministic cursor replay, and authenticated watch-history imports. Observation ingress persists content identity only; provider projection bodies remain outside `content_cache`.
- Added authenticated Bilibili real-stack E2E coverage using memory-only Chrome cookie extraction submitted through the existing manual form, vault, and live verifier path. `ConnectSource` now replays a durable result only when the matching live handle still exists, preventing false connected state after restart; native nav/history/related/creator response and query shapes are normalized, authenticated handles can also use public reads, and response-contract failures no longer masquerade as network failures. Access remains process-local and requires client resubmission after restart.
- Completed anonymous Bilibili real-stack E2E acquisition through production Composition. The endpoint adapter now maps live popular/search/detail envelopes and native fields into strict provider models, sends browser-compatible public headers, normalizes markup/duration/cover URLs, exposes a dead-host test seam, and preserves typed HTTP 412/429/network failures. Search/detail remain read-only; observation ingress owns durable content-reference persistence.
...
- **目标 Presentation Contract Phase A 落地（尚未替换 production shells）**：新增 `frontend/` npm workspace 与四个预声明 package，启用 Vue 3 / Pinia / Vite / Vitest / strict TypeScript / ESLint / Prettier；deterministic OpenAPI 通过 workspace 固定的 `openapi-typescript` 生成 typed client contract，HTTP/SSE client 在 unknown JSON 边界验证并输出安全 typed errors。Presentation package 复用 generated API types，提供 provider/card/action/page/availability contracts 与 video/image/article/discussion/fallback Vue cards；共享 shell 包含 accessibility semantics，URL/media 仅允许 HTTP(S)，backend HTML/CSS/component names/code 不可执行。Web views 与 extension shell/legacy JS deletion 明确等待 Phase 14b/14c。

- **目标 API / CLI Hosts 落地（尚未接入生产组合根）**：新增 `hosts/api` clean `/v1` FastAPI factory、七组 capability routers、strict transport schemas、consistent safe error envelope、typed bounded SSE/WebSocket replay，以及 bind/auth/origin/CSRF/device/body/timeout/rate/subscriber security policy；新增共享同一 `HostFacade` 的 Typer `start/status/config-diagnostics/model-diagnostics/connect/profile/recommend` commands 与 deterministic OpenAPI export script/snapshot。批准 deviation：frontend generated client 等待 Plan 14；legacy `api/app.py`、`api/models.py`、source-auth routes、monolithic `cli.py` 与 production cutover 等待 Plan 15；当前无双路 production serving。

- **目标 Assistant phases 1–5 + observation filter 落地（尚未接入生产组合根）**：新增稳定 `assistant.dialogue` PydanticAI agent、四种 typed output、仅含 Application facade + bounded `DialogueProfile` 的 secret-free dependencies、interactive RunPolicy、八个 native workflow tool contract 与 intent/provider/skill scoped selection、bounded/sanitized tool results、typed `AssistantSkill` registry、local-user/device scoped conversation SQLite persistence、retention/delete/restart、fact-preserving history compaction，以及 exact-effect/expiry pending-action confirmation。普通 assistant message 不产生 observation，只有 explicit preference/feedback/confirmed edit/defined outcome 可进入 typed filter。按批准 deviation，Phase 6 `soul/dialogue.py`、dialogue scheduler、`integrations/agent.py`、旧 orchestrator/skill/fake-tool 删除等待 Plan 15 caller cutover；当前无 production caller、compatibility facade 或双写。

- **目标 Application Workflows phases 1–5 落地（尚未接入生产组合根）**：新增显式 frozen typed dependency facade、model-free source/recommendation/profile/search/detail/job-health reads，以及 connect/disconnect、observation import、feedback、profile edit 和 bounded refresh admission workflows；mutation 强制 idempotency/audit fields，feedback+observation 与 override+audit observation 通过 unit-of-work contract 声明同一 transaction。新增 scoped/expiring `PendingAction` proposal+confirmation，执行时重验 user/account、access 与 content ref，且不保存 credential 或 model executable payload。route/command/tool→workflow matrix 见 `docs/modules/application.md`。按批准 deviation，Phase 6 API/CLI/runtime/Assistant/extension consumer cutover 与旧 sequencing 删除等待 Plans 13/15；当前无 production caller、route、CLI、extension message、compatibility facade 或双写。

- **目标 User Understanding 落地（尚未接入生产组合根）**：新增 frozen canonical profile（stable/emerging interest、avoidance、content/style/creator/language/provider preference、insight）、deterministic claim identity、confidence/freshness/evidence/lifecycle、不可被 inference 覆盖的 explicit override，以及 accepted/rejected/superseded audit ledger。四种稳定 PydanticAI analyzer contract 使用 structured ProposalBatch、明确 capability 与 RunPolicy；bounded observation projection 不含 credential/provider payload。Proposal 先于 decision/profile 写入，evidence/profile/ledger/per-analyzer checkpoint 在单 SQLite transaction 内提交，retry/restart replay idempotent；三种 versioned consumer projection 有字符预算。按批准 deviation，legacy `memory/`/`soul/`/SoulEngine/MemoryManager 删除等待 Plans 11/15 caller cutover，persona/judge eval migration 等待 Plans 12/15；当前无 production caller、compatibility facade 或双写。

- **目标 Observation Ingress 落地（尚未接入生产组合根）**：新增 12 种 immutable Pydantic discriminated observations、producer/source/trust/account/time/content validation、batch/payload bounds、typed inserted/duplicate/rejected receipts、SQLite transaction + `(producer,idempotency_key)` uniqueness、insertion-cursor replay 和 committed-ID-only EventPublisher notification。Future ObservationProvider 仅通过 Core typed registration，浏览器观察限定为未来 signed/device-authenticated producer contract；不包含 cookie、tracker、browser session 或 browser-specific payload。按批准 deviation，built-in producer wiring 等待 Plans 09/11，legacy `runtime/event_ingress.py` 与 scattered writes 删除等待 Plan 15；当前无 production caller 或双写。

- **目标 Douyin / RedNote session-fragile provider schemas 落地（尚未接入生产组合根）**：新增 self-contained `content/providers/douyin/` 与 `content/providers/rednote/`，保留 strict native schemas、canonical identities、purpose projections 和 presentation descriptors。Douyin 的 direct-cookie search/fetch/creator 依赖 session-bound msToken/X-Bogus/risk-control 状态，RedNote 搜索、creator 与画像 bootstrap 也依赖页面 session/动态签名/extension task，因此两个 manifest 均仅声明 projection。两者都标记 degraded，不接受不可稳定重放的手工 Cookie、不实现 browser automation、Cookie extraction、extension task dispatch 或 mutation tools。按批准 deviation，legacy `sources/douyin_*`、`dy_tasks.py`、`sources/xhs_*`、`xiaohongshu_adapter.py` 与 production callers 等待 Plans 10–15 cutover 后删除；当前无 production caller、compatibility facade 或双写。

- **目标 YouTube / Bangumi / V2EX public/API providers 落地（尚未接入生产组合根）**：新增三个 self-contained `content/providers/` package 与 concrete scoped HTTP transport：YouTube 调用匿名 InnerTube search/browse/player，Bangumi 调用官方 v0 search/subjects，V2EX 调用官方 public topic API；response 在 transport boundary 进入 strict native page，status/network/schema 错误安全分类。统一提供已实现的 anonymous search/feed/fetch（YouTube/V2EX 另有 creator）、opaque cursor、50-item hard bound、canonical provider ID/URL、aware published timestamp 与 purpose-specific projections。YouTube 保留 typed Google Takeout directory/zip import（默认 HTML 或 JSON watch history、subscription、likes）；Bangumi/V2EX 声明 manual PAT form + provider-owned live verifier，但 manifest 不宣称尚未接线的私有 capability。旧 YouTube `scrapetube`/`yt-dlp` fallback stack 不复制到目标包：typed InnerTube transport 直接承担目标 read capabilities；legacy implementation 仅留到 Plan 15 caller cutover 后删除。按批准 deviation，legacy `youtube/`、`sources/{bangumi,v2ex}*` 与 producers 删除等待 Plans 10–15 caller cutover；当前无 production caller、compatibility adapter 或双写。

- **目标 Bilibili reference provider 落地（尚未接入生产组合根）**：新增 self-contained `content/providers/bilibili/`，外部 response 在 HTTP boundary 进入 strict video/article/native envelope；实现 anonymous search/feed/fetch/related/creator 与 manual-cookie history/saved/save action，opaque cursor、50-item hard bound、CSRF、idempotency、purpose projections/card descriptor、安全 observation 和 Integration native tools。Provider-owned `builtin.manual` form 要求 exact `SESSDATA` + `bili_jct`，CredentialVault secret 只在 trusted authorization callback 可见；rendered/personalized browser-session mode 明确 unavailable。按批准 deviation，legacy `bilibili/`、`sources/`、browser task/producers 删除等待 Plans 10–15 caller cutover，其他 providers 后续分批落地；当前没有 production caller、compatibility adapter 或双写。
- **目标 Content Integration contracts 落地（尚未接入生产组合根）**：新增 frozen provider/content value objects、要求正 schema version 和 provider-owned typed payload 的 `NativeContent` envelope、彼此独立且带 source timestamp/provenance 的 preview/recommendation/search/card projections、十种 narrow read/mutation/projection/observation capability protocols、opaque provider cursor、显式 idempotency/confirmation action contracts、immutable manifest 与 duplicate/capability-safe registry。PydanticAI search/fetch tools 直接复用 deterministic capability methods，并在 model history 前执行 result budget；mutation tool 只返回 `PendingActionDescriptor`，不执行 provider mutation。AST gate 禁止 Content Integration 导入 concrete providers 或 Understanding/Recommendation/Assistant/Hosts。按批准 deviation，Phase 6 legacy `sources/protocol.py`/registry/dispatcher 替换和删除等待 Plan 07 first-party providers 与 Plan 15 composition cutover；当前无 production caller、compatibility adapter、route/config/frontend 变化或双写。
- **目标 Provider Access boundary 落地（尚未接入生产组合根）**：新增 secret-free frozen access request/descriptor、anonymous/credential discriminated opaque handles、六态安全 status 与 typed verification evidence；anonymous method 只允许 public read 且不虚构 identity，manual method 使用 provider-owned declarative forms/verifiers，把提交值直接写 CredentialVault 并只保留 opaque reference；typed broker 同时约束 caller support、用户 allowlist、provider support 与 permission narrowing，AccessService 提供串行 connect/status/replace/disconnect、bounded verification cache、replace cache invalidation 与 disconnect revoke。AST gate 禁止 Access 导入 product/host 模块并禁止 model-visible package 导入 vault。按批准 deviation，Phase 6 `api/source_auth/`/`auth_core.py` host cutover、legacy config credential reads 删除及 durable status repository 等待 Plans 13/15；browser-extension session、managed-browser 与 OAuth 仅由 `AccessMethod` extension contract 指定，不在本阶段实现。当前无 route、TOML、frontend 或 production graph 变化，也无双写。
- **目标 Model/Embedding Provider plugins 落地（尚未接入生产组合根）**：新增 frozen 非秘密 `ModelInstanceConfig`、CredentialVault callback-scoped PydanticAI native OpenAI/Anthropic/Google/Ollama/OpenRouter 构造、明确 unsupported 的 DashScope chat、稳定 instance/config fingerprint、declared-vs-verified structured-output/tools/vision/streaming capability records 与 opt-in integration probes、封闭安全 construction diagnostics，以及独立于 chat 的 OpenAI/Google/Ollama typed embedding transports、完整 provenance/cache identity、确定性 batch、Core ResourceBudget 并发、retry/timeout/cancellation 和 vector count/dimension validation。默认依赖改由 pinned PydanticAI extras 管理 OpenAI/Google SDK，Anthropic 因 1.56 compatibility 固定 `<0.80`。按批准 deviation，production route wiring、startup probe enforcement、CLI/API diagnostics 和 legacy `llm/` 删除等待 Plans 09–15；当前没有新 TOML shape、production caller 或双路执行。
- **目标 AI Runtime 原语落地（尚未接入生产组合根）**：新增 pinned PydanticAI typed execution boundary、稳定 `AgentId`、startup-validated capability route/fallback、Core `ResourceBudget` 并发边界、native request/token/tool/timeout/retry limits、四类 run priority、安全 typed error normalization、bounded history/context/tool result 与完整 message secret-canary audit、全维 usage attribution persistence port，以及只消费 recorded inputs 的 typed offline dataset/runner/report mechanics。正常测试全局 `ALLOW_MODEL_REQUESTS=False`。按批准 deviation，Phase 5 domain-agent conversion、旧 `eval/` 内容迁移与 Phase 7 `llm/`/`eval/`/orchestrator/skill 删除等待 Plans 09–13 owner modules/hosts 落地；当前生产 model calls 仍只走 legacy `llm/`，没有 compatibility facade、双路执行或新配置 shape。
- **目标 Infrastructure 原语落地（尚未接入生产组合根）**：新增规范化 target schema 与 atomic migration runner、破坏性变更前 verified timestamped SQLite/config backup、WAL/FK/busy-timeout `SqliteDatabase` 与显式 transaction-only `SqliteSession`、opaque `CredentialVault`（OS keyring + permission-checked file fallback）、scoped/retry-safe HTTP factory、bounded atomic files、typed post-commit events 和 mandatory-redaction telemetry；未版本化用户库会停止并要求明确 reset/import 决定。按批准的 scope adjustment，本阶段不机械拆分 18k 行/479 method 的 `storage/database.py`：per-aggregate repositories 与 god object/旧 migration helper 删除推迟到 Plans 05–12 的 owner protocols/domain models 落地后执行；当前无生产接线、compatibility wrapper 或双写。`events/websocket.py` host delivery adapter 明确推迟到 Plan 13，在 API host wiring 时实现，避免本阶段引入未接线的 transport。
...
- Phase 14 presentation cutover adds Vue web/extension shells, typed shared contracts, generated frontend packaging, and removes handwritten JavaScript; the reduced extension no longer handles cookies or provider tasks.
