...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排基础（issue #56）**：确认收藏与稍后再看的统一路由契约——所有动作先写 OpenBiliClaw 本地；收藏同步到各平台收藏 / 书签 / Saved / 专用播放列表，稍后再看优先平台原生能力、缺失时降级到收藏；自动同步总开关默认关闭，本地收藏页与稍后看页保留显式手动同步和逐项结果。首阶段已落地 canonical `source_platform:content_id` 身份及 `saved_items` / `saved_memberships` / `native_save_states` 本地表、带 marker 的旧 `watch_later` / `favorites` 单次事务迁移、normalized DAO 与旧 Bilibili wrapper 兼容读写；新增平台无关 `NativeSaveAdapter` protocol / capability router 与 `SavedSyncService`，自动 / 手动任务共用 UUID 和持久化状态，按平台分组、平台内串行执行，单项失败不回滚本地 membership，adapter 异常只落安全错误码而不暴露响应正文。跨平台同裸 ID 可并存，移除迁移项会同步清理旧表，native state 不允许脱离本地 membership 成为 orphan。生产平台 adapter、新的 platform-neutral HTTP API 与三端同步 UI 仍按「通用基础设施 + B站首适配」后续任务推进，其它平台在契约实测稳定后分计划接入。
