...
- **Node .mjs 脚本 @ts-check JSDoc 迁移（Phase F）**：所有 `extension/scripts/*.mjs`、`tests/js/*.test.mjs`、`tests/web_model_config_state.test.mjs` 加上 `// @ts-check` 注释并补全 JSDoc 类型注解。`extension/scripts/build.mjs` 新增 `@ts-check`，其 JSDoc 类型已随现有 TypeScript 声明自动推断。`tests/js/` 中 5 个 test helper 文件同步加上 `// @ts-check` 入口。新增 `scripts/tsconfig.scripts.json`（`checkJs: true`、`strict: true`）供显式类型检查。`node --check` 通过全部 18 个 .mjs 文件；`node --test tests/js/` 57 passed；`npm run build:web` + `npm run build:bundle` 编译通过。没有 .ts 文件重命名或 JS→TS 转换。
