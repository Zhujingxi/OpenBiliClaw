...
- **源码安装事务具备跨进程唯一所有权**：local prepare/start 现在由独立且限时的 lifecycle file lock 串行，锁覆盖 verified stop、migration、API/worker launch、process-state publication、doctor 与 protected readiness；`SKIP_START=1` 也先验证停止旧 managed pair，migration 不再与运行中实例交错。安装器以 `0600` 持久化 root-bound UUID，并把 runtime state 绑定 canonical checkout root、instance ID 与单调 generation；复制/移动/篡改状态会失败关闭。失败清理使用 generation compare-and-swap，只能移除本轮仍拥有的 state，并发第二次启动会等待前次完成后验证停止旧 pair，最终只保留并追踪最新 API/worker。
- **Docker 安装成功条件覆盖真实 worker**：两份 Compose 为 worker 增加运行中 PID 1、Alembic head、Huey SQLite 完整性和可回滚写事务联合 healthcheck；installer 解析 Compose JSON 状态，只有一次性 migration 以 0 退出且 API/worker 均为 `running/healthy`、受保护 API 可访问才报告成功，worker crash-loop 不再被 API readiness 掩盖。Docker `SKIP_START=1` 现在也先执行隔离的 `compose run --rm migrate`，prepared 结果不再虚称已迁移。
- **vNext SQLite backup 关闭 source TOCTOU 且保留 WAL commit**：backup 不再在 `lstat` 后按原 pathname 重开数据库；main 先由 `O_NOFOLLOW` FD 固定并以 device/inode 验证，再把 main 与存在的 WAL/SHM/journal 以 hard-link 组成随机 `0700` 私有 source set。SQLite online backup 前后复核原 source/sidecar presence 与 inode，写入只落在持有的 `0600` 目标 FD，最终 no-overwrite 发布；路径换成 symlink/不同 inode、晚到或替换 sidecar 都失败关闭，未 checkpoint 的已提交 WAL row 有真实回归。
