...
- **模型路由保存获得独立事务边界（阶段 7）**：新增 `ModelConfigService` 的脱敏 snapshot、revision guard、stable-ID 增删改移、exact draft probe 与 `keep/set/clear/env` credential action；首次 legacy 保存先建立不覆盖、保留 mode 的 pre-refactor backup，再用同目录 temp、文件/目录 fsync 与 `os.replace` 原子落盘，最后短锁交换预构造的 `RuntimeContext` candidate。写入、swap 或 swap 期间取消会恢复旧字节、mode 与精确 runtime 身份并保留取消语义；`config.local.toml` shadow path 保持只读且不烘焙。model-only source editor 用 `tomllib` 识别并去重 `[models]` / `[llm]` authority，未知 table、边界注释、空白与 CRLF 原字节保留，inline/dotted 歧义失败关闭。production consumer、API 与 UI composition 继续由 Task 8 接入。
