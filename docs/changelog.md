...
- **原生保存自动同步配置契约（issue #56）**：新增默认关闭的 `[saved_sync].auto_sync_enabled = false`，支持 TOML 加载 / 保存、`GET/PUT /api/config` 和 `openbiliclaw config-show`；API 只接受真实布尔值，字符串或数字返回 422。该开关目前仅提供后续同步服务所需的配置边界，不代表平台 adapter、同步路由或桌面 / 移动 Web / 插件设置控件已完成。
