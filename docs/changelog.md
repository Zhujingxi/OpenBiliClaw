...
- **新增 evaluator `json-minify` 单变量真实回放臂**：生产默认继续使用既有 pretty JSON；replay B 仅把 batch evaluator 的 profile layers、negative examples 与 content batch 改为 `ensure_ascii=False + sort_keys=True + separators=(",", ":")`，字段、值、数组顺序、system prompt、reason/output schema、batch size 和 runtime 语义不变。新的实例级 seam 不修改 module global，并以隐私安全摘要/长度证明 A/B JSON 语义相同、A 为精确 pretty、B 为精确 compact 且传输更短；artifact v3 逐 logical run 记录 prompt/completion/total/cached/uncached token、供应商 cache 口径、profile-layer cache、图片输入摘要、分类 agreement/fill 与 root/member-repair 归因。原始内容/图片 ID 改用进程级随机盐摘要，避免产物成为可复用查表。首轮真实回放发现 OpenAI-compatible 适配器在 `json_object` 空内容后会执行成功的无格式约束重试，而旧 harness 只看最终响应、漏记前一次已消耗 usage；该不完整回放已主动中止且不作证据。现在每个成功 wire attempt 都在同一 ContextVar logical call 下独立记账并累加，任何 attempt 缺 usage 都 fail closed。除原质量/route/embedding/recall 门外，B 还必须至少减少 `10%` prompt token、`8%` total token，不能出现任何缺 usage/error call、cache metric 缺失或超过 A/A 噪声的 cache/repair 回归，才允许改生产默认。
