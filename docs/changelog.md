...
- **真实质量门连续校准 compact 边界，最终保留 96 / 16**：64 兴趣 / 每域 12 specifics 的首轮严格 100×3 replay 让 treatment Spearman 中位数降至 `0.494686`（control floor `0.570454`）、admission delta 中位数为 `-0.09`（floor `-0.07`），因此先保守回调到 80 / 16；该边界曾在 `11f77a64` 的冻结画像上通过，但最终 clean-commit 回放时 effective profile 已从不超过 80 项增长到 87 项，80 / 16 又出现 treatment flip-rate 中位数 `15%`（ceiling `7%`）和 Spearman 中位数 `0.356586`（floor `0.564777`）失败。gate 未放宽；实现改为 96 兴趣 / 每域 16 specifics，长尾 recall 从第 97 项开始，完整保留当前 87 项兴趣。该边界在当前画像保留 `98.29%` 序列化字符，在 mature fixture 仍缩短 `53.58%`、极限 fixture 的实际分层 prompt 仍缩短 `61.17%`；它是已验证的 80 / 96 / 128 候选中继续满足既有“极限画像至少缩短 60%”测试的最大边界。失败 artifact `data/eval/profile-diet-compact-failed-397fe03e.json`（SHA-256 `d58bb688…dadcdc`）只保留诊断用途；compact / reason-diet 从新的 clean commit 重跑。
...
- **Discovery eval embedding 预过滤进入 shadow rollout**：`[discovery].eval_prefilter_mode` 新增 `off` / `shadow` / `enforce` 三档，默认 `shadow` 只记录 `prefilter-shadow` would-filter 候选并继续送 LLM；确认 would-filter 中几乎没有高于 admission 门槛的内容后可切 `enforce`，由本地 embedding 相似度提前缓存低分并跳过 LLM。`explore` 候选始终豁免，enforce 单批过滤超过 50% 时自动 fail-open。
- **LLM token diet 收口到评估 / 表达热路径**：`discovery.evaluate_batch` 坏输出不再整批退化成 N 次单条调用；当前 landing 实现只对成功响应里的缺失 / malformed member 做有界 subset/split retry（最多深度 3、额外 6 请求），provider 异常原样传播。LLM caller bucket 补齐 `discovery.keyword*`、`discovery.x*`、`discovery.douyin*`、`runtime.bilibili_extension_search*`、`pool_purge*`、`api.sentiment*`，整条高频链路都能用 `[llm.discovery/evaluation/recommendation/soul]` 分层模型配置覆盖。内容评估画像最终使用 compact summary（20 核心 / 96 兴趣 / 32 域 × 16 specifics / 12 recent，长期避雷不裁剪），每条候选另带从画像块外长尾（权重 97..256 名）召回的最多 3 个 `related_interests` 兴趣名（画像不超过 96 兴趣时零开销），digest 覆盖 compact 画像和精确召回池。此版本最初记录的 replay divergence PASS 后续因 source-context 污染、独立快照和非生产预算被判无效；landing hardening 的严格 64 / 12 replay 未通过质量门，80 / 16 虽在一次冻结画像上通过、但在画像增长到 87 兴趣后重验再次失败，故门槛不变并以 96 / 16 最终验收。推荐表达 / legacy 分类共用同一个 `compact_content_prompt_profile_summary()` 收口，`interests=` 内容相关替换保留；原定正文 200+100 截断被后续严格 Reddit 100×3 gate 否决并完整回滚，discovery 与 recommendation 均保留完整 `body_text`。
