...
- **Safari 插件接入 `extension-v*` 发版链**：`release-extension.yml` 新增 macOS 构建腿，推 `extension-v*` tag 时在 `macos-14` 上执行 `npm run package:safari -- --notarize`（build → convert → xcodebuild Developer ID 签名 → notarytool 公证 → stapler staple → 打 `openbiliclaw-extension-v*-safari.dmg`），与 Chrome/Firefox 资产一起上传 `extension-v*` 与 `openbiliclaw-v*` 聚合 Release；`verify-release-completeness.yml` 在 `SAFARI_SIGNING_ENABLED != 'false'` 时把 Safari DMG 列为发布完整性门禁，`sync-aggregate-release.sh` 同步展示 Safari 下载行与资产。新增 `extension/scripts/package-safari.mjs`（本地可用 `npm run package:safari` 产 ad-hoc 未签名 DMG/ZIP，`--notarize` 走签名+公证），所需 secrets：`APPLE_DEVELOPER_ID_CERTIFICATE_BASE64` / `APPLE_DEVELOPER_ID_CERTIFICATE_PASSWORD` / `APPLE_TEAM_ID` / `APPLE_NOTARY_USER` / `APPLE_NOTARY_PASSWORD`。注意：Safari 签名默认开启；未配置 Apple 凭据时 `extension-v*` 会 fail-closed，只有显式设置仓库变量 `SAFARI_SIGNING_ENABLED=false` 才跳过 Safari 资产。
