...
- **Issue #113 首次初始化空库存准入解环**：阶段 2/3 的 `soul.*` 调用在 canonical durable inventory 为空时原本会按 maintenance 停在后台 admission，而建立首池又依赖画像先完成。guided init 现在只在这两个异步任务的 task-local scope 内绕过后台 admission；总 provider gate 仍生效，并行 discovery 与普通后台任务不继承该 scope，现有 Soul / Analyzer / ProfileBuilder 公开 API 保持不变。
