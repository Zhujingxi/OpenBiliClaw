...
- **源码安装事务具备跨进程唯一所有权**：local prepare/start 现在由独立且限时的 lifecycle file lock 串行，锁覆盖 verified stop、migration、API/worker launch、process-state publication、doctor 与 protected readiness；`SKIP_START=1` 也先验证停止旧 managed pair，migration 不再与运行中实例交错。Installer metadata 在首次 lifecycle 前持久绑定唯一 lock UUID/device/inode；POSIX 通过 held parent dir FD，native Windows 通过无 `dir_fd` 的 direct-path 分支，在临界区前后复核 metadata 与 pathname。绑定后 pathname 缺失或换 inode、复制 lock inode，以及 symlink/junction ancestor 都失败关闭，不能产生第二 holder。安装器以 `0600` 持久化 root-bound UUID，并把 runtime state 绑定 canonical checkout root、instance ID 与单调 generation；复制/移动/篡改状态及 directory/FIFO 等非普通 state 对象会失败关闭。`.env` 每轮按 instance metadata 强制重绑定 root/app DB/Huey path，同时保留 access/encryption secret 与外部 LiteLLM connection。停止或失败回收只终止已验明进程，不再 pathname-unlink state；ownership-bound dead record 保留到下一次验明 ownership 的 publication，因此旧 cleanup 没有删除新 generation 的路径竞态。
