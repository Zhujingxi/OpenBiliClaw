...
- **七平台能力连接器与通用浏览器任务合同**：为 `bilibili`、`xiaohongshu`、`douyin`、`youtube`、`twitter`、`zhihu`、`reddit` 建立显式 manifest/registry、来源专属严格 settings 和只读 normalize adapter；creator/channel、community/subreddit 与 explore 保留独立能力，不支持操作明确拒绝，transport 原始 row 不越过来源 package，公开结果只有稳定身份的不可变 `ActivityEvent` / `ContentItem`。新增持久化 generic source task service，按来源+能力入队，支持原子 lease claim、过期重领、旧 token 拒绝和相同结果幂等 complete，并在入库前递归拒绝 credential-shaped payload/result。`import-linter` 加入 dev lock 与架构门禁。本项尚未新增 HTTP route、替换 legacy 平台 endpoint 或重接扩展 dispatcher，当前生产来源路径不变。
