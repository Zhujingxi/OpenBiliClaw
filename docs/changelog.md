...
- **初始化不再把自己的产出丢掉：历史进事件账本、觉察/洞察进长期库**：此前 init 的 stage 1 数据只存在局部变量里（喂完 `analyze_events` / `build_initial_profile` 就丢），觉察/洞察也只活在内存的 `_init_cognition_context`（塑造完首份画像即消失）。两件事叠加的后果：① **没有去重**——`seen_items` 由 view 事件派生，事件表里没有 init 拉的历史，刚装好的实例可以把用户自己看过的视频当新内容推回去；② **无法重建**——认知循环读事件表，init 的素材对它不可见，觉察也没有真实 event id 可引用；③ 全新装机跑完 init 后「待聊确认」是空的，系统刚形成了具体猜测却一条都不问。现在 stage 1 拉到的历史/收藏按 `view`/`favorite` 写入事件账本（`seen_items` 自动派生，去重立即生效），按 canonical item key 幂等——重跑或续跑 init 只跳过不重复；`progress`/`duration`/`view_at` 一并进 metadata，供画像抽样的完播权重与时间分层使用。觉察/洞察草稿则经与常规认知同一条 merge 路径落库（去重、生命周期、用户判断语义一致），觉察引用本轮记录的事件并**如实标注 `approximate`**（模型是按轮归属而非按条），落库动作记 `init_cognition_persist` 台账。落库后 `build_initial_profile` 读长期层会同时看到草稿的持久化副本和内存副本，因此按归一化文本去重后再喂画像构建——否则同一条觉察会被当成两条独立观察加倍计权。
