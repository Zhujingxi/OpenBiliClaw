...
- **Docker 发布补丁**：`openbiliclaw-ollama` 多架构镜像构建时对 `ollama pull bge-m3` 增加 3 次有界重试，并改为通过 `trap` 在模型 digest 校验和 seed 复制完成后再停止 `ollama serve`，避免 GitHub runner / 上游模型下载偶发 `unexpected EOF` 直接打断 Docker 渠道发布；同时补齐发布契约测试覆盖该构建顺序。Douyin runtime source selection 加显式 tuple 类型注解，修复 `main` CI 的 MyPy literal tuple 推断误报。
