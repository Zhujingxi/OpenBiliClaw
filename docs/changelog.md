...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排 / API / 四端 UI 基础（issue #56）**：所有保存 local-first；favorite 写平台收藏，watch-later 优先原生、缺失回退收藏，自动同步默认关闭。canonical identity、normalized 保存表、capability router、durable sync service、B 站 adapter、平台中立 `/api/saved/*` 与 task polling 已落地。插件 side panel、插件设置、桌面 Web 与移动 Web 现在统一发送 canonical payload；收藏 / 稍后再看页提供单项重试、批量确认、真实目标与 `待同步 / 同步中 / 已同步 / 需要登录 / 同步失败` 状态，按平台汇总结果。自动同步首次开启必须确认账号写入警告；手动同步始终可用，平台失败不回滚本地，删除本地记录不删除平台记录。「全部稍后看」用 local-save `Promise.allSettled`，只移除本地成功项并显示精确成功 / 同步中 / 失败数。每次 sync 在单个事务写 durable task 快照并领取 live owner；严格 identity、热重载取消和 heartbeat/watchdog fence 语义保持不变。
...
- **原生保存 Phase 1 文档与验证收口（issue #56）**：配置、CLI、存储迁移、runtime、integration、推荐、扩展和四份架构图统一为当前 local-first 契约；明确当前只有 Bilibili 注册真实账号写入 adapter，其它平台仍是 local-only 后续工作。全量验证补齐 canonical `content_cache.item_key` 夹具，并修复 desktop 共享 normalizer 对 `x → twitter` 的别名归一化，以及移动设置加载成功后被 `.btn` 样式错误显示的「重试加载」。默认 smoke 和本地视觉 E2E 不发送 favorite / watch-later 请求；命名 BV ID 的真实写入必须等待当次明确授权或测试账号。
