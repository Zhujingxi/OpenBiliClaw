...
- **Chrome Web Store listing metadata API 自动化设计**：确认 API v2 不提供商店文案写接口，改用仍受支持到 2026-10-15 的 v1.1 metadata bridge；流程先用现有 Actions OAuth 密钥只读探测 draft，成功后才撤回审核、精确覆盖短描述 / 详细描述、回读校验并重新提审。截图没有公开写接口，设计明确禁止猜测 Dashboard 私有端点或读取浏览器 Cookie。
