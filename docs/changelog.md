...
- **Chat 从单备选升级为稳定 ID 有序路由（阶段 5）**：新增 `llm.route.OrderedLLMRoute` / `CircuitTable`，允许同一种连接类型以不同 connection ID 重复出现并严格按 `models.chat.connections` 数组位置尝试；Provider 内 transport retry 完成后才 fallback，整条 route 共用一个总 deadline，剩余时间耗尽后不再启动新连接。rate-limit 优先读取适配器安全解析的 `Retry-After`（缺失时 60 秒），auth/model-not-found 保持 open 到 revision 变化或 exact probe 成功，timeout/connection/5xx 按 15/30/60/120/240/300 秒冷却，invalid-response/moderation 只在当前请求 fallback；exact probe 可绕过 open circuit 并在成功时关闭。耗尽错误只携带固定摘要的 connection ID/type/preset/position/failure kind，不保存上游异常或 secret；`LLMResponse` 在保留 provider/model 计价字段的同时补齐四个 connection metadata。`LLMService` 的普通、结构化、多模态和工具路径现只走一个全局 `complete()`，模块 caller 仅用于并发/usage；旧 module override DTO/parser 暂留给 Task 8 未改构造器但完全不参与选择，完整 `RuntimeContext`/API/UI composition 与旧参数删除仍由 Task 8 完成。
