...
- **Manifest 驱动的扩展任务运行时**：service worker 每轮读取 `/api/v1/sources`，仅对 manifest primary/fallback browser transport 与本地 executor 的交集构造 dispatcher；per-source settings 写入后立即重建 API registry。Douyin 默认 direct 模式只领取 bootstrap，切换 extension 后动态加入 search/trending/feed 及现有页面 executor。claim deadline 在调用 executor 前验证，执行中到期通过 AbortSignal 取消 tab/message/listener 等待并只回写 typed failure；迟到 result 不会 success-complete，所有 timer、listener 与临时 tab 均有界清理。
- **来源 transport 热更新与迟到入队终态加固**：source settings commit 与 API registry publish 串行，长期 worker 每个 source/feed job 重读当前 settings；迟到成功的 enqueue 会继续后台 cancel，失败日志只保留异常类名。
