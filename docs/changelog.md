...
- **统一 runtime producer 的候选 claim 所有权**：API / OpenClaw 各自已构造的抖音、YouTube、知乎 producer 在挂接 `CandidateEvalCoordinator` 后只把 raw 候选 enqueue；pipeline 的单次 enqueue 回调立即唤醒协调器，producer 不再同步 `drain_pending()` 另领 batch，避免它们与 3×30 worker pool 重叠而突破 90 条 raw 在途上限。独立/CLI producer 未挂协调器时仍保留旧 inline drain 兼容行为。
