...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排 / API 基础（issue #56）**：所有保存 local-first；favorite 写平台收藏，watch-later 优先原生、缺失回退收藏，自动同步默认关闭。canonical identity、normalized 保存表、capability router、durable sync service、B 站 adapter、平台中立 `/api/saved/*` 与 task polling 已落地。每次 sync 在单个事务写 `native_save_tasks` / `native_save_task_items` 快照并领取 live owner，因此 partial/all missing、零 eligible、already-synced、active-owner、membership 删除和 service/API 重建后都保留稳定 UUID 与逐项结果；未知 UUID 404。严格 identity 只接受无空白/控制字符的 `<platform>:<stable-id>` 或 `<platform>:url:<24位小写hex>`，URL fallback 只接受无凭据且 host/port 有效的 HTTP(S)。`RuntimeContext` 热重载先取消 registry inflight 并原子发布新 client/service；registry 只跟踪顶层 runner，service-owned heartbeat/save/watchdog 保留到真实 I/O 结束。task 由 runner / task heartbeat / execution / item heartbeat 分层 fence；平台失败不回滚本地。四端 UI 仍属后续任务。
- **原生保存自动同步配置契约（issue #56）**：默认关闭的 `[saved_sync].auto_sync_enabled = false` 支持 TOML、`GET/PUT /api/config` 和 `config-show` 严格 round-trip，并已由新保存 API 在请求时读取；开启只创建后台 sync、不阻塞本地保存，手动 `/sync` 始终可用。桌面 / 移动 Web / 插件设置控件仍待接入。
