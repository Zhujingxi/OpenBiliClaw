...
- **Task 20 复审加固 durable workflow**：DB→Huey 改用 immediate TaskWrapper enqueue 与 `job_runs.dispatched_at` publish-then-marker；普通重试 reconcile undispatched row，worker startup 重发全部 pending row，覆盖 queue failure、enqueue/marker 与 dequeue/claim 两个崩溃窗口，重复消息由原子 claim 消解且 restart-safe。四个 handler 的 progress checkpoint 保持独立可见，而 cancellation running guard 与 activity、profile revision+consumed ledger、content+assessment+feed entry、terminal cleanup 分别共用一个 UoW；条件 update 取得 SQLite write lock，明确保证 cancellation 先提交则零后续 effect，feature guard 先取得锁则 feature 先原子提交。`source_sync_interval_minutes` 通过每分钟 tick + DB 幂等时间桶生效。画像增加独立 `profile_consumed_evidence`，与 revision 原子提交并携带 expected base revision 拒绝陈旧 AI delta。Feed 在 batch 前排除同 revision 已评估和历史 admitted/interacted/dismissed 内容，有界扩量补足新候选，持久化全部评估，并把 topic cap 收紧为任一 topic 饱和即拒绝。两份 Compose 让 backend/worker 显式共享 vNext DB、Huey 文件继续隔离；真实 SqliteHuey dequeue-loss recovery、迁移 parity、并发/回滚、二次补池与四类持久化边界取消均有离线回归。
