...
- **公开运行时切换到薄 `/api/v1` 与运维 CLI**：删除 11k 行 FastAPI app 与 9k 行 feature CLI 的权威入口，改为 12 个 feature router、注入式 `ApplicationContainer`、集中 401/403/404/409/422/503 错误映射和只来自环境的 constant-time bearer gate。chat、onboarding/job progress 使用标准 SSE，扩展来源工作只走 generic long-poll claim/complete；OpenAPI 以稳定 `v1_*` operation ID 可重复生成，frozen metadata 暴露正确 JSON-object schema 且无 secret default/example。CLI 仅保留 `serve`、`worker`、`doctor`、`eval`、`db migrate/backup`，Compose 的 `api`/`worker` 使用同一正式入口与 vNext readiness。现有静态 Web 继续挂载但尚未重接，新 API 不保留 legacy endpoint/command 兼容。
