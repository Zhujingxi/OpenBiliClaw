...
- **修复连点点赞/点踩/聊一聊时 feedback 请求卡死或超时**：`/api/feedback` 过去在响应内同步等待统一兴趣线流水线的偏好 LLM 分析（真实运行一轮 30–76 秒，桌面端客户端超时仅 30 秒、移动端无超时），连续点击会并发触发多轮分析并占满浏览器连接，表现为“无反应”或“反馈提交失败”。现在事件先落库、HTTP 立即返回，流水线 ingest 改为后台任务并用锁串行，连点不再并发跑 LLM；`unified_interest_line=false` 的回退路径行为不变。
- **统一兴趣线流水线 ingest 加锁串行，连点不再并行跑多轮 LLM 分析**：`ProfileUpdatePipeline` 的 `ingest_batch` / `tick` / `flush` 现由 `_ingest_lock` 串行化——快速连点、浏览器事件批次与反馈同时到达时，同一层缓冲不会再被并发 drain 出多轮偏好分析（省 LLM 调用、避免 `pipeline_state.json` 快照互相覆盖），与旧反馈批线的 `_feedback_batch_lock` 保护对齐；单次调用行为不变。
- **移动端反馈提交补 30 秒超时、惊喜卡失败不再假装成功**：移动端 `submitFeedback`（点赞/点踩/聊一聊评论）此前没有超时，后端异常时会一直转圈；现在与桌面端一致设置 30 秒超时。惊喜卡「喜欢 / 不感兴趣」请求失败时，移动端不再本地移除卡片或标记已发，而是保留卡片显示“操作失败，请重试”并恢复按钮；桌面端同样保留卡片并弹 toast 提示可重试。
