...
- **模型连接记录可安全构造协议 Adapter（阶段 4，尚未切换运行链）**：新增 `llm.connection_factory`，以稳定 connection/provider ID 命名 Chat 与 Embedding adapter；OpenAI、DeepSeek、OpenRouter 与 custom preset 统一构造同一个 `OpenAIProtocolProvider`，每个实例使用 frozen options 隔离 API mode、reasoning body 与 attribution headers，不再通过可变子类状态切换 DeepSeek 请求；hook 元数据不进入 repr，env credential 来源在 runtime options 构造时复制成只读快照。Anthropic 官方和 custom gateway 统一构造 `AnthropicCompatibleProvider`，按 timeout/connection/5xx 与 429/auth/其他 4xx 区分重试，并把 cache read/create 纳入 total usage；Gemini 与新 protocol adapter 的未知上游异常只产生固定 ID 文本且不保留 secret-bearing cause/context。OpenAI / Anthropic custom URL 在任何 proxy/client callback 前由共享 HTTP(S) validator 拒绝 userinfo、query/fragment、控制字符、反斜线、外层空白、无效 host/port，合法 path/port 原样保留；Gemini 保持原生 SDK，Ollama 保持 `num_ctx` 与强制直连。inline / 精确 env / Codex OAuth / descriptor 允许的 none credential 只在构造时解析，Codex token lookup 前严格拒绝任何非精确 OpenAI 官方 scheme/host/port/path/query/fragment/userinfo endpoint。Embedding provider 记录仍不含 model/setting override，adapter 持有调用方传入的同一个不可变共享 settings 对象。legacy `build_llm_registry()` / `build_embedding_service()` 构造 API 与行为继续保留；本阶段没有 ordered route、circuit、runtime cutover、模型 API 或 UI。
