...
- **升级后先恢复历史合格库存再调用 LLM**：每次 runtime 启动或热重载都会先在原子 pool maintenance 事务中检查历史 `suppressed` 行；仅恢复未推荐、未看过、非 dislike/shown/purged、仍达 admission 与完整文案/分类/链接/XHS 本人/delight guards 的结果。恢复先补来源缺口但允许任意来源填满全局 target，到 target 即停且重复执行幂等；`recovered_suppressed` 报告维护后仍 fresh 的净恢复量。delight 预处理、候选评估和文案后台 loop 全部在该零 LLM 恢复之后启动，避免升级后为已有结果重复付费。
