...
### LLM token diet landing hardening

- **真实 replay gate 升级为 fail-closed artifact v2**：`scripts/run_profile_diet_ab.py` 现在加载与生产一致的 effective profile（user overrides + active speculations）并镜像 archived-topic serialization；按生产 30 条 claim grouping、`source_context=mixed` 和 4096 output ceiling 跑 repeated A/A + A/B。每个 logical run 归因并校验实际 provider / instance / model；embedding exception、空 / NaN / Inf / 维度漂移、recall 不完整、route / snapshot 漂移、body-cap 零受影响或常量漂移都会进入最终 blocking reasons 并非零退出。单个 chunk 遇到明确的瞬时 rate limit 时会等待 registry 的 60 秒 cooldown 后最多重试两次，重试前恢复候选评估字段；402/余额/计费错误与其它失败不重试，恢复的限流调用仍保留在 route artifact 中。compact / body-cap / reason-diet 三臂都可生成不含正文、完整画像或 secret 的独立 JSON；temperature 固定 0 以隔离采样噪声，同时在 artifact 披露生产默认 0.7。2026-07-18 的旧 replay PASS 继续作废。
- **Replay 限流分类尊重 provider 规范化边界**：真实 compact 验收捕获到 transient HTTP 429 被误判为永久额度问题；根因是 replay 在看到 adapter 已归一化的 `LLMRateLimitError("rate limit exceeded")` 后仍继续扫描 SDK raw cause，而部分网关的原始 429 元数据含通用 `billing` 字段。分类现在止于第一个规范化限流异常：明确映射的 429 / cooldown 可按每 chunk 独立预算恢复，明确映射为 HTTP 402、余额不足或计费错误的异常仍立即失败；测试覆盖误导性 raw cause、前一 chunk 用满两次额度后下一 chunk 仍有自己的预算，以及永久额度错误零重试。
- **真实质量门否决过度压缩，compact 边界回调到 80 / 16**：最终 commit 上的首轮 100×3 compact replay 让 64 兴趣 / 每域 12 specifics 的 treatment Spearman 中位数降至 `0.494686`（control floor `0.570454`），admission delta 中位数为 `-0.09`（floor `-0.07`），因此该配置明确不作为 landing 证据。实现没有放宽 gate，而是把内容 prompt 边界调为 80 兴趣 / 每域 16 specifics，长尾 recall 改从第 81 项开始；这会完整保留当前真实画像的兴趣和 specifics，只删除高频状态中的易变元数据，同时在成熟 fixture 上仍缩短约 58%、极限 fixture 上缩短约 63%。修正后的三臂必须从新 commit 全部重跑，旧失败 artifact 只保留诊断用途。
- **Eval cache v2 形成可恢复的输入闭包**：single / batch key 覆盖实际 prompt-visible content/context、compact profile + 精确 tail-recall pool、negative-examples 内容 digest、embedding namespace、prefilter mode 与 schema version；正文、指标、来源上下文、长尾权重或模型 namespace 变化都会 miss。临时 / 部分 embedding recall 失败不写 normal cache；混合平台、不稳定隐式 context 和 vision attempt 整批绕过 per-item normal cache。cache 保存 raw per-item result，franchise/style cap 在 caller grouping 上重放，包含 enforce-prefilter 边界的 cold/warm 结果一致；空 metadata 会明确清除对象旧值。
...
- **LLM token diet 收口到评估 / 表达热路径**：LLM caller bucket 补齐 `discovery.keyword*`、`discovery.x*`、`discovery.douyin*`、`runtime.bilibili_extension_search*`、`pool_purge*`、`api.sentiment*`，整条高频链路都能用 `[llm.discovery/evaluation/recommendation/soul]` 分层模型配置覆盖。内容评估画像改用 compact summary（20 核心 / 80 兴趣 / 32 域 × 16 specifics / 12 recent，长期避雷不裁剪），每条候选另带从画像块外长尾（权重 81..256 名）召回的最多 3 个 `related_interests` 兴趣名（画像不超过 80 兴趣时零开销），digest 覆盖 compact 画像和召回池；最初 64 / 12 的实测结果后续被 fail-closed artifact v2 否决，当前门槛不变并以 80 / 16 重跑。推荐表达 / legacy 分类共用同一个 `compact_content_prompt_profile_summary()` 收口，`interests=` 内容相关替换保留；discovery 与推荐侧 eval / 表达 `body_text` 进入 prompt 前截断为头 200+尾 100。抖音 / X / YouTube / 小红书 / B站扩展兜底的关键词生成统一改用查询瘦身画像 `build_query_generation_profile_summary`，与主 search / explore / keyword_planner 相同的稳定口味输入。（坏输出重试沿用 main 的 keyed-sibling 缺失成员有界重试，未采用早期二分拆批方案。）
...
- **Discovery eval embedding 预过滤进入 shadow rollout**：`[discovery].eval_prefilter_mode` 新增 `off` / `shadow` / `enforce` 三档，默认 `shadow` 只记录 `prefilter-shadow` would-filter 候选并继续送 LLM；确认 would-filter 中几乎没有高于 admission 门槛的内容后可切 `enforce`，由本地 embedding 相似度提前缓存低分并跳过 LLM。`explore` 候选始终豁免，enforce 单批过滤超过 50% 时自动 fail-open。
- **LLM token diet 收口到评估 / 表达热路径**：`discovery.evaluate_batch` 坏输出不再整批退化成 N 次单条调用；当前 landing 实现只对成功响应里的缺失 / malformed member 做有界 subset/split retry（最多深度 3、额外 6 请求），provider 异常原样传播。LLM caller bucket 补齐 `discovery.keyword*`、`discovery.x*`、`discovery.douyin*`、`runtime.bilibili_extension_search*`、`pool_purge*`、`api.sentiment*`，整条高频链路都能用 `[llm.discovery/evaluation/recommendation/soul]` 分层模型配置覆盖。内容评估画像改用 compact summary（20 核心 / 80 兴趣 / 32 域 × 16 specifics / 12 recent，长期避雷不裁剪），每条候选另带从画像块外长尾（权重 81..256 名）召回的最多 3 个 `related_interests` 兴趣名（画像不超过 80 兴趣时零开销），digest 覆盖 compact 画像和精确召回池。此版本最初记录的 replay divergence PASS 后续因 source-context 污染、独立快照和非生产预算被判无效；landing hardening 的首次严格 64 / 12 replay 也未通过质量门，当前改为 80 / 16 后由 artifact v2 重新验收。推荐表达 / legacy 分类共用同一个 `compact_content_prompt_profile_summary()` 收口，`interests=` 内容相关替换保留；discovery eval `body_text` 进入 prompt 前截断为头 200+尾 100，批量池文案同为 200+100。
