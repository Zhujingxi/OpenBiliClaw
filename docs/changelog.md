...
- **Task 20 复审加固 durable workflow**：DB→Huey 改用 immediate TaskWrapper enqueue 与 `job_runs.dispatched_at` publish-then-marker，queue failure、启动 reconcile 和 enqueue 后崩溃重复投递均可恢复，原子 claim 防重复执行；四个 handler 新增应用库 cancellation/progress checkpoint，来源/模型边界后与持久化前协作停止。`source_sync_interval_minutes` 通过每分钟 tick + DB 幂等时间桶生效。画像增加独立 `profile_consumed_evidence`，与 revision 原子提交并携带 expected base revision 拒绝陈旧 AI delta。Feed 在 batch 前排除同 revision 已评估和历史 admitted/interacted/dismissed 内容，有界扩量补足新候选，持久化全部评估，并把 topic cap 收紧为任一 topic 饱和即拒绝。两份 Compose 让 backend/worker 显式共享 vNext DB、Huey 文件继续隔离；真实 SqliteHuey execution、迁移 parity、并发/回滚、二次补池与阻塞边界取消均有离线回归。
