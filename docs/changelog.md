...
- **跨平台原生保存设计、首阶段计划与本地存储 / 编排基础（issue #56）**：所有保存 local-first；favorite 写平台收藏，watch-later 优先原生、缺失回退收藏，自动同步默认关闭。首阶段已落地 canonical identity、normalized 保存表、capability router 与 durable sync service。task 由唯一 runner token、task heartbeat、execution token 和 item heartbeat 分层 fence；跨 service 不并发跑 batch，heartbeat failure 会与 work 同步监控并安全取消。adapter 因 item heartbeat 异常、响应 deadline 或调用方取消进入 detached 后，tracked watchdog 统一以有界退避持续重试 lease 到真实终止，并归一化 late terminal / malformed 结果，避免后续心跳异常开放重叠重试。rolling upgrade 为 started active 旧行写保留 legacy runner sentinel 与 fresh lease，旧 runner heartbeat 可继续保护，stale 后新 runner才接管；公开 runner ID 禁用保留前缀。状态机、空白 ID/filter、malformed adapter 输出均 fail closed；本地 membership 不因平台失败回滚。首个生产 adapter 已支持 B 站 exact-title `OpenBiliClaw` 收藏夹与原生稍后再看，通过 `SESSDATA + bili_jct` 网络前置校验、application-aware 正整数 aid 解析、同一 client 实例/title 收藏夹单飞（不宣称跨 client/process）、脱敏 GET/POST HTTP/`-101` 错误和 operation-specific code 归一化输出 `synced / already_synced / login_required / rate_limited / failed`：只有 resource-deal POST 的 `11201` 会包装成 dedicated duplicate success，folder/resolver 同码保持 failed；watch-later `90003` 固定为视频不可用失败；GET/POST HTTP 412/429 共用安全 `-412/-429` 限流码并保留 cause；HTTP/runtime/UI wiring 仍属后续任务。
