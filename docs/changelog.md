...
- **Browser contract review hardening**：extension origin 明确排除 loopback trust 与 CORS auth bypass，只能 device-key exchange finite bearer；login/exchange 使用分离、per-peer、bounded failure limiter。vNext auth 改为 environment-only，startup keyed password fingerprint rotation 与 auth epoch increment 原子提交。两份 Compose 只向 API 转发 password hash、required session secret、extension digest records 与 public Admin URL，不向 worker 传 browser-auth material。Starlette 404/405 统一进入 `ErrorEnvelope`。七平台 settings 通过 GET/PUT 严格验证后写入现有 `settings` table，并在 registry rebuild 时恢复；worker 在 runtime scope 应用/恢复 persisted network/logging policy。Explicit profile edit 使用严格晚于上一 revision 的 fresh timestamp。全部合同仍不包含明文 password/device key，也不宣称前端或真实浏览器完成。
