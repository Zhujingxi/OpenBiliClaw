...
- Fixed Kimi Assistant tool-output compatibility without a generic provider-body escape hatch. The coding endpoint supports tool calls, but its default thinking mode rejects PydanticAI's forced `tool_choice = "required"`; the reviewed `[model.options] disable_thinking` toggle now sends only `thinking: {type: "disabled"}` through the native OpenAI constructor. The Assistant output tool schema now also enumerates its four valid discriminator values instead of advertising an unconstrained string. Real application-graph Assistant and preference-analysis E2E coverage verifies the correction.
