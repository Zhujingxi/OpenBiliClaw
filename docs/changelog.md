...
- **triage regression sweep：mypy 类型修复、release 脚本 Python 3.11+ 选择、`/api/config` 日志路径脱敏、删除 dead `ClaudeProvider`**：`sync-aggregate-release.sh` 在 `tomllib` 不可用时改探测 `python3.11+`，避免 macOS 自带 `python3`（3.9）拉发布包失败；`GET /api/config` 的 `logging.directory`、`logging.filename` 与 `logging.file_path` 改为不可逆脱敏输出——相对 `directory` 原样返回，绝对路径（如 `/srv/private/openbiliclaw/logs`）、`~` 前缀家目录相对路径、Windows 盘符绝对路径与 UNC 路径统一降为 basename，绝对 `filename` 同样降为 basename，根/卷根路径（`/`、`C:\`、`~`）脱敏为空且 `file_path` 回退为仅文件名，不再泄露主机文件系统布局；`PUT /api/config` 改用无歧义协议——未修改时客户端回传 `file_path` 精确匹配脱敏 wire 形式即识别为 echo 并跳过，有意修改（包括改到与显示 basename 完全相同的相对路径）时省略 `file_path`、直接提交 `directory`/`filename` 即生效（此前值相等启发式无法区分未修改 echo 与有意 basename 编辑；新增 `tests/test_api_app.py` POSIX root、Windows drive root、bare `~` 及 intentional-basename 碰撞回归）；桌面与插件设置页同步按「未修改发 `file_path`、已修改发 `directory`/`filename`」拆分保存负载；`openbiliclaw.llm` 删除未被任何生产代码使用的独立 `ClaudeProvider`（209 行）与其在 `__all__` 的导出，Anthropic 官方 / 自定义 Messages 网关统一由 `AnthropicCompatibleProvider` 承载，`tests/test_llm_providers.py` 同步收敛到现存 adapter；`mypy src/` 在 cli_models.py 类型漂移修复后回到 0 错误。本次为 maintenance 收口，不提升版本号或修改 README 发布预告。
