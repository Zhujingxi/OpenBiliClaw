# OpenBiliClaw Documentation

Current-state documentation:

- [Architecture](architecture.md)
- [Specification](spec.md)
- [Runtime Composition](modules/composition.md)
- [Core Runtime](modules/runtime.md)
- [Configuration](modules/config.md)
- [CLI](modules/cli.md)
- [API](modules/api.md)
- [Provider Access](modules/provider-access.md)
- [Content Integration](modules/content-integration.md)
- [Provider authoring and production-status matrix](modules/content-provider-authoring.md)
- Provider references: [Bilibili/native boundary](modules/content-integration.md#bilibili-native-boundary), [YouTube](modules/provider-youtube.md), [Bangumi](modules/provider-bangumi.md), [V2EX](modules/provider-v2ex.md), [Hacker News](modules/provider-hackernews.md), [Weibo](modules/provider-weibo.md), [Linux.do](modules/provider-linuxdo.md), [Reddit](modules/provider-reddit.md), [X](modules/provider-x.md), [Zhihu](modules/provider-zhihu.md), [RedNote](modules/content-provider-rednote.md), and [Douyin](modules/content-provider-douyin.md)
- [Observations](modules/observations.md)
- [User Understanding](modules/understanding.md)
- [Target Recommendation](modules/recommendation-target.md)
- [Application Workflows](modules/application.md)
- [Assistant](modules/assistant.md)
- [AI Runtime](modules/ai-runtime.md) and [AI providers](modules/ai-providers.md)
- [Frontend](modules/frontend.md) and [Extension](modules/extension.md)
- [Installation](agent-install.md), [deployment](agent-deployment.md), and [Docker](docker-deployment.md)
- [Privacy](privacy.md), [FAQ](faq.md), and [contributing](contributing.md)
- [Chrome Web Store](chrome-webstore-listing.md) and [Firefox source build](firefox-amo-source-build.md)
- [E2E Testing Plan](e2e-testing-plan.md) (maintained plan preserving the historical L0–L6 layer record and providing an opt-in automated L7 browser journey) and [testing log](e2e-testing-log.md)
- [Changelog](changelog.md)

Provider-specific target docs are named `modules/provider-*.md` or `modules/content-provider-*.md`. Completed feature plans and superseded pre-cutover documents are removed after current behavior is captured in the specification/module docs; the retained E2E plan/log maintain the validation guidance and historical verification records. This index lists the maintained documentation set.
