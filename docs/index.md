...
| 配置参考 | [modules/config.md](modules/config.md) | `config.example.toml` + `src/openbiliclaw/model_config/` | ✅ 持续更新（含原生 `[models]` schema、事务保存/回滚、legacy backup、`config.local.toml` 只读覆盖与既有通用配置） |
