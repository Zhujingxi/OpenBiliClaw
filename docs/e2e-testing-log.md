...
10. **Fresh-state replenishment failure:** an in-container traceback showed `RecommendationPipeline.replenish` passing the complete accepted tuple to `EvaluationService.evaluate`, whose single-batch guard rejected more than 20 candidates. A fresh Docker database can combine up to 20 search and 20 feed candidates, while prior stateful runs happened to admit fewer. The shared evaluation service now preserves the combined order, evaluates all accepted supply in batches bounded by the evaluator's declared maximum, rejoins candidates and records once, and leaves the existing selection limit to operate over the complete set. A hermetic mixed-supply regression proves 20+5 candidates are each evaluated once in order and that the trailing feed candidate can win downstream selection. No evaluator/model budget or acquisition limit changed; real Docker was deliberately not run in this repair worktree and remains the post-merge validation.
...
## V2EX layer + capability-aware sources incidents

- **l1v2ex live layer (2/2 green)**: anonymous connect → hot feed identity invariants → dynamic topic detail fetch. SEARCH was dropped from the V2EX manifest — no official search endpoint exists; local hot-list filtering is not honest search (YouTube-FEED precedent).
- **UI pass found the Search provider select offered v2ex despite its lack of SEARCH capability.** Fixed at the seam: `/v1/sources` entries now include declared `capabilities` (new `HostFacade.provider_capabilities` → flat `SourceStatusEntry` transport model), and SearchView filters the select to search-capable providers (entries without capability metadata stay visible for backward compatibility). OpenAPI snapshot + generated client regenerated.
- **Pre-existing transport-typing defect fixed**: `NativeContent.payload: SerializeAsAny[StrictBaseModel]` exported to OpenAPI as `BaseModel` = `Record<string, never>`, making the generated client reject every real payload at the type level. New `NativeContentView` transport model serializes the payload as a plain JSON object (`dict[str, JsonValue]`).
- **Process incident**: a piped gate (`npm run typecheck | tail`) masked a typecheck failure in the Phase 1 sweep — pipelines return the last command's exit code. Gates must run unpiped (or with pipefail). The masked failure was caught in this phase and fixed.

## Linux.do upstream block (Phase 3 stop condition)
...
## Final all-platform sweep (2026-08-15) + seeder root fix

- **Full sweep green**: l0 4/4, l1a 2/2, l1b 2/2, l1youtube 2/2, l1bangumi 2/2, l1v2ex 2/2, l1weibo 2/2, l2 2/2, l3 4/4, l4 2/2, l5 3/3, l6 1/1. Five providers enabled and connected in the final UI pass (bilibili, youtube, bangumi, v2ex, weibo).
- **Seeder trap recurred and was root-fixed**: the sweep's model layers 401'd because `data-e2e/config.e2e.toml`'s model ref again resolved to the DeepSeek key (verified by sha256/length comparison only). Root cause: `_seed_config` kept any *resolvable* ref without checking it wraps the layer's key file. Fix: the seeder now byte-compares the resolved secret against the key file inside the vault callback and re-stores on mismatch; regression test added (`test_seed_config_replaces_resolvable_but_mismatched_vault_ref`).
- l4 flaked once ("no content preference derived after three attempts"), green on rerun — known model-derivation variance with the limited test key.
- **Transient**: a stale long-running server instance 500'd weibo connect (`temporary_failure`); a fresh server accepted all five connects. Noted gap: the API middleware's catch-all 500 handler does not log the swallowed exception — worth a follow-up.
