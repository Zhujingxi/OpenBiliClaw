...
共同绑定进程状态。Installer metadata 在首次进入临界区前持久绑定 lock UUID/device/inode；
POSIX 通过 held parent dir FD、Windows 通过不含 `dir_fd` 的 direct-path 分支，在临界区
前后复核 metadata 与 pathname。已绑定 pathname 缺失/换 inode、复制 checkout 或
