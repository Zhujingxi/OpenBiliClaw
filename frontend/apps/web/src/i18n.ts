...
export const LOCALE_STORAGE_KEY = "obc-locale";
...
export function useLocale(): {
  locale: WritableComputedRef<SupportedLocale>;
  setLocale: (locale: SupportedLocale) => void;
} {
  const { locale: activeLocale } = useI18n({ useScope: "global" });
  const locale = computed({
    get: () => matchLocale(activeLocale.value) ?? "en",
    set: (next: SupportedLocale) => {
      activeLocale.value = next;
    },
  });
  return { locale, setLocale: (next) => (locale.value = next) };
}

export type WebI18n = ReturnType<typeof createWebI18n>;
