...
def main(argv: list[str] | None = None) -> int:
    ...
    baseline = {
        ...
        "description": (
            ...
            "known_failures entries are fingerprinted over the full normalized "
            "failure content — the normalized message attribute PLUS the full "
            "JUnit element text, joined on a channel separator: "
            "whitespace-collapsed with numbers/tmp paths scrubbed, then SHA-256 "
            "digested and prefixed with a bounded human-readable preview "
            "('<preview> | sha256:<hex>'). The digest covers BOTH channels in "
            "full with no truncation, so a failure at the same node with a "
            "different exception type, headline, or nested cause anywhere — "
            "attribute or body — is rejected as a new bug, and a body-only "
            "mutation cannot hide behind an absent or generic message "
            "attribute. Every baseline entry must match this exact fingerprint "
            "grammar; the comparator rejects legacy strings, fingerprintless "
            "objects, and malformed fingerprints as a corrupt baseline. "
