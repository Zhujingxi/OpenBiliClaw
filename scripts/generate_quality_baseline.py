...
def main(argv: list[str] | None = None) -> int:
    ...
    baseline = {
        ...
        "description": (
            ...
            "failure message: whitespace-collapsed with numbers/tmp paths scrubbed, "
            "then SHA-256 digested and prefixed with a bounded human-readable "
            "preview ('<preview> | sha256:<hex>'). The digest covers the ENTIRE "
            "message with no head/tail truncation, so a failure at the same node "
            "with a different exception type, headline, or nested cause anywhere "
            "in the message is rejected as a new bug. "
