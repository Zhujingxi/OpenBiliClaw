...
def _base_go_environment() -> dict[str, str]:
    env = os.environ.copy()
    env.update(
        {
            "CGO_ENABLED": "0",
...
def _list_target_packages(goos: str, goarch: str, tags: tuple[str, ...]) -> list[dict[str, Any]]:
    env = _base_go_environment()
    ...
    for packages in package_sets:
        for package in packages:
            ...
            if not all(
                ...
                for value in (module_path, version, module_dir_raw, package_dir_raw)
            ):
                import_path = package.get("ImportPath", "<unknown>")
                raise NoticeGenerationError(
