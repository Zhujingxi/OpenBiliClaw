...
const PROBE_FINGERPRINT_FIELDS = /* @__PURE__ */ new Set([
  "name",
  "type",
  "model",
  "preset",
...
export function createModelOperationGate() {
  ...
  return {
    ...
    canStartSaveAfterLoad({ startedSaveGeneration, loadResult, state }) {
      return Boolean(
        loadResult?.snapshotApplied === true && loadResult?.descriptorsInstalled === true && startedSaveGeneration === saveGeneration && !saveInFlight && !state?.dirty
      );
    },
    controlState() {
      return {
...
export async function loadIndependentModelResources({
  ...
  return {
    snapshotApplied: snapshotOutcome.status === "fulfilled" && snapshotOutcome.value && snapshotOwned && !snapshotBlocked,
    descriptorsInstalled: descriptorOutcome.status === "fulfilled" && descriptorOutcome.value && descriptorOwned
  };
}
...
function buildOverrideLocks(overrides) {
  ...
  const locks = Object.fromEntries(OVERRIDE_CONTROL_PATHS.map((path) => {
    ...
    const lock = entries.find((override) => {
      ...
      return overridePath && (normalizedPath === overridePath || normalizedPath.startsWith(`${overridePath}.`));
    });
...
function safeCredentialStatus(raw) {
  const source = ["none", "inline", "env", "oauth"].includes(raw?.source) ? raw.source : "none";
  return {
    source,
...
function cleanDraftRecord(record, kind) {
  ...
  if (["keep", "set", "clear", "env"].includes(action)) {
    ...
    hydrated.credential.value = ["set", "env"].includes(action) ? String(record.credential.value || "") : "";
...
export function hydrateModelConfig(snapshot) {
  ...
  const embedding = models.embedding && typeof models.embedding === "object" ? models.embedding : {};
  const connections = Array.isArray(chat.connections) ? chat.connections.map((item) => hydrateRecord(item, "chat")) : [];
  const providers = Array.isArray(embedding.providers) ? embedding.providers.map((item) => hydrateRecord(item, "embedding")) : [];
    ...
    models: {
      ...
      embedding: {
        ...
        settings: {
          ...
          similarity_threshold: Number.isFinite(Number(embedding.settings?.similarity_threshold)) ? Number(embedding.settings.similarity_threshold) : 0.82,
...
export function updateRouteField(state, kind, id, field, value) {
  ...
  const previousValue = item[field] === void 0 ? void 0 : clone(item[field]);
  if (field === "credential") {
...
export function applyPreset(state, kind, id, presetDefinition, options = {}) {
  ...
  for (const [field, value] of Object.entries(presetDefinition?.defaults || {})) {
    ...
    const matchesPreviousDefault = Object.hasOwn(previousDefaults, field) && item[field] === previousDefaults[field];
    const isBlank = item[field] === "" || item[field] === null || item[field] === void 0;
    if (!next.touched[touchedKey(kind, id, field)] && (isBlank || matchesPreviousDefault)) item[field] = value;
  }
  item.probe = null;
  next.touched[touchedKey(kind, id, "preset")] = true;
  return markChanged(next);
}
export function changePreset(state, kind, id, descriptor, presetDefinition, options = {}) {
  const index = findIndex(state, kind, id);
  const item = routeItems(state, kind)[index];
  const nextPreset = String(presetDefinition?.id || "");
  const allowed = new Set(
    (descriptor?.fields || []).filter((field) => (!field.capabilities?.length || field.capabilities.includes(kind)) && (!field.presets?.length || field.presets.includes(nextPreset))).map((field) => field.name)
  );
...
function isPopulated(field, value) {
  if (field === "credential") {
    ...
    return Boolean(
      value?.action === "set" || value?.action === "env" || status?.configured || status?.source === "oauth"
    );
  }
...
export function changeConnectionType(state, kind, id, descriptor, options = {}) {
  ...
  const credentialSemanticsChanged = Boolean(previousCategory) && previousCategory === "oauth" !== (nextCategory === "oauth");
  const allowed = new Set(
    (descriptor?.fields || []).filter((field) => (!field.capabilities?.length || field.capabilities.includes(kind)) && (!field.presets?.length || field.presets.includes(item.preset))).map((field) => field.name)
  );
  const possible = kind === "chat" ? CHAT_FIELDS : EMBEDDING_FIELDS;
  const incompatibleFields = possible.filter(
    (field) => !allowed.has(field) && isPopulated(field, item[field])
  );
  if (credentialSemanticsChanged && isPopulated("credential", item.credential) && !incompatibleFields.includes("credential")) incompatibleFields.push("credential");
  if (incompatibleFields.length && !options.confirmed) {
    return { state: clone(state), incompatibleFields, changed: false };
  }
  const next = clone(state);
  ...
  const presets = (descriptor?.preset_definitions || descriptor?.presets || []).filter((preset) => typeof preset === "string" || !preset.capabilities?.length || preset.capabilities.includes(kind)).map((preset) => typeof preset === "string" ? preset : preset.id);
...
function credentialPayload(credential) {
  const action = ["keep", "set", "clear", "env"].includes(credential?.action) ? credential.action : "keep";
...
export function createProbeSignature(state, kind, id) {
  const index = findIndex(state, kind, id);
  const item = routeItems(state, kind)[index];
  const draft = kind === "chat" ? chatPayload(item) : {
  ...
  return {
    ...
    models: {
      ...
      embedding: {
        ...
        settings: embeddingSettingsPayload(state.models.embedding.settings),
        providers: embeddingEnabled ? state.models.embedding.providers.map(embeddingPayload) : []
      }
    },
...
export function circuitView(record, nowMs = Date.now()) {
  ...
  return {
    ...
    retrySeconds,
    label: circuit.permanent ? `熔断打开 · ${circuit.failure_kind || "永久失败"}` : retrySeconds !== null ? `熔断打开 · ${retrySeconds}s 后重试` : `熔断打开${circuit.failure_kind ? ` · ${circuit.failure_kind}` : ""}`
  };
}
...
export function prepareLocalOllamaEmbedding(state, descriptor, defaults) {
  ...
  if (descriptor?.id !== "ollama" || !descriptor?.capabilities?.includes("embedding")) {
    throw new Error("The server does not advertise an Embedding-capable Ollama descriptor.");
  }
  const baseUrlField = (descriptor.fields || []).find((field) => field.name === "base_url" && (!field.capabilities?.length || field.capabilities.includes("embedding")));
  if (!baseUrlField) {
...
//# sourceMappingURL=model-config-state.js.map
