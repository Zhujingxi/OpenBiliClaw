/**
 * Tests for the page-context xhs cover harvester.
 *
 * The harvester exists because xhscdn's TLS-fingerprint hotlink protection
 * (2026-07) 403s every server-side fetch — the page context is the only
 * client that can still read cover bytes, so correctness here decides
 * whether xhs cards have covers at all.
 */
