 ...
 *
 * What we capture (the user's own text contributions only):
 ...
 *      only reported when the response JSON's top-level `code === 0`. An
 *      HTTP 2xx with a bilibili business error (e.g. `code: -412`) is dropped.
 *
 * Fixture / field shapes are modelled on the community-documented bilibili
 * write APIs (bilibili-API-collect) pending a real end-to-end capture. The
 * parser only depends on the endpoint path, the form field names (`msg` /
 * `message`), and the `code===0` gate.
 *
...
export type BiliInteractionKind = "danmaku" | "comment";
 ...
 */
export function classifyBiliInteractUrl(url: string): BiliInteractionKind | null {
  if (!url) return null;
...
export function parseBiliInteract(captured: CapturedBiliRequest): BiliInteraction | null {
  ...
  const kind = classifyBiliInteractUrl(url);
  if (!kind) return null;
  // Business gate (invariant 7b): HTTP 2xx alone is not success.
  if (!biliResponseOk(responseBody)) return null;

  const text = kind === "danmaku" ? formField(requestBody, "msg") : formField(requestBody, "message");
  if (!text) return null;

  const interaction: BiliInteraction = { kind, text };
    ...
    try {
      ...
      if (url && classifyBiliInteractUrl(url)) {
        ...
        void result
          ...
          .then((responseBody) => {
            const interaction = parseBiliInteract({ url, requestBody, responseBody });
