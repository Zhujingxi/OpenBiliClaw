...
📌 最新版本：**v0.3.176（2026-07-19）**

- **每个来源的「已就绪」不再一视同仁** —— 现在明确标出这个结论是真联网验过、还是只读了本地文件或浏览器心跳，八个平台各配「测试连接」按钮当场验。
- **抖音不再永远显示「待验证」** —— cookie 有效就如实认，且验过之后 6 小时内保持新鲜，而不是一分钟就打回原形。
- **Bangumi 也纳入同一套契约** —— 匿名可读公开收藏、配了个人令牌则真验令牌，两种状态分得清。
