...
📌 最新版本：**v0.3.177（2026-07-19）**

- **修复 LLM 配置失效时首次设置向导整页卡死** —— 降级模式不再拦截向导依赖的共享脚本，Base URL 能填、配置能修，不再进死胡同。
- **插件在后端降级时直接指路修复** —— 推荐页显示具体配置报错和「去设置修复」按钮，不再只说「接口没回」。
- **初始化失败必说原因** —— 无效 API Key / 服务不可达 / 模型不存在分得清；重启打断的初始化也会如实标注，不再留幽灵进度。
- **本地 Ollama 定位统一为向量检索** —— 向导、CLI、安装脚本不再把它列为聊天服务商；进阶用户仍可在设置页启用。
