...
📌 最新版本：**v0.3.185（2026-07-26）**

- **桌面 Web 列表不再乱跳** —— 开着滚动自动加载浏览时，后台补货不再重建卡片、切回标签页不再重排列表。
- **X 内容发现恢复可用** —— 搜索恒 404 的根因已修，关键词发现重新返回真实推文。
- **模型鉴权失败一眼定位** —— 401 不再重试刷屏，直接指名是哪个 provider 和哪个地址拒绝了 key。
- **X / Reddit 遵循海外网络设置** —— 两个来源的命令行链路统一走 `[network].mode`，不再碰运气继承环境代理。
