...
### vNext 后端、`/api/v1` 与独立 worker（前端待重接）

v0.4.0 后端权威入口已切到 feature-oriented `/api/v1`、运维 CLI 与独立 Huey worker。`ApplicationContainer` 注入 use case，app factory 只负责 composition/lifecycle/router/static mount；导入时不连接 Provider 或平台。Bearer access token、SSE chat/job/onboarding、generic source-task claim/complete、稳定 OpenAPI 和集中安全错误均已接线。四个任务仍以应用库 `job_runs` 为产品状态权威。**现有 Web/扩展静态文件仍是原样挂载，API client 重接由下一任务完成；legacy 文件等待最终删除但不再是 API/CLI authority。**
...
已实现：领域/来源/persistence/use case、typed AI、四任务 worker、`/api/v1`、SSE、运维 CLI、OpenAPI
待接线：现有 Web/扩展 client；legacy 数据不迁移，旧模块在最终清理任务删除
```
