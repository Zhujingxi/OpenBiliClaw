...
- **移动端**：点击插件顶部的手机图标扫二维码，或手动输入 `http://<电脑局域网 IP>:8420/m/`。适合手机上刷推荐、看画像、和阿B聊天；顶部设置可独立管理 Saved Sync，并以列表→详情编辑 Chat / Embedding 有序模型路由与 Runtime 参数。
...
- 🔧 **完全可控** — 桌面、移动 Web 与插件设置都可按列表增删与排序 Chat / Embedding 连接，全局 Chat 路由依序故障转移；移动端使用触摸友好的列表→详情与上下移动，还可直接编辑画像、写自定义 Skill 扩展发现策略

## 🏛️ 架构概览

### vNext 领域、用例与独立 worker（公开 API 尚未切换）

v0.4.0 已落地无框架依赖的领域契约、七平台 connector、lease-safe generic source task、隔离 persistence、类型化设置、Fernet 凭据、六个 typed AI task，以及 activity/profile/feed/library/chat application service 和独立 Huey worker。worker 固定构造全部七个平台并只在首次 direct 调用时解密凭据；默认来源全部关闭，启动不触发 live source call。四个后台任务使用应用库 `job_runs` 作为状态、可靠 dispatch、幂等、运行中取消、单调进度和重启恢复的权威，Huey result 只属于 transport；启动重发全部 pending row，原子 claim 消解重复消息，write-first cancellation 与持久化事务内的 running guard 将取消和业务 effect 按 SQLite 写序原子排序。画像 consumed-evidence ledger 与 revision 原子提交，Feed 不会重复评估/准入历史内容。**当前 API、legacy 业务运行时、CLI、前端和扩展 dispatcher 尚未切换**；新库不会替换或迁移现有数据。

```text
七平台显式 SourceManifest + Connector ──规范化──► Activity / Profile / Content / Feed
             │ durable deadline + lease/cancel/abandon
             └──────────────────────────────► source_tasks
                                   │ typed repositories
                                   ▼
                     SQLAlchemy repositories + UnitOfWork
                                   │
            Alembic 0001 ──► data/vnext/openbiliclaw.db（隔离新库）
                                   ├─ typed DatabaseSettings / UserSettings
                                   └─ Fernet credential ciphertext

vNext use case ─► typed TaskSpec / PydanticAI ─► TaskRunner ─► LiteLLM ─► providers
                    │ semantic retry only            │
                    ├─ obc-interactive / obc-analysis│ routing/fallback/retry/cache
                    └─ obc-embedding ◄─ embedding ───┘

Huey scheduler/transport ─► source sync / profile / feed / cleanup ─► job_runs（全 pending 恢复 / 原子 claim / 事务 guard）

已实现：领域/七平台来源/task/persistence、application service、typed AI、四任务 worker、LiteLLM/Huey Compose
未接线：HTTP/扩展 task route、legacy 数据迁移、/api/v1 与前端切换
```

应用代码只认识 `obc-interactive`、`obc-analysis`、`obc-embedding` 三个稳定别名；provider routing/fallback、网络重试、限流与缓存只由 LiteLLM 管理。来源能力矩阵见 [vNext 多来源连接器与通用浏览器任务](docs/modules/vnext-sources.md)，用例/任务见 [vNext 用例与后台任务](docs/modules/vnext-use-cases-jobs.md)，AI 边界见 [vNext 类型化 AI 模块](docs/modules/vnext-ai.md)。
...
│ lifecycle 锁下捕获 settled runtime/loop 快照；取消等待不写盘/不发布 │
│ backup → temp/fsync/replace → 发布 graph → 串行 drain/restart → 单次事件 │
│ public stop/restart 整段互斥；drain 排除 guided_init；事件只见最终 slots │
│ degraded 成功后解除；中途取消由 shielded rollback 重取 lifecycle ownership │
