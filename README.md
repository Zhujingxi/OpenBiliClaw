...
v0.4.0 已落地无框架依赖的领域契约、七平台 capability/concrete-operation connector、带 durable cancellation 的 lease-safe generic source task、隔离的 SQLAlchemy/Alembic persistence、类型化设置、Fernet 凭据密文，以及只经 LiteLLM 的 PydanticAI typed-task 基础；B 站 search 合同明确记录 direct primary + browser fallback。**当前 API、业务运行时、CLI 和前端尚未使用它们，扩展也尚未切换 generic task route**。新库默认位于 `data/vnext/openbiliclaw.db`，不会替换或迁移现有数据；在完成生产接线与数据切换前，下方 v0.3 legacy storage/runtime 仍是唯一实际业务路径。
