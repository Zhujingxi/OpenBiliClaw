...
v0.4.0 已先落地无框架依赖的领域契约，但**尚未接入当前 API、运行时、数据库或前端**。后续 backend-first 任务会在这层之上添加 persistence、AI、source adapter、use case 与 `/api/v1`；在切换完成前，下方 v0.3 架构仍是实际运行路径。

```text
SourceManifest + SourceConnector ──规范化返回──► ActivityEvent | ContentItem
Activity：ActivityEvent + ProfileSignal（事件投影 use case 待接入）
Profile：ProfileSnapshot + ProfileDelta + apply_profile_delta()
Feed：ContentItem + CandidateAssessment + FeedEntry + Interaction
Library：CollectionItem（引用 content_id）    Chat：ChatTurn（独立对话契约）

当前状态：冻结 Pydantic 契约 + 纯策略；无 runtime / storage / API 接线
```
