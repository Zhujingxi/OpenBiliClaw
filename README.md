...
┌─────────────────────────────────────────────────────────────┐
│                       Chrome Extension                       │
│ (行为采集 · 推荐展示 · 对话 · xhs 被动收集/任务/初始化导入)  │
└──────────────────────────┬──────────────────────────────────┘
                           │ REST API / WebSocket
┌──────────────────────────▼──────────────────────────────────┐
│                      Agent 编排层                            │
...
B 站走 API 直连（WBI 签名），小红书由扩展直接从页面 state / DOM 提取元数据和初始化画像信号（默认不深滚；初始化任务显式启用时才有限滚动并分批回传；不后端爬取），通用 Web 走 Playwright CDP + LLM 内容提取。
...
| 小红书交互 | 扩展 DOM/state 元数据提取 + 后台标签页任务调度 + 初始化画像导入（零后端爬取） |
