...
📌 最新版本：**v0.3.175（2026-07-19）**

- **海外来源默认跟随系统代理** —— Bangumi / YouTube 等海外来源不再默认直连超时；配置页会直接标出哪些来源需要代理，以及改哪个设置有用。
- **小红书内容终于有头图** —— 封面地址与图片字节改为在扩展抓取时一并采集，绕开了服务端取图被 403 的老问题。
- **Bangumi 卡片显示制作方，扩展识别账号真的能用了** —— 导演 / 原作 / 开发商从条目信息直接解析；此前「已登录 bgm.tv 让扩展自动识别」这条路在三个界面里一个都走不通。
- **配置坏了也进得去设置页** —— LLM 配置失效或尚未初始化时，访问首页会被引导进设置 / 初始化向导，而不是看到一段原始 503 JSON。
