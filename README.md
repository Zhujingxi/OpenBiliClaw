...
v0.4.0 已落地无框架依赖的领域契约，以及隔离的 SQLAlchemy/Alembic persistence、类型化设置和 Fernet 凭据密文基础；但**当前 API、运行时、CLI、安装器和前端尚未使用它们**。新库默认位于 `data/vnext/openbiliclaw.db`，不会替换或迁移现有数据；在完成生产接线与数据切换前，下方 v0.3 legacy storage/runtime 仍是唯一实际运行路径。
...
已实现：领域契约、schema/migration、repository/UoW、设置与凭据加密基础
未接线：生产 runtime/API、安装器密钥生命周期、legacy 数据迁移与前端切换
```
