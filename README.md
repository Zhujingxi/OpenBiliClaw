...
- **第三方 API 网关适配** —— Claude 支持自定义 Base URL 指向中转网关；OpenAI / 兼容 provider 新增 Responses 协议（`/v1/responses`）选项。
- **开代理不再误报「B站 未登录」** —— B站 探测恒直连，代理开着不用关；检测失败时给出具体原因和排查提示。
- **自动更新对各安装形态完备工作** —— 少写 `.git` 的克隆不再被「远端不在允许列表」永久卡死；Docker 版会提示拉取新镜像；被拒时原因可见。
- **Windows 桌面包兼容系统 SOCKS 代理** —— 系统设了 socks5 代理时安装包不再启动即崩；未初始化引导在网页 / 安装包 / 插件 / Docker 四个入口全部加固。
