...
### vNext 领域、持久化与类型化 AI 基础（尚未切换）

v0.4.0 已落地无框架依赖的领域契约、隔离的 SQLAlchemy/Alembic persistence、类型化设置、Fernet 凭据密文，以及只经 LiteLLM 的 PydanticAI typed-task 基础；但**当前 API、业务运行时、CLI 和前端尚未使用它们**。新库默认位于 `data/vnext/openbiliclaw.db`，不会替换或迁移现有数据；在完成生产接线与数据切换前，下方 v0.3 legacy storage/runtime 仍是唯一实际业务路径。

```text
SourceManifest + SourceConnector ──规范化──► Activity / Profile / Content / Feed
                                   │ typed repositories
                                   ▼
...
已实现：领域/persistence 基础、typed AI/embedding/health、离线 eval dataset、LiteLLM Compose
未接线：生产 use case/runtime/API、legacy 数据迁移与前端切换
```

应用代码只认识 `obc-interactive`、`obc-analysis`、`obc-embedding` 三个稳定别名；provider routing/fallback、网络重试、限流与缓存只由 LiteLLM 管理。详见 [vNext 类型化 AI 模块](docs/modules/vnext-ai.md)。
