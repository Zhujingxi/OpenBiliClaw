...
📌 最新版本：**v0.3.163（2026-07-11）**

- **登录状态更诚实** —— PC 与 Web 配置页只读取本地凭据和插件心跳，区分已登录、待验证与内容令牌，不再为展示状态访问平台。
- **Web 库存自动恢复** —— 推荐或运行状态瞬时超时会有界重试，恢复后的库存不会覆盖正在浏览或滚动追加的卡片。
- **Embedding 冷加载不再误报** —— 本地 Ollama 首次加载模型的超时与真实停服分开判定，引导初始化仍严格等待有效向量。
- **偏好反馈更准确** —— 兴趣与避雷反馈同时作用于细、粗主题并保留真实平台来源，三端操作语义保持一致。
...
│             → pool 统计 · 已看身份                     │
│ API projected 库存 → 3×30 worker → 串行入池；OpenClaw 单次 ≤90 inline → 四端 │
