...
📌 最新版本：**v0.3.157（2026-07-05）**

- **惊喜推荐不再打断你打字** —— 在惊喜卡聊天框里输入时，后台推送的新候选不再抢走当前卡、收起输入框。
- **桌面 Web 体验打磨** —— 视频卡改真链接（中键 / Ctrl+点击）+ 时长 / 播放 / 点赞元信息、暗色模式、滚动到底自动加载。
- **备选 LLM Provider 容灾更稳** —— 主服务返回空响应也会切备选，初始化前置检查也认备选，主挂备好时不再拦初始化。
- **Windows 中文用户名向量模型自愈** —— 模型路径含中文导致 bge-m3 加载失败时，一键迁移到纯英文目录并重拉。
