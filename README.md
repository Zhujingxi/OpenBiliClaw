...
📌 最新版本：**v0.3.181（2026-07-20）**

- **推荐不再被单一来源霸占** —— 候选池份额全链路生效，修掉「reddit 占满 300 池、bangumi / 抖音 / YouTube 长期为 0」的四层饿死盲区。
- **PC Web 平台 Tab 成为真正的推荐作用域** —— 切到某平台后「换一批 / 加载更多」只从该平台选片，并显示各平台真实可推库存。
- **惊喜推荐等正式文案就绪才出现** —— 不再把兜底文案当成朋友式推荐词提前推给你。
- **按真实反馈调节奏** —— 初始化不再给不靠谱的耗时预估，改报已用时与真实进度；惊喜卡自动轮播从 4 秒放慢到 60 秒。
