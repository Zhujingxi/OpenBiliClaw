...
📌 最新版本：**v0.3.189（2026-07-30）**

- **品牌图标全入口一致** —— 浏览器标签、PWA / Apple 主屏幕、插件与网页头图、桌面启动页、社交分享图、商店素材和文档截图均已清除外围白边与旧版 `B` 图标。
- **惊喜推荐叉号真正永久生效** —— 三端点“看过了，不再推荐”会写入统一已看账本，普通推荐和惊喜推荐都不会再把同一内容带回来。
- **桌面浏览和聊口味更稳定** —— 自动续页不再把页面跳回平台 Tab，待聊确认卡片补齐清晰分层、响应式布局与可访问交互。
- **来源任务故障不再伪装成空结果** —— 抖音与小红书任务会诚实区分超时、失败、预算耗尽与安全验证冷却，并保留可重试的关键词和队列状态。
