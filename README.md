...
### vNext 领域、七平台来源、持久化与类型化 AI 基础（尚未切换）

v0.4.0 已落地无框架依赖的领域契约、七平台 capability/concrete-operation connector、带 durable request deadline 与有界取消补偿的 lease-safe generic source task、隔离的 SQLAlchemy/Alembic persistence、类型化设置、Fernet 凭据密文，以及只经 LiteLLM 的 PydanticAI typed-task 基础；B 站 search 合同明确记录 direct primary + browser fallback。过期任务即使本地 cleanup 延迟也不能重新 claim；claim/complete 使用数据库时钟避免锁等待后的陈旧授权，late enqueue 异常会被安全取回；cleanup 具有独立有限窗口，因此总返回时间可超过 browser execution timeout。**当前 API、业务运行时、CLI 和前端尚未使用它们，扩展也尚未切换 generic task route**。新库默认位于 `data/vnext/openbiliclaw.db`，不会替换或迁移现有数据；在完成生产接线与数据切换前，下方 v0.3 legacy storage/runtime 仍是唯一实际业务路径。
...
未来 use case ─► typed TaskSpec / PydanticAI ─► TaskRunner ─► LiteLLM ─► providers
                    │ semantic retry only            │
                    ├─ obc-interactive / obc-analysis│ routing/fallback/retry/cache
                    └─ obc-embedding ◄─ embedding ───┘

已实现：领域/七平台来源/task/persistence 基础、typed AI/embedding/health、离线 eval、LiteLLM Compose
未接线：生产 source composition、HTTP/扩展 task route、use case/runtime/API、legacy 数据迁移与前端切换
```

应用代码只认识 `obc-interactive`、`obc-analysis`、`obc-embedding` 三个稳定别名；provider routing/fallback、网络重试、限流与缓存只由 LiteLLM 管理。来源能力矩阵见 [vNext 多来源连接器与通用浏览器任务](docs/modules/vnext-sources.md)，AI 边界见 [vNext 类型化 AI 模块](docs/modules/vnext-ai.md)。
