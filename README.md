...
小红书、抖音、YouTube、知乎走 Chrome 插件任务链路，Reddit 日常 discovery 默认走随后端安装的 rdt-cli、初始化信号仍走插件，X 的 discovery 走服务端 cookie 重放；这些读取链路都不需要你额外启动 CDP 调试 Chrome。Reddit/X 与 YouTube 原生保存 executor 已接入并通过 fixture 测试，但未真实账号验证；小红书、抖音、知乎原生保存 executor 仍待接。`[sources.browser].cdp_url` 只保留给通用 Web / 自定义网页源的浏览器抓取场景。
...
- ⭐ **本地优先收藏 / 稍后看** — 推荐卡先写本地 SQLite；B 站可明确授权后同步原生目标；Reddit/X 与 YouTube executor 已接入并通过 fixture 测试，但未真实账号验证；小红书 / 抖音 / 知乎 executor 仍待接
...
│ 六平台 adapter → broker → shared MV3 recovery barrier → Reddit/X/YT executor（fixture-only；另三平台待接）│
└────────────────────────────────────────────────┘
```
