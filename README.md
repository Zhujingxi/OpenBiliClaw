...
📌 最新版本：**v0.3.197（2026-08-06）**

- **五个账号来源会自动增量刷新** —— 小红书、抖音、YouTube、知乎和 Reddit 默认按持久轮询每天回拉一次，扩展离线或初始化期间会安全暂停。
- **跨平台回传更耐崩溃、更少重复** —— Reddit 加入原子终态与分型去重，小红书严格执行任务 scope 和单 scope 上限，重试不会扩大画像事件预算。
- **登录态识别更准确** —— 小红书适配当前 `/explore` 登录门；B 站等来源同步凭据后，PC Web 的黄色 Cookie 警示会立即自动消退。
- **真实环境已验收** —— Chrome 登录态事件后 1ms 内触发状态刷新，独立 B 站只读探针返回 `replayed=false / verified`。
