...
📌 最新版本：**v0.3.169（2026-07-15）**

- **首次初始化真正由单一流水线接管** —— 初始化期间暂停后台画像任务，偏好分片按并发动态计时并可取消，空库存准入环、幽灵运行锁和终态后台反锁死均已消除。
- **DeepSeek / Ollama 连通性测试不再误报** —— 探针关闭无必要推理、只把显式聊天模型加入 Ollama，embedding-only 配置不会再偷跑 `llama3`；DeepSeek 自定义 Base URL 也会正确生效。
- **模型推理档位默认统一为 medium** —— OpenAI、Claude、Gemini、DeepSeek 和 OpenRouter 使用原生推理参数，发现、推荐和来源抽取等短任务继续采用低延迟档位。
- **桌面体验与初始化反馈更清晰** —— 新用户默认经典配色，三端显示真实批次、耗时、取消与恢复动作，窄栏惊喜卡和滚动续载同步修复。
