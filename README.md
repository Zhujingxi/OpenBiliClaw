...
- **向量模型问题可自诊自修** —— 初始化页直接显示 Ollama 没运行 / 缺模型 / 模型损坏等具体原因，一键自动下载 bge-m3 并实时显示进度。
- **手机点推荐直达对应 App** —— B站 / 小红书 / 抖音 / YouTube / X / 知乎深链拉起；插件与桌面 Web 新增醒目「手机版」扫码入口。
- **重启不再导致推荐页永久为空** —— 评估被打断遗留的候选卡死已根治，启动即自愈。
- **浏览器网页翻译不再写坏配置** —— 设置页下拉框对翻译免疫，后端拒收被翻译污染的 provider 值。
