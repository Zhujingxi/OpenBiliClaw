...
📌 最新版本：**v0.3.183（2026-07-23）**

- **同一种模型渠道也能建多个独立实例** —— 每个实例单独配置 Base URL、token、模型与协议，并按顺序组成全局或模块级故障切换链。
- **模型选择不再全靠手填** —— PC Web、插件和 Setup 可从当前 OpenAI 兼容端点真实获取模型列表，Effort 提供本地建议且始终允许手填。
- **旧配置升级可恢复、可回退** —— 旧 Provider 配置无损投影，首次迁移永久备份原文件，也能导出给旧版本读取的兼容配置。
