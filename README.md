...
│       InspirationKeywordPipeline: 轴库 yield 学习闭环 + 生命周期 + breadth 收敛 │
...
│               discovery_inspiration_* · content_cache · recommendations · chat_turns │
│               avoidance_state                                      │
...
关键词生成侧可选启用 search-backed query inspiration：planner 从 like 二级兴趣按 coverage 抽样，先脑暴搜索 probe，再按配置的 provider 链（默认已启用平台源 → Exa → You.com free MCP）搜索灵感；平台源只把 B站 / YouTube / X / Reddit、抖音 direct client，以及小红书 / 知乎 bridge 可用时的搜索结果当 evidence，不写候选池。regular + explore 同轮触发时共享一次 brainstorm / grounding 预算，B 站 / 抖音 / X 等风险源还会受独立 probe budget、每 probe 页数预算和 cooldown / 限流约束。随后 `discovery.keyword_inspiration` 生成按平台区分、带溯源的具体搜索词；实验模式下可完全跳过旧 merged keyword planner，只用新流程给各平台 regular 关键词池产词，并在 B 站 explore 到期时补探索词池。replace 前可用 `keyword-inspiration-report` 对比 inspiration / merged cohort 的准入、delight 和 topic 多样性。这套 ①–⑥ 编排已抽成独立 `InspirationKeywordPipeline`；可复用轴库还会**学习**：每轮 production 取轴前用纯 SQL 回填真实成绩（好轴上浮、坏轴沉底），持续零产出或过时的轴自动 stale/retire 并清理，探索广度由单个 `inspiration_breadth`（low/medium/high）档位控制，可选 embedding 近邻还会把近义轴合并去重（服务不可用时无损降级）。
