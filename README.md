...
📌 最新版本：**v0.3.156（2026-07-05）**

- **Firefox 版抖音 / Reddit 采集修复** —— 打包版注入路径错误导致两个来源在 Firefox 上一直采不到内容，现已修复。
- **非 B站 推荐不再空置数小时** —— 推荐出流对有存量的平台做保底补位，知乎 / 小红书 / 抖音标签页更快出现内容。
- **初始化失败直接显示具体原因** —— 不再只有一句「初始化过程中出错了」，三端都能看到真实报错细节。
- **备选 LLM Provider 可诊断可测试** —— 同名备选保存被拒、静默失效有告警，设置页新增「测试备选 Provider」按钮。
