...
📌 最新版本：**v0.3.164（2026-07-13）**

- **推荐补货更快、更可靠** —— 持续评估与文案生成采用持久化微批和库存优先级控制，避免推荐池见底后长时间无内容。
- **Web 交互更流畅** —— 首屏渐进加载、侧栏动画、Delight 容器响应与滚动阈值都经过真实浏览器回归，慢请求不再拖住主要内容。
- **Delight 与探针状态一致** —— 拖拽、切卡、提交中、失败重试和动效反馈在重渲染及多端入口中保持一致，减少误触与重复提交。
- **对话失败安全可恢复** —— 模型失败会回滚临时历史、持久化安全的类型化错误且不触发学习，CLI、Web 与 OpenClaw 使用同一失败语义。
